...
function buildPrompt(userIdea,patterns,{voiceTranscript="",referenceImages=[],language="en",industry="technology",terminology=[],createDemoVideo=false,themeMode="auto",themePreset="auto",primaryColor="",accentColor="",backgroundColor="",styleRequest=""}={}) {
...
export async function runAutonomousEngine(userIdea,options={}) {
  ...
  const promptOptions={voiceTranscript,referenceImages,language,industry,terminology,createDemoVideo,themeMode,themePreset,primaryColor,accentColor,backgroundColor,styleRequest};
  ...
  return {status:"preview_ready",idea:combinedIdea,specification,aiProvider:provider,...(model?{aiModel:model}:{}),intelligence:{engine:"Soolen AI",product:PRODUCT_BRAND.name,industryPatternsMatched:patterns.length,patternLibrary:"industry_patterns",voiceInput:Boolean(voiceTranscript),referenceImages:referenceImages.length,language:buildSoolenGenerationContext({language,industry,terminology}).language.name,terminologyCount:buildSoolenGenerationContext({language,industry,terminology}).terminology.length,themeMode,themePreset,media:"local-first",demoVideo:createDemoVideo},nextStep:"preview",test:{status:"pending"},security:{status:"pending"},publish:{allowed:false,requiresHumanApproval:true}};
}
