...
The HMAC secret must remain server-side. Requests older/newer than the allowed clock window are rejected. A nonce may only be claimed once. A stable message idempotency key may only be claimed once per service client; a retry is accepted only when the original request body hash matches.
