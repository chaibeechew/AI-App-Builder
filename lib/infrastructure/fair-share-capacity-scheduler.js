...
export const LANERIQ_FAIR_SHARE_SCHEDULER_VERSION = "2026-09-04.2";
...
function normalizeTenant(entry, index, starvationAgeMs) {
...
function allocateWeighted({ candidates, capacity, hardCap, initialAllocations = new Map() }) {
  const allocations = new Map(candidates.map((tenant) => [tenant.tenantId, initialAllocations.get(tenant.tenantId) || 0]));
  let remaining = Math.max(0, Math.floor(capacity));
  let active = candidates.filter((tenant) => {
    const current = allocations.get(tenant.tenantId) || 0;
    return !tenant.isolated && current < Math.min(tenant.demand, hardCap);
  });
  ...
  while (remaining > 0 && active.length > 0) {
    ...
    for (const tenant of active) {
      ...
      if (room <= 0) continue;
      const weighted = Math.max(1, Math.floor((roundCapacity * tenant.effectiveWeight) / totalWeight));
...
export function buildFairShareSchedule({
  ...
  const normalized = [...tenants]
    .map((entry, index) => normalizeTenant(entry, index, starvationAgeMs))
  ...
  const reservedUsed = [...criticalPass.allocations.values()].reduce((sum, value) => sum + value, 0);
  const initialAllocations = new Map(normalized.map((tenant) => [tenant.tenantId, criticalPass.allocations.get(tenant.tenantId) || 0]));
  const allPass = allocateWeighted({
    candidates: normalized,
    capacity: Math.max(0, totalCapacity - reservedUsed),
    hardCap,
    initialAllocations,
  });

  const rows = normalized.map((tenant) => {
    const allocated = Math.min(tenant.demand, allPass.allocations.get(tenant.tenantId) || 0);
