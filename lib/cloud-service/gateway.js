...
function productionRuntime(){return String(process.env.VERCEL_ENV||"").toLowerCase()==="production";}
...
export function cloudServiceMode(){
 ...
 if(productionRuntime())return oidc?"remote":"remote-unavailable";
 return oidc||secret.length>=32?"remote":"embedded";
}
...
export async function executeCloudService(input,{embedded}){
 ...
 const mode=cloudServiceMode();
 if(mode==="embedded")return embedded(checked.value);
 if(mode==="remote-unavailable"){const e=new Error("CLOUD_SERVICE_OIDC_REQUIRED");e.code="CLOUD_SERVICE_OIDC_REQUIRED";throw e;}
 const url=httpsUrl(process.env.LANERIQ_CLOUD_SERVICE_URL),secret=String(process.env.LANERIQ_CLOUD_SERVICE_SECRET||""),oidc=runtimeOidcToken();
