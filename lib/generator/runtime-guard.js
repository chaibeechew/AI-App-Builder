...
export function normalizeAppSpec(input) {
  ...
  const platforms=safeArray(raw.platforms).filter(value=>typeof value==="string"&&value.trim()).map(value=>value.trim().toLowerCase());
  return {
    ...
    platforms: platforms.length?[...new Set(platforms)]:["web"],
