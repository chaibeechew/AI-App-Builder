...
import { normalizeCinemaCameraInput } from './cinema-camera-engine.js';
...
function safeCamera(value){try{return normalizeCinemaCameraInput(value);}catch(error){throw new VideoGenerationGatewayError('Video camera controls are invalid.',clean(error?.message,100)||'VIDEO_GENERATION_CAMERA_INVALID',400);}}
...
export function buildVideoGenerationPayload({task,input={},requestId}={}){
  ...
  return{
    ...
    input:{
      ...
      camera:safeCamera(input.camera),motion:clean(input.motion,300)||null,motionStrength:safeNumber(input.motionStrength,0,100),
