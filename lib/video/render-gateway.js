...
function rendererHeaders(requestId="") {
...
export async function startVideoRender({ project, version, editJson, requestId }) {
  ...
  if (!config.configured) return { configured: false, started: false, status: "draft", jobId: null, outputPath: null, provider: null, admissionRoute: config.admissionRoute };
  ...
  try {
    response = await fetch(endpoint, {
      method: "POST",
      headers: rendererHeaders(stableRequestId),
      body: JSON.stringify({
        schemaVersion: 2,
  ...
  if (!response.ok) throw new VideoRenderGatewayError("The connected video renderer rejected this job.", cleanText(data?.code, 100) || "VIDEO_RENDER_REJECTED", response.status >= 400 && response.status < 600 ? response.status : 502);
  ...
  if (!jobId && !outputPath) throw new VideoRenderGatewayError("The connected video renderer returned no usable job or approved output.", "VIDEO_RENDER_INVALID_RESPONSE", 502);

  return { configured: true, started: true, status, jobId, outputPath, provider: config.provider, admissionRoute: config.admissionRoute, chargeRequired: config.chargeRequired, zeroCostExecution: config.zeroCostExecution };
}
...
export async function checkVideoRenderStatus({ jobId }) {
  const config = getVideoRendererConfig();
  if (config.blockedByCostPolicy) throw new VideoRenderGatewayError("Connected video rendering is blocked by the active cost policy.", "VIDEO_RENDER_COST_POLICY_BLOCKED", 403);
  if (!config.configured || !config.statusEndpoint) return { checked: false, provider: config.provider, status: null, outputPath: null, jobId, admissionRoute: config.admissionRoute };
  ...
  if (outputPath) status = "completed";
  return { checked: true, provider: config.provider, status, outputPath, jobId, admissionRoute: config.admissionRoute, zeroCostExecution: config.zeroCostExecution };
}
