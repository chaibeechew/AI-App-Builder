...
function safeId(value){const id=clean(value,180);return OPAQUE_ID.test(id)?id:null;}
function safeIds(value,max=24){return [...new Set((Array.isArray(value)?value:[]).map(v=>typeof v==="string"?v:v?.id).map(safeId).filter(Boolean))].slice(0,max);}
function safeDigest(value){const digest=clean(value,64).toLowerCase();return SHA256.test(digest)?digest:null;}
function safeTruth(value){return [...new Set((Array.isArray(value)?value:[]).map(v=>clean(v,80)).filter(Boolean))].slice(0,12);}
function safeLifecycle(value={}){const source=value&&typeof value==="object"&&!Array.isArray(value)?value:{};return{task:clean(source.task,120)||null,parentAssetIds:safeIds(source.parentAssetIds),retryOf:safeId(source.retryOf),supersedes:safeId(source.supersedes),promptHash:safeDigest(source.promptHash),versionNo:Math.max(1,Math.floor(Number(source.versionNo)||1)),truth:safeTruth(source.truth),quality:source.quality&&typeof source.quality==="object"?{score:Number.isFinite(Number(source.quality.score))?Math.max(0,Math.min(100,Number(source.quality.score))):null,decision:clean(source.quality.decision,40)||null,gatePassed:source.quality.gatePassed===true}:null,evidenceDigest:safeDigest(source.evidenceDigest)};}
...
async function resolveOwnedParentIds(admin,userId,requested){const input=safeIds(requested);if(!input.length)return[];const{data,error}=await admin.from("asset_library").select("id").eq("user_id",userId).in("id",input);if(error)throw new DurableVideoOutputError("Video lineage validation failed.","VIDEO_LINEAGE_OWNER_LOOKUP_FAILED");const owned=new Set((data||[]).map(row=>row.id));if(owned.size!==input.length)throw new DurableVideoOutputError("Video lineage contains an unowned asset.","VIDEO_LINEAGE_OWNER_MISMATCH",403);return input.filter(id=>owned.has(id));}

export async function persistRenderedVideo({admin,userId,projectId,versionId,requestId,outputPath,lifecycle={}}){
  const life=safeLifecycle(lifecycle);const parentAssetIds=await resolveOwnedParentIds(admin,userId,life.parentAssetIds);const buffer=await fetchMp4(outputPath);const fingerprint=createHash("sha256").update(buffer).digest("hex");let asset=await existingAsset(admin,userId,fingerprint);let created=null;
  if(!asset){
    ...
    const intelligence={purpose:"video_render_output",generated:true,providerOutput:true,projectId,versionId,generationRequestId:requestId,reusableAcrossUsers:false,rawPrivateAssetsReusableAcrossCustomers:false,privateCustomerAsset:true,source:"renderer",lifecycleVersion:1,mediaTask:life.task,parentAssetIds,retryOf:life.retryOf,supersedes:life.supersedes,promptHash:life.promptHash,versionNo:life.versionNo,truth:life.truth,quality:life.quality,evidenceDigest:life.evidenceDigest,rawPromptStored:false};
