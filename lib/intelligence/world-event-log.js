...
export function forkWorldEventLog(log,{branchId,fromVersion=log?.headVersion}={}){
  const verified=verifyWorldEventLog(log);if(!verified.ok)throw new Error('UNIFIED_EVENT_LOG_TAMPERED');
  const branch=safeId(branchId,'UNIFIED_EVENT_BRANCH_ID_INVALID');
  const target=Math.floor(Number(fromVersion));
  if(target<log.baseVersion||target>log.headVersion)throw new Error('UNIFIED_EVENT_FORK_VERSION_INVALID');
  const selected=log.events.filter(row=>row.version<=target);
  const headHash=selected.length?selected[selected.length-1].eventHash:log.genesisHash;
  return freezeLog({...log,branchId:branch,headVersion:target,headHash,events:selected.map(row=>({...row,branchId:branch}))});
}
