...
export function assessActionAuthority({action={},authorization={},evidence={},securityAssessment={}}={}){
  ...
  const physical=action.physical===true;const external=action.external===true||physical;const irreversible=action.irreversible===true;
  const blockers=[];
  ...
  const governance=assessRealityEvidence({claimType:physical?'physical-action':'simulation',evidence,authorization,action:{...action,effects}});
  if(physical&&!governance.allowed)blockers.push(...governance.blockers);
  const securityStatus=securityIntelligenceCloudStatus();
  const allowed=[...new Set(blockers)].length===0;
  return freeze({
    allowed,truth:allowed?UNIFIED_TRUTH_LEVELS.OBSERVED_VERIFIED:UNIFIED_TRUTH_LEVELS.ACTION_AUTHORIZATION_REQUIRED,
    blockers:freeze([...new Set(blockers)]),governance,
    ...
    rule:'Security Intelligence supplies defensive risk evidence but cannot authorize CLEAN by itself. Action Authority separately requires scoped permission, trust evidence and human approval for irreversible effects.',
  });
}
