...
export function appendEvidence(ledger,input={}){
  ...
  if(LEVELS[level]>=LEVELS.SIGNED_OBSERVED&&!signatureVerified)throw new Error('UNIFIED_EVIDENCE_SIGNATURE_VERIFICATION_REQUIRED');
  ...
  const core={
    ...
    sourceId:clean(input.sourceId,160)||null,observedAt,expiresAt,independent,signed,signatureVerified,provenanceVerified,
...
export function assessEvidence(ledger,{evidenceIds=[],minimumLevel='OBSERVED',now=Date.now(),requireIndependent=false,artifactHash=null}={}){
  ...
  const eligible=rows.filter(row=>{
    const levelOk=LEVELS[row.level]>=LEVELS[minimum];
    const signatureOk=LEVELS[row.level]<LEVELS.SIGNED_OBSERVED||(row.signed===true&&row.signatureVerified===true);
    return levelOk&&signatureOk&&(!requireIndependent||row.independent===true)&&(!row.expiresAt||Date.parse(row.expiresAt)>Number(now))&&(!targetHash||row.artifactHash===targetHash);
  });
