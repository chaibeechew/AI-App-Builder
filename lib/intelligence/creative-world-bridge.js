...
export function proposeCreativeWorldUpdate({context,worldState,result,evidenceLedger,evidenceIds=[],eventId,entityId=null,entityKind='asset',attributes={}}={}){
  assertRealityContextMatches(context,{worldId:worldState?.worldId,projectId:worldState?.projectId,worldVersion:worldState?.version});
  const winner=result?.winner;const judgement=winner?.judgement;const observation=judgement?.observation||{};
  const artifactHash=observation.artifactHash||winner?.artifactHash||null;
  const accepted=result?.ok===true&&result?.status==='completed'&&result?.truth==='REAL_OUTPUT_QUALITY_VERIFIED'&&judgement?.productionEligible===true&&observation.ok===true&&observation.signedEvidence===true&&Array.isArray(observation.hardBlockers)&&observation.hardBlockers.length===0;
  ...
  const canonicalId=clean(entityId||context.characterId||context.assetId,160);if(!ID.test(canonicalId))return freeze({allowed:false,truth:UNIFIED_TRUTH_LEVELS.EVIDENCE_REQUIRED,blockers:freeze(['canonical-world-entity-id-required']),evidence:assessed});
  const normalizedEventId=clean(eventId,160);if(!ID.test(normalizedEventId))return freeze({allowed:false,truth:UNIFIED_TRUTH_LEVELS.EVIDENCE_REQUIRED,blockers:freeze(['world-event-id-required']),evidence:assessed});
  ...
  const logEvent=freeze({eventId:normalizedEventId,expectedVersion:worldState.version,type:'entity.upsert',actorType:'creative-media',reason:'accepted-observed-creative-output',reversible:true,evidenceRefs:freeze([...evidenceIds]),patch:freeze({entity})});
