import { createBuilderProjectDataAdapter } from "../cloud-adapters/builder-project-data.js";
...
export async function loadBuilderGenerationReplay({ requestId }) {
  ...
  return Object.freeze({ ok: true, principal: result.principal, app: result.app || null, version: result.version || null });
}
...
export async function saveBuilderModification(payload) {
  ...
  return Object.freeze({ ok: true, principal: result.principal, version: result.version, replayed: result.replayed, memorySaved: result.memorySaved });
}
