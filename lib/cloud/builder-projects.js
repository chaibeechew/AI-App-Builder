...
export async function loadBuilderGenerationInputs({ assetIds = [] } = {}) {
  ...
  if (!result?.ok) return fail("GENERATION_INPUTS_UNAVAILABLE", result?.detail || null);
...
export async function createBuilderStorePublishRequest(payload) {
  ...
  if (!result?.ok) return fail("STORE_PUBLISH_REQUEST_FAILED", result?.detail || null);
...
export async function saveBuilderStoreListing(payload) {
  ...
  if (!result?.ok) return fail("STORE_LISTING_SAVE_FAILED", result?.detail || null);
