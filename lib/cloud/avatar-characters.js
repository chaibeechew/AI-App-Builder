...
function manifestOk(manifest,characterId){if(!manifest||typeof manifest!=="object"||Array.isArray(manifest))return false;if(manifest.schema!=="laneriq.living-character"||manifest.characterId!==characterId)return false;const version=Number(manifest.schemaVersion);return Number.isInteger(version)&&version>=2&&version<=3&&Buffer.byteLength(JSON.stringify(manifest),"utf8")<=32768;}
