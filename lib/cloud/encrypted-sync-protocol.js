...
function normalizeSeenOperations(seenOperations) {
  const byId = new Map();
  for (const item of seenOperations || []) {
    if (!item || typeof item !== "object" || Array.isArray(item)) throw new Error("LANERIQ_SYNC_SEEN_OPERATION_INVALID");
    const operationId = boundedId(item.operationId, "SEEN_OPERATION_ID");
    const ciphertextHash = String(item.ciphertextHash || "").trim();
    if (!ciphertextHash || ciphertextHash.length > 128) throw new Error("LANERIQ_SYNC_SEEN_CIPHERTEXT_HASH_INVALID");
    if (byId.has(operationId) && byId.get(operationId) !== ciphertextHash) throw new Error("LANERIQ_SYNC_SEEN_OPERATION_AMBIGUOUS");
    byId.set(operationId, ciphertextHash);
  }
  return byId;
}
...
export function assessEncryptedSyncApply(record, { currentRevision = 0, seenOperations = [] } = {}) {
  const operationId = boundedId(record?.operationId, "OPERATION_ID");
  const ciphertextHash = String(record?.ciphertextHash || "").trim();
  if (!ciphertextHash || ciphertextHash.length > 128) throw new Error("LANERIQ_SYNC_CIPHERTEXT_HASH_INVALID");
  const revision = positiveRevision(record?.revision, "REVISION");
  const previousRevision = positiveRevision(record?.previousRevision, "PREVIOUS_REVISION", { allowZero: true });
  const current = positiveRevision(currentRevision, "CURRENT_REVISION", { allowZero: true });
  const seen = normalizeSeenOperations(seenOperations);
  if (seen.has(operationId)) {
    if (seen.get(operationId) !== ciphertextHash) {
      return Object.freeze({ accepted: false, replayed: false, conflict: true, replayMismatch: true, nextRevision: current });
    }
    return Object.freeze({ accepted: true, replayed: true, conflict: false, replayMismatch: false, nextRevision: current });
  }
  if (previousRevision !== current || revision !== current + 1) {
    return Object.freeze({ accepted: false, replayed: false, conflict: true, replayMismatch: false, nextRevision: current });
  }
  return Object.freeze({ accepted: true, replayed: false, conflict: false, replayMismatch: false, nextRevision: revision });
}
...
export function publicEncryptedSyncProtocolPolicy() {
  return Object.freeze({
    ...
    contextBound: true,
    replayAware: true,
    replayRequiresCiphertextIdentity: true,
