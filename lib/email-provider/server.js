...
function safeTransportErrorCode(error){
  ...
  return SAFE_TRANSPORT_ERROR_CODES.has(code)?code:"transport_failed";
}
function normalizeManagedEmailRuntime(){
  const host=String(process.env.SMTP_HOST||"").trim().toLowerCase();
  if(host!=="smtp.gmail.com")return;
  const original=String(process.env.SMTP_PASS||"");
  const cleaned=original
    .normalize("NFKC")
    .replace(/[\s\u200B-\u200D\uFEFF]+/g,"")
    .replace(/^[`'"“”‘’]+|[`'"“”‘’]+$/g,"");
  if(cleaned&&cleaned!==original)process.env.SMTP_PASS=cleaned;
}
...
export async function processLaneriqEmail({messageId:requestedId=null}={}){
  ...
  try{
    normalizeManagedEmailRuntime();
    const delivered=await sendManagedEmail(payload);
    ...
    if(attempts>=maxAttempts){
      await finish(admin,{messageId:claimed.message_id,status:"failed",errorCode:"transport_not_ready"});
      return {status:"failed",channel:"email",provider:PROVIDER_NAME,messageId:claimed.message_id,errorCode:"transport_not_ready"};
    }
    await finish(admin,{messageId:claimed.message_id,status:"deferred",errorCode:"transport_not_ready",retryAfterSeconds:300});
    return {status:"deferred",channel:"email",provider:PROVIDER_NAME,messageId:claimed.message_id};
  }catch(error){
    const attempts=Number(claimed.attempts||1);
    const maxAttempts=Number(claimed.max_attempts||DEFAULT_MAX_ATTEMPTS);
    const terminal=attempts>=maxAttempts;
    const errorCode=safeTransportErrorCode(error);
    await finish(admin,{messageId:claimed.message_id,status:terminal?"failed":"deferred",errorCode,retryAfterSeconds:terminal?null:300});
    return {status:terminal?"failed":"deferred",channel:"email",provider:PROVIDER_NAME,messageId:claimed.message_id,errorCode};
  }
}
...
export async function sendLaneriqEmail(payload){
  ...
  if(delivered.status==="deferred")return {status:"integration_required",channel:"email",provider:PROVIDER_NAME,messageId:queued.messageId,errorCode:delivered.errorCode||null};
  return delivered;
}