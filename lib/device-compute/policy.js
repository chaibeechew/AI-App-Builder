...
export const DEVICE_COMPUTE_POLICY_VERSION = "2026-09-05.3";
...
function applyUnknownThermalGuard(budget, deviceClass) {
  const factor = deviceClass === "mobile" ? 0.65 : deviceClass === "tablet" ? 0.7 : deviceClass === "laptop" ? 0.85 : 0.9;
  const guarded = scaleBudget(budget, factor);
  if (deviceClass === "mobile") {
    guarded.burstSeconds = Math.min(guarded.burstSeconds, 20);
    guarded.recoverySeconds = Math.max(guarded.recoverySeconds, 30);
  } else if (deviceClass === "tablet") {
    guarded.burstSeconds = Math.min(guarded.burstSeconds, 30);
    guarded.recoverySeconds = Math.max(guarded.recoverySeconds, 30);
  }
  return guarded;
}
...
export function computeDeviceBudget(input = {}) {
  ...
  let reason = settings.localComputeEnabled ? "local_compute_enabled" : connectivity.internetAvailable ? "cloud_only_selected" : ownDesktopReachable ? "same_user_desktop_available" : "offline_local_compute_disabled";
