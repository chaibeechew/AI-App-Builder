export const NATIVE_RESOURCE_GUARDIAN_VERSION = "2026-09-05.2";
...
export function applyNativeAdmissionToBudget(budget = {}, admission = {}) {
  if (!admission.allowed) {
    const route = budget.ownDeviceMeshAvailable ? "own_desktop" : budget.internetAvailable ? "cloud_fallback" : "offline_queue";
    return Object.freeze({
      ...budget,
      route,
      ...
      nativeGuardianReason: admission.reason || "blocked",
      nativeGuardianMaxAllowedShare: 0,
    });
  }

  const cap = clamp(admission.maxAllowedShare, 0, 0.05);
  const limit = (value) => Math.min(Math.max(0, Number(value || 0)), cap);
  ...
  return Object.freeze({
    ...
    schedulerDutyCycleShare: limit(budget.schedulerDutyCycleShare),
    nativeGuardianBlocked: false,
    nativeGuardianReason: admission.reason,
