...
import { buildFusedEvidence,getTrendFusionStatus } from './trendFusionEngine.js';
...
function getLegacyTrendStatus(snapshot,now){
  ...
  return {
    ...
    mode:'single-source-legacy',
    region:snapshot?.region||null,
    targetMarket:snapshot?.region||null,
...
export function getTrendLearningStatus(options={}){
  if(options.snapshot)return getLegacyTrendStatus(options.snapshot,options.now);
  return getTrendFusionStatus({
    input:options.input||'',
    market:options.market,
    now:options.now,
    fusionSnapshot:options.fusionSnapshot||TREND_FUSION_SNAPSHOT,
  });
}

function buildLegacyEvidence(snapshot,now){
  const status=getLegacyTrendStatus(snapshot,now),evidence=new Map();
  ...
  for(const [patternId,ev] of evidence){
    const trend=clamp((ev.bestRankWeight*0.85)+Math.min(0.15,(ev.mentions-1)*0.03))*status.freshnessWeight;
    output.set(patternId,{trend:Number(trend.toFixed(4)),evidenceCount:ev.mentions,chartCount:ev.charts.size,marketCount:1,sourceCount:1,markets:[snapshot?.region||'unknown']});
  }
  return output;
}

export function scoreTrendPatterns(input,options={}){
  const text=normalize(input);
  const evidence=options.snapshot
    ?buildLegacyEvidence(options.snapshot,options.now)
    :buildFusedEvidence({input,market:options.market,now:options.now,fusionSnapshot:options.fusionSnapshot||TREND_FUSION_SNAPSHOT});
  return TRENDING_APP_REFERENCE_PATTERNS.map((ref,index)=>{
    const ev=evidence.get(ref.id),relevance=intentScore(text,ref),trend=ev?.trend||0;
    return {
      ref,
      index,
      relevance,
      ...
      chartCount:ev?.chartCount||0,
      marketCount:ev?.marketCount||0,
      sourceCount:ev?.sourceCount||0,
      markets:ev?.markets||[],
...
export function selectAdaptiveTrendingReferences(input,options={}){
  const bounded=Math.max(1,Math.min(Number(options.limit)||5,8));
  return scoreTrendPatterns(input,options).slice(0,bounded).map(item=>({
    ...item.ref,
    ...
    evidenceMarkets:item.markets,
  }));
}

export function buildAdaptiveTrendingReferenceContext(input,options={}){
  const refs=selectAdaptiveTrendingReferences(input,{...options,limit:5});
  const status=getTrendLearningStatus({...options,input});
  const activeMarkets=Array.isArray(status.activeMarkets)&&status.activeMarkets.length?status.activeMarkets.join(', '):status.region||'none';
  const lines=[
    'LANERIQ AI Continuous Trend Learning Layer:',
    'LANERIQ AI Multi-Source Trend Fusion Layer:',
    `Trend evidence: ${status.freshness} · region ${status.region||'GLOBAL'} · target ${status.targetMarket||'GLOBAL'} · active markets ${activeMarkets} · ${status.evidencePatterns} pattern families evidenced.`,
    ...refs.map(ref=>`- ${ref.family}: ${ref.patterns.join(', ')}${ref.trendScore>0?` [trend ${ref.trendScore.toFixed(2)} · markets ${ref.marketCount||1}]`:''}`),
  ...
  if(status.sourceRegistry?.providerReady>0)lines.push('Provider-ready trend adapters without verified snapshots carry zero LIVE scoring weight until evidence is connected.');
  return lines.join('\n');
}

export { TREND_SIGNAL_SNAPSHOT };
