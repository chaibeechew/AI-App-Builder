...
function exactIndustrySignal(text,industry){const needle=industry.toLowerCase();const index=text.indexOf(needle);if(index<0)return false;const before=index===0?' ':text[index-1],after=index+needle.length>=text.length?' ':text[index+needle.length];return !/[a-z0-9]/.test(before)&&!/[a-z0-9]/.test(after);}

export function detectIndustryIntent(input){const text=normalize(input);const exact=INDUSTRIES.filter(industry=>exactIndustrySignal(text,industry)).sort((a,b)=>b.length-a.length);if(exact.length){const industry=exact[0];return {industry,confidence:0.98};}let best=null;let bestScore=0;for(const industry of INDUSTRIES){const score=scoreSignals(text,signalsFor(industry));if(score>bestScore){best=industry;bestScore=score}}return {industry:best,confidence:bestScore>0?Math.min(0.98,0.48+bestScore/24):0};}
