...
function signalsFor(industry){return INDUSTRY_SIGNALS[industry]||INDUSTRY_INTELLIGENCE_PROFILES[industry]?.keywords||[industry.toLowerCase()]}
...
export function selectIndustryTemplateBlend(input,{limit=6}={}){
  const {industry,confidence}=detectIndustryIntent(input), explicitArchetypes=detectArchetypeIntent(input), styleId=detectStyleIntent(input), blueprint=profileFor(industry);
