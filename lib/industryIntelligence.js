import { ARCHETYPES, INDUSTRIES, STYLES, getTemplateCatalog } from './templateCatalog.js';
...
const ARCHETYPE_SIGNALS = Object.freeze({
  booking:['booking','reservation','appointment','schedule','book','预约','预订','订位'],
  crm:['crm','lead','customer','client','pipeline','sales','顾客','客户','线索'],
  marketplace:['marketplace','listing','directory','browse','seller','buyer','平台','市场','列表'],
  store:['store','shop','cart','checkout','order','commerce','购物','商店','下单'],
  directory:['directory','map','listing','find nearby','目录','地图','附近'],
  operations:['operations','job tracking','workflow','dispatch','team','运营','工单','派单'],
  membership:['membership','subscription','member','portal','会员','订阅'],
  learning:['learning','course','lesson','quiz','student','课程','学习','测验'],
  service:['service','quote','project','consulting','服务','报价','项目'],
  inventory:['inventory','stock','supplier','warehouse','库存','供应商','仓库'],
  community:['community','social','feed','group','post','社区','动态','群组'],
  analytics:['analytics','report','dashboard','metric','kpi','分析','报表','指标'],
});

const STYLE_SIGNALS = Object.freeze({
  luxury:['luxury','premium','editorial','high-end','高级','豪华','奢华'],
  glass:['glass','cinematic','immersive','liquid glass','玻璃','电影感','沉浸'],
  'dark-tech':['dark','tech','futuristic','cyber','深色','科技','未来'],
  natural:['natural','warm','organic','human','自然','温暖','人性化'],
  minimal:['minimal','clean','simple','modern','极简','简洁','现代'],
});
...
export function detectIndustryIntent(input){
  const text=normalize(input);let best=null;let bestScore=0;
  for(const industry of INDUSTRIES){const score=scoreSignals(text,INDUSTRY_SIGNALS[industry]||[industry.toLowerCase()]);if(score>bestScore){best=industry;bestScore=score}}
  return {industry:best,confidence:bestScore>0?Math.min(0.98,0.48+bestScore/24):0};
}

export function detectArchetypeIntent(input){
  const text=normalize(input);return Object.entries(ARCHETYPE_SIGNALS).map(([id,signals])=>({id,score:scoreSignals(text,signals)})).filter(x=>x.score>0).sort((a,b)=>b.score-a.score).map(x=>x.id);
}

export function detectStyleIntent(input){
  const text=normalize(input);return Object.entries(STYLE_SIGNALS).map(([id,signals])=>({id,score:scoreSignals(text,signals)})).filter(x=>x.score>0).sort((a,b)=>b.score-a.score)[0]?.id||null;
}

export function selectIndustryTemplateBlend(input,{limit=6}={}){
  const {industry,confidence}=detectIndustryIntent(input);const explicitArchetypes=detectArchetypeIntent(input);const styleId=detectStyleIntent(input);
  const blueprint=INDUSTRY_BLUEPRINTS[industry]||{archetypes:[],entities:[],workflow:[]};
  const archetypeIds=[...new Set([...explicitArchetypes,...blueprint.archetypes])].slice(0,6);
  const catalog=getTemplateCatalog();
  const styles=styleId?[styleId,...STYLES.map(s=>s.id).filter(id=>id!==styleId)]:['glass','minimal','luxury','dark-tech','natural'];
  const selected=[];
  for(const archetypeId of archetypeIds){
    for(const sid of styles){const hit=catalog.find(t=>(!industry||t.industry===industry)&&t.archetypeId===archetypeId&&t.styleId===sid);if(hit){selected.push(hit);break}if(selected.length>=limit)break}
    if(selected.length>=limit)break;
  }
  if(!selected.length&&industry){selected.push(...catalog.filter(t=>t.industry===industry).sort((a,b)=>b.score-a.score).slice(0,Math.min(limit,4)))}
  return {industry,confidence,archetypeIds,styleId,entities:blueprint.entities||[],workflow:blueprint.workflow||[],templates:selected.slice(0,Math.max(1,Math.min(Number(limit)||6,8)))};
}

export function buildIndustryIntelligenceContext(input){
  const blend=selectIndustryTemplateBlend(input,{limit:6});
  if(!blend.industry&&!blend.templates.length)return '';
  const lines=['LANERIQ AI Industry Intelligence Engine:'];
