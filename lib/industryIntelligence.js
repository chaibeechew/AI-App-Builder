import { INDUSTRIES, STYLES, getTemplateCatalog } from './templateCatalog.js';
...
export function detectIndustryIntent(input){const text=normalize(input);let best=null;let bestScore=0;for(const industry of INDUSTRIES){const score=scoreSignals(text,INDUSTRY_SIGNALS[industry]||[industry.toLowerCase()]);if(score>bestScore){best=industry;bestScore=score}}return {industry:best,confidence:bestScore>0?Math.min(0.98,0.48+bestScore/24):0};}
...
export function selectIndustryTemplateBlend(input,{limit=6}={}){
  const {industry,confidence}=detectIndustryIntent(input), explicitArchetypes=detectArchetypeIntent(input), styleId=detectStyleIntent(input), blueprint=INDUSTRY_BLUEPRINTS[industry]||{archetypes:[],entities:[],workflow:[]};
  const archetypeIds=[...new Set([...explicitArchetypes,...blueprint.archetypes])].slice(0,6), catalog=getTemplateCatalog(), styles=styleId?[styleId,...STYLES.map(s=>s.id).filter(id=>id!==styleId)]:['glass','minimal','luxury','dark-tech','natural'], selected=[];
  for(const archetypeId of archetypeIds){for(const sid of styles){const hit=catalog.find(t=>(!industry||t.industry===industry)&&t.archetypeId===archetypeId&&t.styleId===sid);if(hit){selected.push(hit);break}}if(selected.length>=limit)break}
  ...
  return {industry,confidence,archetypeIds,styleId,entities:blueprint.entities||[],workflow:blueprint.workflow||[],templates:selected.slice(0,Math.max(1,Math.min(Number(limit)||6,8))),trendingReferences:selectTrendingAppReferences(input,{limit:5})};
}

export function buildIndustryIntelligenceContext(input){
  const blend=selectIndustryTemplateBlend(input,{limit:6}), lines=['LANERIQ AI Industry Intelligence Engine:'];
