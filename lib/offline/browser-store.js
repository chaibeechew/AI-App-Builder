...
export async function listOfflineMutations({ scopeKey, state } = {}) {
  ...
  let records;
  if (normalizedState) {
    records = await withStore(MUTATION_STORE, "readonly", (store) => requestResult(store.index("by_scope_state").getAll([scopeKey, normalizedState])));
  } else {
    records = await withStore(MUTATION_STORE, "readonly", (store) => requestResult(store.index("by_scope_project").getAll(IDBKeyRange.bound([scopeKey, ""], [scopeKey, "\uffff"]))));
  }
  return (records || []).filter((record) => {
    try { return assertExactOfflineScope(record, scopeKey); } catch { return false; }
