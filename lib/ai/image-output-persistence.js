...
async function resolveOwnedParentIds(admin,userId,requested){
  const input=safeIds(requested);if(!input.length)return[];
  const{data,error}=await admin.from("asset_library").select("id").eq("user_id",userId).in("id",input);
  if(error)throw new DurableImageOutputError("Generated image lineage validation failed.","IMAGE_LINEAGE_OWNER_LOOKUP_FAILED");
  const owned=new Set((data||[]).map(row=>row.id));if(owned.size!==input.length)throw new DurableImageOutputError("Generated image lineage contains an unowned asset.","IMAGE_LINEAGE_OWNER_MISMATCH");
  return input.filter(id=>owned.has(id));
}
...
export async function persistGeneratedImages({admin,userId,requestId,items,mode,style,palette,placement,lifecycle={}}){
  const durable=[];const created=[];const life=safeLifecycle(lifecycle);const parentAssetIds=await resolveOwnedParentIds(admin,userId,life.parentAssetIds);
  try{
    for(let index=0;index<items.length;index++){
      ...
      if(!asset){
        ...
        const intelligence={purpose:"image_studio_generated",generated:true,mode,style,providerOutput:true,generationRequestId:requestId,reusableAcrossUsers:false,rawPrivateAssetsReusableAcrossCustomers:false,privateCustomerAsset:true,source:"model",lifecycleVersion:1,mediaTask:life.task,parentAssetIds,retryOf:life.retryOf,supersedes:life.supersedes,promptHash:life.promptHash,versionNo:life.versionNo,truth:life.truth,quality:life.quality,evidenceDigest:life.evidenceDigest,rawPromptStored:false};
