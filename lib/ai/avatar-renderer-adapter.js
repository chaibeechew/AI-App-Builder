...
export function createAvatarRendererPlan(manifest,{profile,thermalState="nominal",batteryLevel=1,lowPowerMode=false,reducedMotion=false,background=false,deviceTier="mid",viewportWidth=390,viewportHeight=844,devicePixelRatio=2}={}){
  ...
  return{
    ...
    budget:{
      ...base,
      particles:Boolean(runtimeProfile.particles&&!reducedMotion&&renderer!=="2.5d"),
      secondaryMotion:Boolean(base.secondaryMotion&&runtimeProfile.secondaryMotion&&!reducedMotion)
    },
...
export function buildAvatarRenderPacket({plan,runtimeState,faceFrame,nowMs=0}={}){
  ...
  const semanticMotion={
    idle:"idle-breathe",
    listening:"attention-lean",
    thinking:"thinking-hold",
    speaking:"speech-support",
    acting:"action-focus",
    success:"success-release",
    concerned:"concern-hold"
  }[state]||"idle-breathe";
  const face=buildFaceRigCommand(faceFrame||{},{maxChannels:plan.maxFaceChannels});
  return{
    contract:plan.contract,
    characterId:plan.characterId,
    timestampMs:Math.max(0,Math.round(Number(nowMs)||0)),
    ...
    semanticMotion,
    face,
    body:{motion:semanticMotion,secondaryMotion:Boolean(plan.budget?.secondaryMotion),motionIntensity:plan.adaptation?.reducedMotion?.2:state==="success"?.85:state==="acting"?.72:.5},
    effects:{particles:Boolean(plan.budget?.particles&&state==="success"),shadows:plan.budget?.shadows,lighting:plan.budget?.lighting},
...
export function shouldReplanAvatarRenderer(plan,signals={}){
  if(!plan)return true;
  const nextProfile=selectAvatarPerformanceProfile(signals);
  if(nextProfile!==plan.profile)return true;
  if(Boolean(signals.reducedMotion)!==Boolean(plan.adaptation?.reducedMotion))return true;
  if(Boolean(signals.background)!==Boolean(plan.adaptation?.background))return true;
  if(clean(signals.thermalState,20)&&clean(signals.thermalState,20)!==plan.adaptation?.thermalState)return true;
  return false;
}
...
export function buildMobileAvatarSurfaceContract({platform="generic"}={}){
  ...
  return{
    contract:"laneriq-mobile-avatar-surface-v1",
    platform:["ios","android"].includes(p)?p:"generic",
    primarySurface:"in-app-character",
    surfaces:["in-app-character","compact-character-card","voice-session-surface","notification-state"],
    continuousCharacterRendering:"in-app-only",
    backgroundPolicy:"state-only-no-continuous-render",
