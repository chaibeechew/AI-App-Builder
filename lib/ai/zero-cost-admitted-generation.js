...
export async function generateWithZeroCostAdmission(prompt, {
  ...
  const reuse = reuseAllowed
    ? lookupSemanticReuse({ scope, purpose, keyMaterial, variant: reuseVariant, reuseClass, allowApproximate: allowApproximateReuse })
    : Object.freeze({ hit: false, exact: false, approximate: false, reason: "reuse_disabled" });
  ...
  if (admission.route === "REUSE") {
    runtime.reuseHits += 1;
    return providerResult({ provider: "laneriq-semantic-reuse", result: reuse.result, attempts: 0, errors: [] }, admission, "reuse", reuse);
  }
...
export function getZeroCostAdmissionRuntimeTruth() {
  ...
  return Object.freeze({
    ...
    evidenceBoundary: "Admission telemetry is per-runtime routing evidence. It does not prove provider billing statements, permanent free quota, native-device inference, or unlimited compute.",
  });
}
