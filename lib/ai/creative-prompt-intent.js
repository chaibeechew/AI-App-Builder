...
function scoreFamily(text,rule){let score=0;const matches=[];for(const term of rule.terms){if(text.includes(term)){score+=term.length>=6?3:2;matches.push(term);}}return {family:rule.family,score,matches};}
