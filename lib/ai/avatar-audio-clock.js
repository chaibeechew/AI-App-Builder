...
export function advanceAvatarAudioClock(clock,{nowMs,reportedPlaybackMs,paused=false,buffering=false,maxSoftDriftMs=80,maxHardDriftMs=240}={}){
  if(!clock?.sessionId)throw new Error("AVATAR_AUDIO_CLOCK_REQUIRED");
  const now=Math.max(clock.lastNowMs||0,Number(nowMs)||0),dt=Math.max(0,now-(clock.lastNowMs||now));
  ...
  const abs=Math.abs(drift),soft=Math.max(20,Number(maxSoftDriftMs)||80),hard=Math.max(soft,Number(maxHardDriftMs)||240);
  let playback=predicted,rate=1,resyncCount=clock.resyncCount||0;
  if(hasReported&&abs>=hard){playback=reported;resyncCount+=1;}
  else if(hasReported&&abs>=soft&&!buffering){rate=clamp(1+drift/1600,0.94,1.06);playback=predicted;}
  else if(hasReported){playback=predicted+drift*.18;}
  if(paused||buffering){rate=0;playback=hasReported?reported:clock.playbackMs;}
  const sampleJitter=hasReported?Math.abs(reported-(clock.lastReportedPlaybackMs||0)-dt):0;
  const jitter=(clock.jitterMs||0)*.8+sampleJitter*.2;
...
export function getAvatarAudioClockFrame(clock){
  if(!clock?.sessionId)throw new Error("AVATAR_AUDIO_CLOCK_REQUIRED");
  return{contract:clock.contract,sessionId:clock.sessionId,playbackMs:Math.max(0,Math.round(clock.playbackMs||0)),rate:clock.rate||0,driftMs:Math.round(clock.driftMs||0),jitterMs:Math.round(clock.jitterMs||0),resyncCount:clock.resyncCount||0,stable:Math.abs(clock.driftMs||0)<120&&(clock.jitterMs||0)<120,paused:Boolean(clock.paused)};
}

export function shouldHardResyncAvatarAudio(clock,{driftLimitMs=240,jitterLimitMs=180}={}){
  return Math.abs(Number(clock?.driftMs)||0)>=Math.max(80,Number(driftLimitMs)||240)||(Number(clock?.jitterMs)||0)>=Math.max(80,Number(jitterLimitMs)||180);
}
