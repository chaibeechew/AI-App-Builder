...
export function inferMobileGamePlan(idea=""){
  ...
  const wants3d=/\b3d\b/i.test(source)||/three.?dimensional/i.test(source)||/三维|三維|3D/.test(source),wantsMultiplayer=/multiplayer|pvp|co.?op|多人|联机|聯機/i.test(source),wantsAds=/\bads?\b|advertis|rewarded video|interstitial|广告|廣告/i.test(source),wantsIap=/in.?app purchase|iap|purchase|coins|gems|battle pass|内购|內購|充值/i.test(source),wantsCamera=/camera|ar game|augmented reality|相机|相機|增强现实|擴增實境/i.test(source),wantsLocation=/gps|location game|nearby|地图游戏|地圖遊戲|位置游戏|位置遊戲/i.test(source);

  const screens=["Boot / loading","Home / lobby","Gameplay","Pause","Results / rewards","Settings / accessibility"];if(wantsMultiplayer)screens.push("Matchmaking / room","Friends / party or session status");if(wantsIap)screens.push("Store / inventory");
  const systems=[
    "core gameplay loop with explicit start, player action, immediate feedback, reward, win/lose and replay states",
    "touch-first input abstraction with drag/D-pad/gesture choices, pointer-cancel recovery, safe areas, optional haptics and keyboard preview",
    ...
    "performance budget targeting responsive 60fps where practical, bounded frame delta, entity caps, memory/texture limits, battery and thermal awareness",
  ...
  const platformRules=[
    ...
    "Prepare identifiers, icons, screenshots, privacy declarations, age/content ratings and signing/store checklists, but never claim submission or approval without evidence."
  ];
  ...
  const rows=[
    ...
    `Detected product: Mobile Game. Genre: ${genre.label}. Runtime: ${wants3d?"3D-capable roadmap; prove a playable vertical slice before expanding":"2D-first playable vertical slice; expand only after runtime evidence passes"}.`,
  ...
  return{matched:true,productType:"mobile_game",genreId:genre.id,genre:genre.label,dimensions:wants3d?"3d-capable":"adaptive-2d-3d",platforms:["ios","android","web-preview"],screens,systems,multiplayer:wantsMultiplayer,monetization:{ads:wantsAds,inAppPurchases:wantsIap},permissions:{camera:wantsCamera,location:wantsLocation},mediaCapabilities:["ai-art","ai-video","ai-photo-video","ai-avatar"],qualityTarget:100,qualityDimensions:["gameplay","controls","physics","progression","save","feedback","mobile","performance","accessibility","reliability"],brief:rows.join("\n")};
}
