...
import {inferUniversalGameCreationPlan,buildCompleteGameSpec} from "../game/universal-game-creation-core-v1.js";
import {inferRepositoryAwareCapabilities} from "../game/repository-aware-game-development-agent-v5.js";
...
export function inferMobileGamePlan(idea=""){
  ...
  const universalPlan=inferUniversalGameCreationPlan(source);
  const repositoryPlan=inferRepositoryAwareCapabilities(source);
  ...
  const completeGameSpec=buildCompleteGameSpec({idea:source,genre:genre.id,dimensions:wants3d?"3d-capable":"adaptive-2d-3d",online:wantsMultiplayer,commerce:{iap:wantsIap,ads:wantsAds,receiptVerificationContract:true,restorePurchases:true,parentalPurchaseSafeguard:true,consentGate:true},privacy:{permissions:[...(wantsCamera?["camera"]:[]),...(wantsLocation?["location"]:[])],permissionPurposes:{camera:"gameplay-camera-feature",location:"gameplay-location-feature"},parentalControlsReadiness:true,moderation:true,reportBlock:true,dataMinimization:true},build:{targets:["ios","android","web-preview"],safeArea:true,androidBack:true,lifecycleRecovery:true,lowMemoryMode:true}});
  ...
  systems.push(...universalPlan.systems.map(item=>`UNIVERSAL GAME CREATION: ${item}`));
  systems.push(...taxonomyPlan.systems.map(item=>`TAXONOMY: ${item}`));
  if(repositoryPlan.matched)systems.push(...repositoryPlan.systems.map(item=>`REPOSITORY-AWARE V5: ${item}`),`REPOSITORY-AWARE V5 TRUTH: ${repositoryPlan.truthRule}`);
  ...
  const rows=[
    ...
    `UNIVERSAL GAME CREATION CORE: ${universalPlan.internalCapabilityCount} shared cross-genre capabilities across ${universalPlan.domains.length} domains. ${universalPlan.truthRule}`,
    taxonomyPlan.brief,
    ...
    repositoryPlan.matched?`REPOSITORY-AWARE DEVELOPMENT V5: ${repositoryPlan.systems.join(" ")} ${repositoryPlan.truthRule}`:"Repository-aware V5 is available when repository/commit/diff evidence is part of the request.",
