...
export function inferMobileGamePlan(idea=""){
  ...
  const genre=mobaPlan.matched?{id:"moba",label:"MOBA / 5v5 Hero Battler"}:aviationPlan.matched?{id:"air_combat",label:"Air Combat / Flight"}:(GENRES.find(item=>hasAny(source,item.patterns))||{id:"custom",label:"Custom Mobile Game"});
  const wants3d=aviationPlan.matched||/\b3d\b/i.test(source)||/three.?dimensional/i.test(source)||/三维|三維|3D/.test(source),wantsMultiplayer=mobaPlan.matched||aviationPlan.multiplayer||/multiplayer|pvp|co.?op|多人|联机|聯機/i.test(source),wantsAds=/\bads?\b|advertis|rewarded video|interstitial|广告|廣告/i.test(source),wantsIap=/in.?app purchase|iap|purchase|coins|gems|battle pass|内购|內購|充值/i.test(source),wantsCamera=/camera|ar game|augmented reality|相机|相機|增强现实|擴增實境/i.test(source),wantsLocation=/gps|location game|nearby|地图游戏|地圖遊戲|位置游戏|位置遊戲/i.test(source);

  const screens=["Boot / loading","Home / lobby","Gameplay","Pause","Results / rewards","Settings / accessibility"];if(wantsMultiplayer)screens.push("Matchmaking / room","Friends / party or session status");if(wantsIap)screens.push("Store / inventory");if(mobaPlan.matched)screens.push("Hero select / team composition","Battle HUD / minimap / scoreboard","Item shop","Reconnect / match recovery");if(aviationPlan.matched)screens.push("Hangar / aircraft select","Mission briefing","Flight HUD / instruments","Map / navigation","Mission debrief","Controls / realism assists");
  const systems=[
