...
const SAFE_EVIDENCE_ID=/^[A-Za-z0-9._:-]{1,200}$/;
...
function safeBox(value){if(!value||typeof value!=='object'||Array.isArray(value))return null;const out={};for(const key of ['top','right','bottom','left','width','height','x','y']){const n=safeNumber(value[key],0,IMAGE_GENERATION_LIMITS.maxDimension);if(n!=null)out[key]=n;}return Object.keys(out).length?out:null;}
function safeEvidence(value){if(!value||typeof value!=='object'||Array.isArray(value))return null;const artifactHash=clean(value.artifactHash||value.sha256,128).toLowerCase();const providerRequestId=clean(value.providerRequestId||value.requestId,200);const provenanceId=clean(value.provenanceId||value.contentCredentialId,200);return{artifactHash:/^[a-f0-9]{64}$/.test(artifactHash)?artifactHash:null,providerRequestId:SAFE_EVIDENCE_ID.test(providerRequestId)?providerRequestId:null,provenanceId:SAFE_EVIDENCE_ID.test(provenanceId)?provenanceId:null,safetyPassed:value.safetyPassed===true};}
...
export function buildCreativeImagePayload({task,input={},requestId}={}){
  ...
  return{
    ...
    input:{
      ...
      seed:Number.isInteger(seed)?seed:null,count,transparent:input.transparent===true,canvas:safeBox(input.canvas),expand:safeBox(input.expand),
    ...
    referencesRequireOwnerValidation:true,
