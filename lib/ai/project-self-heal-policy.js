...
const TRUSTED_SECURITY_METADATA_PATHS=new Set([
  "security.secrets",
...
function isTrustedSecurityMetadataPath(path){return TRUSTED_SECURITY_METADATA_PATHS.has(path.map(part=>String(part)).join("."));}
...
export function inspectProjectSpecification(specification={}){
  ...
  // The MAX manifest itself intentionally contains three canonical metadata labels named secrets/tokens;
  // allow only those exact LANERIQ-owned paths, never an arbitrary subtree or nested credential field.
  walk(spec,(value,path)=>{const key=String(path[path.length-1]??"");if(key&&SECRET_FIELD.test(key)&&!isTrustedSecurityMetadataPath(path))add("ownership_permissions","error",`Secret-like field/key ${key} must not be embedded in the generated specification.`,{path:path.join(".")});});
