...
export function inspectProjectSpecification(specification={}){
  ...
  const routes=new Set(),routeOwners=new Map();
  ...
  pages.forEach((page,index)=>{
    ...
    if(route){if(routes.has(route))add("missing_routes","error",`${pageName} duplicates route ${route}.`,{page:index});routes.add(route);routeOwners.set(route,index);}
  });
  ...
  // ownership_permissions: inspect schema/config keys and data-field names, not ordinary explanatory prose.
  walk(spec,(value,path)=>{const key=String(path[path.length-1]??"");if(key&&SECRET_FIELD.test(key))add("ownership_permissions","error",`Secret-like field/key ${key} must not be embedded in the generated specification.`,{path:path.join(".")});});
  ...
  // api_failures: external-capability plans must include failure/recovery evidence.
  ...
  if(externalIntent&&!/(error|retry|timeout|fallback|fail|loading|status)/.test(stabilityText))add("api_failures","warning","External-capability projects should explicitly define loading/error/timeout/retry or fallback behavior.");
