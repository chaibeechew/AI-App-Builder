...
export async function generateExternalImages({ prompt, mode, style, palette, count, colors = {} }) {
  ...
  if (config.blockedByCostPolicy) throw new ImageGenerationGatewayError("Connected image generation is blocked by the active cost policy.", "IMAGE_GENERATION_COST_POLICY_BLOCKED", 403);
  if (!config.configured) return { configured: false, generated: false, provider: null, images: [], admissionRoute: config.admissionRoute };
  ...
  if (!response.ok) throw new ImageGenerationGatewayError("The connected image runtime rejected the request.", cleanText(data?.code, 100) || "IMAGE_GENERATION_REJECTED", response.status >= 400 && response.status < 600 ? response.status : 502);
  ...
  if (!images.length) throw new ImageGenerationGatewayError("The connected image runtime returned no usable approved image output.", "IMAGE_GENERATION_INVALID_RESPONSE", 502);
  return { configured: true, generated: true, provider: config.provider, images, admissionRoute: config.admissionRoute, chargeRequired: config.chargeRequired, zeroCostExecution: config.zeroCostExecution };
}
