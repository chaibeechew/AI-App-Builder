...
function securityEvidencePresent(specification){return Object.keys(object(specification?.security)).length>0;}
function buildDimensionRows({buildQuality,outcome,coverage,completeness,securityActive}){
...
function buildDirectives({dimensions,outcome,coverage,buildQuality,securityActive}){
  const directives=[];
...
export function assessGenerationQuality(specification={},options={}){
  ...
  const dimensions=buildDimensionRows({buildQuality,outcome,coverage,completeness,securityActive});
  ...
  const directives=buildDirectives({dimensions,outcome,coverage,buildQuality,securityActive});
