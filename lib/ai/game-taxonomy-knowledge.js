...
import {inferContentProductionCapabilities} from "../game/content-production-pipeline-v1.js";
import {inferGameStudioCapabilities} from "../game/game-studio-intelligence-v1.js";
import {inferAutonomousGameDirectorCapabilities} from "../game/autonomous-game-director-v2.js";
import {inferAutonomousDevelopmentCapabilities} from "../game/autonomous-game-development-agent-v3.js";
import {inferAutonomousDevelopmentV4Capabilities} from "../game/autonomous-game-development-agent-v4.js";
...
export function inferGameTaxonomy(idea=""){
  ...
  const contentProduction=inferContentProductionCapabilities(source);
  const gameStudio=inferGameStudioCapabilities(source);
  const autonomousDirector=inferAutonomousGameDirectorCapabilities(source);
  const autonomousDevelopment=inferAutonomousDevelopmentCapabilities(source);
  const autonomousDevelopmentV4=inferAutonomousDevelopmentV4Capabilities(source);
  ...
  if(contentProduction.matched)systems.push(...contentProduction.systems.map(item=>`CONTENT PRODUCTION: ${item}`));
  if(gameStudio.matched)systems.push(...gameStudio.systems.map(item=>`GAME STUDIO INTELLIGENCE: ${item}`));
  if(autonomousDirector.matched)systems.push(...autonomousDirector.systems.map(item=>`AUTONOMOUS GAME DIRECTOR V2: ${item}`));
  if(autonomousDevelopment.matched)systems.push(...autonomousDevelopment.systems.map(item=>`AUTONOMOUS DEVELOPMENT AGENT V3: ${item}`));
  if(autonomousDevelopmentV4.matched)systems.push(...autonomousDevelopmentV4.systems.map(item=>`AUTONOMOUS DEVELOPMENT AGENT V4: ${item}`));
  ...
  if(contentProduction.matched)systems.push(`CONTENT PRODUCTION TRUTH: ${contentProduction.truthRule}`);
  if(gameStudio.matched)systems.push(`GAME STUDIO INTELLIGENCE TRUTH: ${gameStudio.truthRule}`);
  if(autonomousDirector.matched)systems.push(`AUTONOMOUS GAME DIRECTOR V2 TRUTH: ${autonomousDirector.truthRule}`);
  if(autonomousDevelopment.matched)systems.push(`AUTONOMOUS DEVELOPMENT AGENT V3 TRUTH: ${autonomousDevelopment.truthRule}`);
  if(autonomousDevelopmentV4.matched)systems.push(`AUTONOMOUS DEVELOPMENT AGENT V4 TRUTH: ${autonomousDevelopmentV4.truthRule}`);
  return {
    ...
    brief:[
      ...
      `Content production pipeline: ${contentProduction.matched?contentProduction.systems.join(" "):"no dedicated large-game content pipeline extension inferred"}.`,
      `Game Studio Intelligence: ${gameStudio.matched?gameStudio.systems.join(" "):"no dedicated studio-intelligence extension inferred"}.`,
      `Autonomous Game Director V2: ${autonomousDirector.matched?autonomousDirector.systems.join(" "):"no dedicated autonomous QA/search extension inferred"}.`,
      `Autonomous Development Agent V3: ${autonomousDevelopment.matched?autonomousDevelopment.systems.join(" "):"no dedicated root-cause/regression development extension inferred"}.`,
      `Autonomous Development Agent V4: ${autonomousDevelopmentV4.matched?autonomousDevelopmentV4.systems.join(" "):"no dedicated bisect/mutation/desync/performance diagnostic extension inferred"}.`,
