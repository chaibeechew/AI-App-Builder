...
export function inferGameTaxonomy(idea=""){
  ...
  const explicitOffline=networkMatches.some(item=>item.id==="offline_single_player");
  const explicitOnline=networkMatches.some(item=>item.id!=="offline_single_player")||scaleMatches.some(item=>item.id!=="solo");
