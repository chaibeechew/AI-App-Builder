...
function inferFeatures(text){const explicit=guidedField(text,"What must it include\\?");const found=[];if(explicit)found.push(...explicit.split(/[,;，、]+/));const source=String(text||"");for(const [name,pattern] of FEATURE_RULES){const match=source.match(pattern);if(!match)continue;const index=Number(match.index||0),prefix=source.slice(Math.max(0,index-18),index);if(NEGATION_PATTERN.test(prefix)||/(不要|不用|移除|删除|刪除|排除)\s*$/.test(prefix))continue;found.push(name);}return unique(found,10);}
...
export function buildIdeaPlan(message,{modelPlan=null,previousPlan=null}={}){
  ...
  const messageFeatures=inferFeatures(text),messageAudience=inferAudience(text),messageDomain=inferDomain(text),messageGoal=explicitGoal(text);
  const previousFeatures=unique(previous.features,10),confirmedFeatures=unique([...previousFeatures,...messageFeatures],10);
  const audience=messageAudience||clean(previous.audience,300);
  const confirmedDomain=messageDomain||clean(previous.appType==="Unresolved"?"":previous.appType,120);
  const goal=messageGoal||clean(previous.intent,500);
  const readiness=evaluateIdeaReadiness(text,{audience,features:confirmedFeatures,domain:confirmedDomain,goal});
  const suggestedFeatures=unique(sanitized.features,8),features=unique([...confirmedFeatures,...suggestedFeatures],10);
  const appType=confirmedDomain||clean(sanitized.appType||sanitized.domain,120)||"Unresolved";
  ...
  return {
    ...
    intent:clean(goal||sanitized.intent||"Build an App + Website",500),
    audience:audience||"",
    appType,
    features,
    ...
    gameIntent:gameIntent(text,confirmedDomain,confirmedFeatures),
