...
function inferAudience(text){const explicit=guidedField(text,"Who will use this project\\?");if(explicit)return explicit;for(const [name,pattern] of AUDIENCE_RULES)if(pattern.test(text))return name;return"";}
function inferDomain(text){for(const [name,pattern] of DOMAIN_RULES)if(pattern.test(text))return name;return"";}
function inferFeatures(text){const explicit=guidedField(text,"What must it include\\?");const found=[];if(explicit)found.push(...explicit.split(/[,;，、]+/));for(const [name,pattern] of FEATURE_RULES)if(pattern.test(text))found.push(name);return unique(found,10);}
...
export function evaluateIdeaReadiness(message,{audience="",features=[],domain="",goal=""}={}){
  const text=clean(message,MAX_IDEA_LENGTH),language=detectIdeaLanguage(text);
  const inferredAudience=clean(audience,300)||inferAudience(text);
  const inferredFeatures=unique(features.length?features:inferFeatures(text),10);
  const inferredDomain=clean(domain,120)||inferDomain(text);
  const inferredGoal=clean(goal,500)||explicitGoal(text);
  const hasPurpose=Boolean(inferredDomain||inferredGoal||inferredFeatures.length>=2);
  const hasCapability=Boolean(inferredGoal||inferredFeatures.length>=1);
  const audienceSatisfied=Boolean(inferredAudience);
  const workflowSpecificity=Boolean(inferredGoal&&inferredFeatures.length>=1)||inferredFeatures.length>=2;
  const vague=genericOnly(text);
  const readyToBuild=Boolean(!vague&&hasPurpose&&hasCapability&&(audienceSatisfied||workflowSpecificity));
  const missing=[];
  if(!hasPurpose)missing.push("purpose");
  if(!hasCapability)missing.push("feature");
  if(!audienceSatisfied&&!workflowSpecificity)missing.push("audience");
  if(inferredDomain&&!inferredGoal&&!inferredFeatures.length)missing.push("goal");
  const questions=readyToBuild?[]:unique(missing.map(key=>localizedQuestion(language,key)),3);
  const signalCount=[hasPurpose,hasCapability,audienceSatisfied,workflowSpecificity,!vague].filter(Boolean).length;
...
export function buildIdeaPlan(message,{modelPlan=null,previousPlan=null}={}){
  const text=clean(message,MAX_IDEA_LENGTH),raw=safeObject(modelPlan),previous=safeObject(previousPlan),sanitized=stripSecretKeys(raw)||{};
  const currentFeatures=unique([...inferFeatures(text),...unique(sanitized.features,8)],10);
  const currentAudience=inferAudience(text)||clean(sanitized.audience,300);
  const currentDomain=inferDomain(text)||clean(sanitized.appType||sanitized.domain,120);
  const currentGoal=explicitGoal(text)||clean(sanitized.intent||sanitized.goal,500);
  const readiness=evaluateIdeaReadiness(text,{audience:currentAudience,features:currentFeatures,domain:currentDomain,goal:currentGoal});
  const isCorrection=CORRECTION_PATTERN.test(text);
  const previousFeatures=unique(previous.features,10);
  const features=isCorrection?readiness.features:unique([...readiness.features,...previousFeatures],10);
  const audience=readiness.audience||(!isCorrection?clean(previous.audience,300):"");
  const appType=readiness.domain||(!isCorrection?clean(previous.appType,120):"")||"Unresolved";
  const finalReadiness=evaluateIdeaReadiness(text,{audience,features,domain:appType==="Unresolved"?"":appType,goal:readiness.goal});
  const corrections=unique([...(Array.isArray(previous.corrections)?previous.corrections:[]),...(isCorrection?[text]:[])],8);
  return {
    normalizedIdea:text,
    language:finalReadiness.language,
    intent:clean(readiness.goal||sanitized.intent||previous.intent||"Build an App + Website",500),
    audience:audience||"",
    appType,
    features,
    entities:unique(sanitized.entities||previous.entities,10),
    constraints:unique(sanitized.constraints||previous.constraints,10),
    questions:finalReadiness.questions,
    corrections,
    confidence:finalReadiness.confidence,
    readyToBuild:finalReadiness.readyToBuild,
    gameIntent:gameIntent(text,finalReadiness.domain,features),
    rawPrivateAssetsReusableAcrossCustomers:false,
    readiness:finalReadiness.signals,
  };
}
