...
export function tickLivingAvatarSession(session,{nowMs,playbackMs,attentionTarget,performanceSignals={},reducedMotion,buffering=false,paused=false,actionKind="",speechEnergy=.5}={}){
  ...
  const body=advanceAvatarBodyRuntime(session.body,{nowMs:now,state:runtime.state,emotion:runtime.emotion,speechEnergy,actionKind,reducedMotion:signals.reducedMotion}),faceCommand=buildFaceRigCommand(face.frame,{maxChannels:renderer.maxFaceChannels}),bodyCommand=buildAvatarBodyRigCommand(body),renderPacket=buildAvatarRenderPacket({plan:renderer,runtimeState:runtime,faceFrame:face.frame,nowMs:now}),surfaceFrame=buildAvatar2_5DSurfaceFrame({renderPacket,bodyCommand,faceCommand});
  ...
  return{session:{...session,runtime,face,body,audioClock,renderer,surface,lastTickMs:now,sequence:(session.sequence||0)+1},frame:{timestampMs:now,state:runtime.state,emotion:runtime.emotion,audioClock:audioClockFrame,voice:voiceFrame,face:face.frame,faceCommand,body:bodyCommand,render:renderPacket,surface:surfaceFrame}};
}
