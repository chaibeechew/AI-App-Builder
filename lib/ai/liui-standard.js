...
function deriveEvidence(spec){
  ...
  return {
    ...
    performance:unique([...safeArray(liui?.evidence?.performance),text(design.motionDirection),"Performance mode defaults to BALANCED and reduces blur, animation, particles, 3D, refresh work and local AI load when runtime capability degrades.","Smoothness, battery, heat and frame stability take priority over visual effects.","Network adaptation uses compressed assets, skeleton UI, lazy loading and cache on slow connections, plus local-first queued actions with clear sync status offline."]),
    ...
    aiIntegration:unique([...safeArray(liui?.evidence?.aiIntegration),"Universal AI Command Layer translates natural-language goals into search, filter, generated UI and suggested actions.","Predictive actions may Suggest or Prepare; external or high-impact Execute operations remain permission-bound.","Personal UI Memory can learn layout and workflow preferences while remaining viewable, editable, disableable and resettable.","Voice Native UI exposes live transcription, intent detection, action preview and confirmation before high-risk execution."]),
...
ANTI-PATTERNS
No whole-page glass, animation everywhere, every button 3D, excessive accent colors/gradients, meaningless Bento, chatbot-dominated product, desktop simply shrunk to mobile, mobile simply enlarged into desktop, missing loading/empty/error recovery, beautiful homepage with weak inner pages, or unconfirmed high-risk execution.
