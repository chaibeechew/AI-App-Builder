import { selectIndustryTemplateBlend, detectArchetypeIntent } from "../industryIntelligence.js";
...
function canonicalText(value){return String(value||"").toLowerCase().replace(/&/g," and ").replace(/[^a-z0-9]+/g," ").replace(/\s+/g," ").trim();}
function explicitIndustryFromIdea(idea){
  const haystack=` ${canonicalText(idea)} `;
  return [...INDUSTRIES].sort((a,b)=>canonicalText(b).length-canonicalText(a).length).find(industry=>haystack.includes(` ${canonicalText(industry)} `))||null;
}
function resolveIndustryBlend(idea,variantKey){
  const explicitIndustry=explicitIndustryFromIdea(idea);
  if(!explicitIndustry)return selectIndustryTemplateBlend(idea,{limit:6,variantKey});
  const canonicalBlend=selectIndustryTemplateBlend(explicitIndustry,{limit:6,variantKey});
  ...
  return {
    ...
    archetypeIds:unique([...explicitArchetypes,...list(canonicalBlend.archetypeIds)]).slice(0,6),
...
export function expandZeroCostIndustrySpecification(specification={},idea="",options={}){
  ...
  const variantKey=`zero-cost-local-v1-${Math.max(0,Number(options.variationIndex)||0)}`;
  const blend=resolveIndustryBlend(idea,variantKey);
  ...
  return {
    ...
    zeroCostIndustryIntelligence:{version:ZERO_COST_INDUSTRY_EXPANDER_VERSION,matched:true,industry:blend.industry,confidence:Number(blend.confidence.toFixed(2)),explicitIndustry:Boolean(blend.explicitIndustry),archetypes:blend.archetypeIds.slice(0,6),entityCount:blend.entities.length,workflowCount:blend.workflow.length,inspirationCount:blend.templates.length,originalityMode:"inspiration-only",directCopyAllowed:false},
  };
}

export const ZERO_COST_INDUSTRY_EXPANDER_POLICY=Object.freeze({version:ZERO_COST_INDUSTRY_EXPANDER_VERSION,catalogIndustries:50,explicitCanonicalIndustryWins:true,usesExistingIndustryIntelligence:true,templateUse:"inspiration-only",directCopyAllowed:false,paidProviderRequired:false,paidEmbeddingRequired:false,vectorDatabaseRequired:false,dedicatedServerRequired:false});
