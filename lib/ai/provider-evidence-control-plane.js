...
const SIGNING_DOMAIN = "LANERIQ-PRVE2-HMAC-SHA256";
...
function registry() {
  if (!Array.isArray(globalThis.__LANERIQ_PROVIDER_EVIDENCE_RECEIPTS)) {
    globalThis.__LANERIQ_PROVIDER_EVIDENCE_RECEIPTS = [];
  }
  return globalThis.__LANERIQ_PROVIDER_EVIDENCE_RECEIPTS;
}
...
export function verifyProviderEvidenceReceipt(receipt, signature, { env = process.env, now = Date.now() } = {}) {
  ...
  const currentCostMode = getSoolenCostMode(env);
  ...
  if (normalized.costMode !== currentCostMode) errors.push("COST_MODE_MISMATCH");
...
function pruneRegistry({ env = process.env, now = Date.now() } = {}) {
  const entries = registry();
  const kept = entries
    .filter((entry) => verifyProviderEvidenceReceipt(entry.receipt, entry.signature, { env, now }).liveVerified)
    .sort((a, b) => Date.parse(b.receipt.observedAt) - Date.parse(a.receipt.observedAt))
    .slice(0, MAX_RECORDED_RECEIPTS);
  entries.splice(0, entries.length, ...kept);
  return entries;
}

export function recordSignedProviderEvidenceReceipt(receipt, signature, { env = process.env, now = Date.now() } = {}) {
  const verified = verifyProviderEvidenceReceipt(receipt, signature, { env, now });
  if (!verified.liveVerified || !verified.provider) return verified;

  const entries = pruneRegistry({ env, now });
  const provider = verified.provider;
  const normalized = normalizeReceipt(receipt);
  const next = entries.filter((entry) => String(entry.receipt?.provider || "").toLowerCase() !== provider);
  next.unshift(Object.freeze({ receipt: Object.freeze(normalized), signature: String(signature || "") }));
  entries.splice(0, entries.length, ...next.slice(0, MAX_RECORDED_RECEIPTS));
  return verified;
}

export function getRecordedProviderEvidenceReceipts({ env = process.env, now = Date.now() } = {}) {
  return Object.freeze(pruneRegistry({ env, now }).map((entry) => Object.freeze({ receipt: entry.receipt, signature: entry.signature })));
}

export function resetProviderEvidenceRegistryForTests() {
  registry().splice(0);
}
...
export function buildInternalProviderEvidenceSnapshot(health = [], { env = process.env, now = Date.now(), signedReceipts = null } = {}) {
  const receipts = Array.isArray(signedReceipts) ? signedReceipts : getRecordedProviderEvidenceReceipts({ env, now });
  const providers = health.map((item) => Object.freeze({
    provider: String(item.provider || ""),
    configured: Boolean(item.configured),
    state: runtimeState(item, receipts, env, now),
...
export function buildPublicProviderEvidenceSummary(health = [], { env = process.env, now = Date.now(), signedReceipts = null } = {}) {
  ...
  const receiptRegistrySize = getRecordedProviderEvidenceReceipts({ env, now }).length;
  return Object.freeze({
    contract: PROVIDER_EVIDENCE_CONTRACT,
    providerIdentityInternalOnly: true,
