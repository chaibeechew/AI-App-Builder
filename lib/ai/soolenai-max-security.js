...
export function buildSoolenMaxSecurityManifest(existing={}){
  ...
  return {
    ...
    authentication:{...safeObject(prior.authentication),trustedServerIdentity:true,leastPrivilege:true,clientClaimsNotTrusted:true},
    data:{...safeObject(prior.data),rowLevelSecurity:true,ownerIsolation:true,denyByDefault:true},
    secrets:{...safeObject(prior.secrets),serverOnly:true,clientExposureForbidden:true},
    requests:{...safeObject(prior.requests),schemaValidation:true,sizeBounds:true,sameOriginMutationProtection:true,antiReplay:true,rateLimits:true},
    browser:{...safeObject(prior.browser),csp:true,hsts:true,noSniff:true,frameAncestors:true,strictReferrerPolicy:true,restrictivePermissionsPolicy:true},
    network:{...safeObject(prior.network),ssrfProtection:true,httpsAllowlist:true,privateNetworkBlocked:true,redirectBounds:true,responseBounds:true},
    code:{...safeObject(prior.code),evalForbidden:true,newFunctionForbidden:true,untrustedRemoteScriptsForbidden:true},
    uploads:{...safeObject(prior.uploads),untrustedByDefault:true,exactTypeAllowlist:true,extensionSignatureAgreement:true,activeContentBlocked:true,archivesBlockedByDefault:true,ownerIsolation:true,quarantineWhenRequired:true,scannerFailClosedWhenRequired:true,cleanClaimRequiresHashBoundEvidence:true},
    dependencies:{...safeObject(prior.dependencies),pinned:true,lockfile:true,criticalHighFindingsBlockRelease:true},
    logging:{...safeObject(prior.logging),secretsRedacted:true,tokensRedacted:true,privateContentRedacted:true,noClientStackLeak:true},
    release:{...safeObject(prior.release),blockCriticalHigh:true,manifestRequired:true,defaultVisibility:"private",defaultPublishStatus:"draft"}
  };
}
