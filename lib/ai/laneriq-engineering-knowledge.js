...
export function getLaneriqEngineeringKnowledge(){return{contract:VERSION,domains:CORE,avatar:getAvatarEngineeringKnowledge(),learningContract:"laneriq-governed-experience-learning-v1"};}
...
export function createLaneriqEngineeringProfile({focus=[],platform="web",mode="balanced"}={}){
  ...
  return{contract:VERSION,platform:clean(platform,24)||"web",mode:clean(mode,24)||"balanced",domains:selected.map(id=>({id,objective:CORE[id].objective,rules:[...CORE[id].rules]})),truthPrinciples:["code-ready-is-not-live-ready","exact-sha-evidence","provider-neutral-core","owner-scoped-private-data","zero-free-no-silent-spend","mobile-no-cross-user-community-compute","candidate-lessons-never-self-promote"]};
}
