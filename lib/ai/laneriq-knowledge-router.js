...
export function buildKnowledgePacket(options={}){
  ...
  const rules=routed.profile.domains.flatMap(domain=>domain.rules.slice(0,3)).slice(0,18);
  return{
    contract:"laneriq-knowledge-packet-v1",
    selectedDomains:routed.selectedDomains,
    risk:routed.risk,
    truthPrinciples:routed.profile.truthPrinciples,
    rules,
    instruction:["LANERIQ TASK-SCOPED ENGINEERING KNOWLEDGE:",...rules.map((rule,index)=>`${index+1}. ${rule}.`),"AI output remains a candidate until deterministic validation and authority/evidence gates accept it.","Never self-promote a candidate lesson into a Production rule."].join("\n")
  };
}
