...
import { getZeroCostAdmissionRuntimeTruth } from "./zero-cost-admitted-generation.js";
...
export function providerRouterProductionTruth(env = process.env) {
  ...
  const zeroCostAdmission = getZeroCostAdmissionRuntimeTruth();
  const providerEvidence = buildPublicProviderEvidenceSummary(getProviderRuntimeHealth(), { env });
  return Object.freeze({
    ...
    codeCapabilities: Object.freeze({
      ...
      zeroCostAdmissionController: true,
      semanticReuseNetworkV2: true,
      scopedPrivateReuse: true,
      providerEvidenceControlPlaneV2: true,
      signedProviderEvidenceReceipts: true,
      exactShaBoundProviderEvidence: true,
    }),
