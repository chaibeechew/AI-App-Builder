// LANERIQ AI canonical inspiration template catalog.
// 50 industries × 12 app archetypes × 5 visual styles = 3,000 bounded templates.
...
function freezeTemplate(template) {
  Object.freeze(template.pages);
  Object.freeze(template.features);
  Object.freeze(template.styleTags);
  Object.freeze(template.targets);
  Object.freeze(template.responsive.breakpoints);
  Object.freeze(template.responsive);
  Object.freeze(template.application);
    ...
    for (const archetype of ARCHETYPES) {
      for (const style of STYLES) {
        templates.push(freezeTemplate({
          ...
          description: `${archetype.name} inspiration for ${industry}, using a ${style.name} visual direction.`,
...
export function getTemplateCatalog() {
  if (!catalogCache) catalogCache = Object.freeze(buildTemplateCatalog());
  return catalogCache;
}
...
export function findTemplateById(id) {
  ...
  if (!/^tpl-\d{4}-[a-z0-9-]{3,100}$/.test(safeId)) return null;
  return getTemplateCatalog().find((template) => template.id === safeId) || null;
}

export function searchTemplates({ q = "", industry = "", style = "", archetype = "", limit = 24, offset = 0 } = {}) {
  const needle = String(q).trim().slice(0, 120).toLowerCase();
  const normalizedIndustry = String(industry).trim().slice(0, 80).toLowerCase();
  const normalizedStyle = String(style).trim().slice(0, 80).toLowerCase();
  const normalizedArchetype = String(archetype).trim().slice(0, 80).toLowerCase();
  ...
  const safeOffset = Math.max(0, Math.min(Number(offset) || 0, TEMPLATE_CATALOG_STATS.templates));
...
export const TEMPLATE_CATALOG_STATS = Object.freeze({
  schemaVersion: TEMPLATE_SCHEMA_VERSION,
