...
function smtpFailure(code){
  const error=new Error("Email delivery failed.");
  error.code=String(code||"smtp_transport_failed");
  return error;
}
function normalizeSmtpFailure(error,fallback="smtp_connection_failed"){
  const safeCode=String(error?.code||"").toLowerCase();
  if(safeCode.startsWith("smtp_"))return error;
  const nodeCode=String(error?.code||"").toUpperCase();
  if(["ETIMEDOUT","ESOCKETTIMEDOUT"].includes(nodeCode))return smtpFailure("smtp_timeout");
  if(["ECONNREFUSED","ECONNRESET","EHOSTUNREACH","ENETUNREACH","EAI_AGAIN","ENOTFOUND","EPIPE"].includes(nodeCode))return smtpFailure("smtp_connection_failed");
  const message=String(error?.message||"").toLowerCase();
  if(message.includes("timed out")||message.includes("timeout"))return smtpFailure("smtp_timeout");
  if(message.includes("certificate")||message.includes("tls")||message.includes("ssl"))return smtpFailure("smtp_tls_failed");
  return smtpFailure(fallback);
}
...
function smtpResponseClient(socket){
  ...
  function fail(error){
    ...
    terminalError=error instanceof Error?error:smtpFailure("smtp_connection_failed");
  ...
  function onError(error){fail(normalizeSmtpFailure(error,"smtp_connection_failed"));}
  function onTimeout(){fail(smtpFailure("smtp_timeout"));socket.destroy();}
  function onClose(){if(waiters.length)fail(smtpFailure("smtp_connection_closed"));}
...
function expectSmtp(response,allowed,code="smtp_protocol_failed"){
  if(!response||!allowed.includes(response.code))throw smtpFailure(code);
}
function smtpTimeoutMs(){return Math.max(5000,Math.min(30000,Number(process.env.SMTP_TIMEOUT_MS)||15000));}
function smtpPassword(host,value){
  const pass=String(value||"");
  return String(host||"").trim().toLowerCase()==="smtp.gmail.com"?pass.replace(/\s+/g,""):pass;
}

async function openManagedSmtpConnection({host,port}){
  ...
  if(port===465){
    ...
    try{
      ...
      expectSmtp(await responses.next(),[220],"smtp_greeting_failed");
      socket.write("EHLO laneriq.local\r\n");
      expectSmtp(await responses.next(),[250],"smtp_ehlo_failed");
      return {socket,responses};
    }catch(error){
      if(socket&&!socket.destroyed)socket.destroy();
      throw normalizeSmtpFailure(error,"smtp_tls_failed");
    }
  }

  const raw=net.connect({host,port});
  raw.setTimeout(timeout);
  const plainResponses=smtpResponseClient(raw);
  try{
    expectSmtp(await plainResponses.next(),[220],"smtp_greeting_failed");
    raw.write("EHLO laneriq.local\r\n");
    expectSmtp(await plainResponses.next(),[250],"smtp_ehlo_failed");
    raw.write("STARTTLS\r\n");
    expectSmtp(await plainResponses.next(),[220],"smtp_starttls_failed");
    plainResponses.detach();

    const socket=await new Promise((resolve,reject)=>{
      let secure;
      const onError=error=>reject(normalizeSmtpFailure(error,"smtp_tls_failed"));
      secure=tls.connect({socket:raw,servername:host,rejectUnauthorized:true},()=>{
    ...
    plainResponses.detach();
    raw.destroy();
    throw normalizeSmtpFailure(error,"smtp_connection_failed");
  }
}

async function authenticateManagedSmtp({socket,responses,user,pass}){
  const auth=Buffer.from(`\u0000${user}\u0000${pass}`,"utf8").toString("base64");
  socket.write(`AUTH PLAIN ${auth}\r\n`);
  const plain=await responses.next();
  if(plain?.code===235)return;

  socket.write("AUTH LOGIN\r\n");
  expectSmtp(await responses.next(),[334],"smtp_auth_failed");
  socket.write(`${Buffer.from(user,"utf8").toString("base64")}\r\n`);
  expectSmtp(await responses.next(),[334],"smtp_auth_failed");
  socket.write(`${Buffer.from(pass,"utf8").toString("base64")}\r\n`);
  expectSmtp(await responses.next(),[235],"smtp_auth_failed");
}
...
async function sendManagedSmtpEmail({to,subject,text,html}){
  ...
  if(!host||!pass)throw smtpFailure("smtp_config_missing");

  const {socket,responses}=await openManagedSmtpConnection({host,port});
  try{
    await authenticateManagedSmtp({socket,responses,user,pass});
    socket.write(`MAIL FROM:<${envelopeFrom}>\r\n`);
    expectSmtp(await responses.next(),[250],"smtp_sender_rejected");
    socket.write(`RCPT TO:<${recipient}>\r\n`);
    expectSmtp(await responses.next(),[250,251],"smtp_recipient_rejected");
    socket.write("DATA\r\n");
    expectSmtp(await responses.next(),[354],"smtp_data_rejected");
    socket.write(`${smtpMessage({from,to:recipient,subject,text,html})}\r\n.\r\n`);
    expectSmtp(await responses.next(),[250],"smtp_message_rejected");
    socket.write("QUIT\r\n");
...
async function sendManagedResendEmail({to,subject,text,html}){
  ...
  if(!response.ok){const error=new Error("Email delivery failed.");error.code="resend_transport_failed";throw error;}
