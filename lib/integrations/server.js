...
export async function sendManagedWhatsApp({to,body}){
  ...
  const response=await fetch(`https://graph.facebook.com/v23.0/${phoneId}/messages`,{
    ...
    body:JSON.stringify({messaging_product:"whatsapp",recipient_type:"individual",to:safeText(to,80).replace(/[^0-9]/g,""),type:"text",text:{preview_url:false,body:safeText(body,4000)}})
  });
  const data=await response.json().catch(()=>({}));
...
async function googleAccessToken(){
  ...
  const response=await fetch("https://oauth2.googleapis.com/token",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:new URLSearchParams({grant_type:"urn:ietf:params:oauth-type:jwt-bearer",assertion}).toString()});
  const data=await response.json().catch(()=>({}));
