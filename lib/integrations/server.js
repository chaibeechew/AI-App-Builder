...
export async function sendManagedEmail({to,subject,text,html}){
  const status=integrationStatus().email;
  if(!status.ready)return {status:"integration_required",channel:"email"};
  const response=await fetch("https://api.resend.com/emails",{
    method:"POST",
    headers:{Authorization:`Bearer ${process.env.RESEND_API_KEY}`,"Content-Type":"application/json"},
    body:JSON.stringify({from:process.env.EMAIL_FROM,to:[safeText(to,320)],subject:safeText(subject,180),text:safeText(text,12000),html:html?safeText(html,20000):undefined})
  });
...
export async function sendManagedSms({to,body}){
  ...
  if(!status.ready)return {status:"integration_required",channel:"sms"};
  ...
  const response=await fetch(`https://api.twilio.com/2010-04-01/Accounts/${encodeURIComponent(sid)}/Messages.json`,{
    method:"POST",headers:{Authorization:`Basic ${Buffer.from(`${sid}:${token}`).toString("base64")}`,"Content-Type":"application/x-www-form-urlencoded"},body:form.toString()
  });
  const data=await response.json().catch(()=>({}));
...
export async function createManagedCalendarEvent({summary,description,start,end,timeZone="UTC",attendeeEmail}){
  const status=integrationStatus().calendar;
  if(!status.ready)return {status:"integration_required",channel:"calendar"};
...
export async function createManagedCheckout({name,description,amount,currency="usd",mode="payment",successUrl,cancelUrl,clientReferenceId}){
  ...
  form.set("success_url",safeText(successUrl,1500));
  form.set("cancel_url",safeText(cancelUrl,1500));
  form.set("line_items[0][quantity]","1");
  form.set("line_items[0][price_data][currency]",safeText(currency,10).toLowerCase());
  form.set("line_items[0][price_data][unit_amount]",String(cents));
  form.set("line_items[0][price_data][product_data][name]",safeText(name,180));
  if(description)form.set("line_items[0][price_data][product_data][description]",safeText(description,500));
  if(recurring)form.set("line_items[0][price_data][recurring][interval]","month");
  if(clientReferenceId)form.set("client_reference_id",safeText(clientReferenceId,200));
