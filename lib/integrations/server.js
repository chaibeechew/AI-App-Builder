...
function smtpResponseClient(socket){
  ...
  function onError(){fail(new Error("Email delivery connection failed."));}
  function onTimeout(){fail(new Error("Email delivery timed out."));socket.destroy();}
  function onClose(){if(waiters.length)fail(new Error("Email delivery connection closed unexpectedly."));}
...
async function openManagedSmtpConnection({host,port}){
  ...
  if(port===465){
    ...
    expectSmtp(await responses.next(),[220]);
  ...
  try{
    expectSmtp(await plainResponses.next(),[220]);
    raw.write("EHLO laneriq.local\r\n");
    expectSmtp(await plainResponses.next(),[250]);
    raw.write("STARTTLS\r\n");
    expectSmtp(await plainResponses.next(),[220]);
...
async function authenticateManagedSmtp({socket,responses,user,pass}){
  ...
  expectSmtp(await responses.next(),[334]);
