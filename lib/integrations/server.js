...
function laneriqTransportConfig(){
  const raw=String(process.env.LANERIQ_EMAIL_TRANSPORT_URL||"").trim();
  const secret=String(process.env.LANERIQ_EMAIL_TRANSPORT_SECRET||"");
  if(!raw||secret.length<32||!process.env.EMAIL_FROM)return null;
  try{
...
function laneriqTransportReady(){return Boolean(laneriqTransportConfig());}
function smtpReady(){
...
export function integrationStatus(){
  return {
    email:{ready:laneriqTransportReady()||smtpReady()||resendReady(),managed:true},
...
async function sendLaneriqDirectEmail({to,subject,text,html,laneriqMessageId}){
  const config=laneriqTransportConfig();
  if(!config)return {status:"integration_required",channel:"email"};
  const messageId=safeHeader(laneriqMessageId,160);
  if(!/^lqem_[a-zA-Z0-9_-]{10,}$/.test(messageId))return {status:"integration_required",channel:"email"};
  const body=JSON.stringify({
    messageId,
    to:mailbox(to),
    subject:safeHeader(subject,180),
    text:safeText(text,12000),
    html:html?safeText(html,20000):undefined,
  });
  const timestamp=String(Date.now());
  const signature=crypto.createHmac("sha256",config.secret).update(`${timestamp}.${body}`).digest("hex");
  const endpoint=new URL("/v1/deliver",config.url);
  const response=await providerFetch(endpoint,{method:"POST",headers:{"Content-Type":"application/json","X-LANERIQ-Timestamp":timestamp,"X-LANERIQ-Signature":signature},body},25000);
  const data=await response.json().catch(()=>({}));
  if(response.ok&&data?.status==="sent")return {status:"completed",channel:"email",messageId};
  if(response.status===503||data?.status==="deferred")return {status:"deferred",channel:"email",messageId,errorCode:safeHeader(data?.errorCode,120)||"transport_deferred"};
  if(response.status===422||data?.status==="failed")return {status:"failed",channel:"email",messageId,errorCode:safeHeader(data?.errorCode,120)||"transport_failed"};
  throw new Error("LANERIQ Email Transport returned an invalid response.");
}
...
export async function sendManagedEmail(payload){
  if(laneriqTransportReady()&&/^lqem_[a-zA-Z0-9_-]{10,}$/.test(String(payload?.laneriqMessageId||"")))return sendLaneriqDirectEmail(payload);
