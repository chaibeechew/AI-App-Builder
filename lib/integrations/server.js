...
export async function sendManagedSms({to,body}){
  ...
  const response=await providerFetch(`https://api.twilio.com/2010-04-01/Accounts/${encodeURIComponent(sid)}/Messages.json`,{method:"POST",headers:{Authorization:`Basic ${Buffer.from(`${sid}:${token}`).toString("base64")}`,"Content-Type":"application/x-www-form-urlencoded"},body:form.toString()});
  const data=await response.json().catch(()=>({}));
...
async function googleAccessToken(){
  ...
  const response=await providerFetch("https://oauth2.googleapis.com/token",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:new URLSearchParams({grant_type:"urn:ietf:params:oauth-type:jwt-bearer",assertion}).toString()}).catch(async(error)=>{
    // Keep compatibility with the standard OAuth grant string if a provider rejects the defensive alias above.
    if(String(error?.message||"").includes("timed out"))throw error;
    return providerFetch("https://oauth2.googleapis.com/token",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:new URLSearchParams({grant_type:"urn:ietf:params:oauth:grant-type:jwt-bearer",assertion}).toString()});
  });
  const data=await response.json().catch(()=>({}));
