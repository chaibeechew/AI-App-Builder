...
const PUBLIC_CLOUD_LIVE_CANARY_ENDPOINTS = new Set(["/api/cloud/service-status"]);
const PUBLIC_PROVIDER_ROUTER_LIVE_CANARY_ENDPOINTS = new Set(["/api/ai/provider-router/status"]);
...
export async function updateSession(request) {
  ...
  // The Cloud live canary is kept in its own exact-path set so the established mobile QA
  // observability surface cannot grow silently. It is read-only and performs only a signed project.read canary.
  if (PUBLIC_CLOUD_LIVE_CANARY_ENDPOINTS.has(pathname) && (request.method === "GET" || request.method === "HEAD")) {
    response.headers.set("Cache-Control", "no-store, max-age=0");
    response.headers.set("X-Content-Type-Options", "nosniff");
    return response;
  }

  // Provider Router live verification uses one exact read-only path. Its canary executes only the
  // fixed soolen-local zero-cost provider and the route exposes sanitized aggregate truth only.
  // It cannot invoke a metered/external provider, mutate user data, or bypass auth for any other AI API.
  if (PUBLIC_PROVIDER_ROUTER_LIVE_CANARY_ENDPOINTS.has(pathname) && (request.method === "GET" || request.method === "HEAD")) {
    response.headers.set("Cache-Control", "no-store, max-age=0");
    response.headers.set("X-Content-Type-Options", "nosniff");
    return response;
  }

  // Malware Defense live verification is also an exact read-only path. The route generates only a
  // fixed harmless text canary server-side, signs it, and never exposes a public file-scanning mutation.
