...
const PUBLIC_BROWSER_RUNTIME_ASSETS = new Set(["/laneriq-sw.js"]);
...
export async function updateSession(request) {
  ...
  // Exact browser runtime assets required before authentication may pass through here. Keep this list
  // intentionally tiny: the Service Worker contains no customer data and must be retrievable before a
  // session exists, while /offline, /editor, /generated and every private API keep the normal session gate.
  if (PUBLIC_BROWSER_RUNTIME_ASSETS.has(pathname) && (request.method === "GET" || request.method === "HEAD")) {
  ...
  // Exact server-to-server Malware mutation. The route itself requires a Vercel-issued OIDC token
  // bound to the standalone Malware Defense Production project before it can touch defensive metadata.
  ...
  // Published Website enquiry submission is the only dynamic public browser mutation bypass here.
  ...
  // Cloud public observability is strictly read-only. GET/HEAD may expose sanitized configuration and
  // release status; executable OIDC/exact-SHA canary POSTs must pass the normal session gate and admin check.
  ...
  // Provider Router public observability is strictly read-only. GET/HEAD may expose sanitized aggregate
  // status, but executable canary POSTs must pass the normal session gate and then the route's admin check.
  ...
  // Malware Defense live verification remains owned by the Malware Defense window. The route generates
  // a fixed harmless text canary server-side; access-control hardening for this surface is tracked separately.
