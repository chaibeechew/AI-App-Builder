...
export const SERVICE_OBSERVABILITY_POLICY=Object.freeze({
  version:'obs1',
  requiredStatuses:['healthy','degraded','down','unknown'],
  remoteLiveRequiresDistinctProductionEvidence:true,
  noSilentFallbackAfterUncertainRemoteMutation:true,
});
...
export function buildServiceHealthSnapshot(statusByService={}){
  return Object.entries(registry.services).map(([id,service])=>{
    ...
    return {
      ...
      liveVerified:live,
      evidenceLevel:live?'LIVE_STANDALONE':mode==='remote'?'REMOTE_READY':'CODE_EMBEDDED',
      safeToMutateRemotely:live&&status==='healthy',
    };
  });
}
...
export function planFabricRecovery(statusByService={}){
  ...
  return {
    version:SERVICE_OBSERVABILITY_POLICY.version,
    overall:services.every(x=>x.status==='healthy')?'healthy':services.some(x=>x.status==='down')?'degraded':'attention',
    services,
    actions,
