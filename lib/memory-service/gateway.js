...
export async function executeMemoryService({operation,appId,userId,payload={},embedded}){
  ...
  const sig=signature(secret,{clientId,timestamp,nonce,body});
  let response;
  try{
    response=await fetch(`${url}${PATH}`,{method:"POST",headers:{"content-type":"application/json","x-laneriq-client-id":clientId,"x-laneriq-timestamp":timestamp,"x-laneriq-nonce":nonce,"x-laneriq-signature":sig},body,redirect:"error",cache:"no-store",signal:AbortSignal.timeout(10000)});
  }catch(error){
    const wrapped=new Error("memory_service_unreachable");wrapped.cause=error;throw wrapped;
  }
  const data=await response.json().catch(()=>({}));
  if(!response.ok){const error=new Error(data?.error||"memory_service_failed");error.status=response.status;throw error;}
  return data;
}
