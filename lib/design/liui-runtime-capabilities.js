export const LIUI_RUNTIME_CAPABILITY_VERSION = "2026.09-runtime-v3";
...
export const LIUI_RUNTIME_CAPABILITIES = Object.freeze({
  ...
  supportedRuntimeLocales: 10,
  safeContinuityResume: true,
  contextAdaptiveContinue: true,
  decisionLayerDeDuplication: true,
  semanticMotionTokens: true,
});

export const LIUI_MEMORY_MAX_AGE_MS = 7 * 24 * 60 * 60 * 1000;
export const LIUI_SAFE_NAV_TARGETS = Object.freeze({
  Home: "/",
  Projects: "/my-apps",
  Create: "/create",
  Templates: "/templates",
...
const contexts = [
  ...
  { pageId: 3, pattern: /^\/create\/?$/, surface: "creation", phase: "Build", name: "Build Progress", primaryAction: "Inspect truthful build progress", primaryNav: "Create", query: "flow=build-progress" },
...
function contextByPageId(pageId) {
  return LIUI_ROUTE_CONTEXTS.find(context => context.pageId === Number(pageId)) || null;
}
...
export function resolveLiuiRouteContext(pathname = "", search = "") {
  ...
  // Canonical Page 3 is a truthful state of the real /create route. Keep legacy
  // root flow aliases readable during transition, but never advertise a phantom route.
  if (safePath === "/create" && flow === "build-progress") return contextByPageId(3);
  if (safePath === "/" && flow === "create-project") return contextByPageId(2);
  if (safePath === "/" && flow === "build-progress") return contextByPageId(3);
  if (/^\/workflows\/[^/?]+\/?$/.test(safePath) && params.get("view") === "editor") {
...
export function resolveLiuiSafeResume(value, now = Date.now()) {
  const memory = sanitizeLiuiMemory(value || {});
  const currentTime = Number(now);
  const age = currentTime - memory.timestamp;
  const href = LIUI_SAFE_NAV_TARGETS[memory.primaryNav] || "";
  if (!memory.pageId || memory.pageId === 1 || !href || href === "/") return null;
  if (!Number.isFinite(currentTime) || !Number.isFinite(age) || age < 0 || age > LIUI_MEMORY_MAX_AGE_MS) return null;
  return Object.freeze({
    pageId: memory.pageId,
    phase: memory.phase,
    primaryNav: memory.primaryNav,
    href,
  });
}