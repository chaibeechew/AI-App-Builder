import { createIndustryMediaPlan, detectIndustryFromText, fillIndustryPalette, industryVisualInstruction, resolveIndustryVisualProfile } from "./industry-visual-system.js";
...
export const GENERATED_APP_VISUAL_RULES = Object.freeze([
  ...
  "Auto Theme must visibly adapt by industry when the customer has not chosen a brand palette; color, imagery, card treatment and visual mood cannot default to one universal look.",
  ...
  "Visual QA must reject inconsistent page styling, placeholder composition, inaccessible contrast, industry-irrelevant hero imagery, text-only first screens where media is appropriate, repeated major imagery and random template mixing."
]);
...
export const GENERATED_PAGE_STYLE_RULES = Object.freeze({
  ...
  visualComposition:{industryMediaRequiredWhenUseful:true,avoidTextOnlyHero:true,avoidRepeatedMajorImage:true,foregroundCopyConcise:true},
  ...
  forbidden:["unstyled blue links","browser-default buttons","browser-default form controls","naked tables without mobile treatment","mixed unrelated design systems","placeholder-only sections","industry-irrelevant hero imagery","text-only first screen when meaningful media is appropriate","repeated identical major image"]
});
...
function detectIndustry(specification={},app={}){return detectIndustryFromText(textOfSpec(specification,app));}
...
function industryDesignDefaults(profile){
  return {
    industryLabel:profile.label,
    industryMood:profile.mood,
    primarySubjects:[...profile.primarySubjects],
    serviceActions:[...profile.peopleActions],
    supportingVisuals:[...profile.supportingVisuals],
    cardStyle:profile.cardStyle,
    iconStyle:profile.iconStyle,
    imageStyle:`Industry-first imagery: ${profile.primarySubjects.slice(0,4).join(", ")}; include relevant human/service actions; keep major scenes distinct.`,
    backgroundDirection:`Use recognizable ${profile.label.toLowerCase()} context as the visual foundation; foreground copy stays concise and readable.`,
    heroDirection:`Image-led ${profile.label.toLowerCase()} first screen with a clearly visible subject, concise title and obvious primary action.`,
    wallpaperPreset:profile.wallpaperCycle?.[0]||"golden-valley"
  };
}
...
export function applyGeneratedExperienceStandard({specification={},app={}}={}){
  ...
  const profile=resolveIndustryVisualProfile(industry);
  ...
  const paletteFilled=fillIndustryPalette(existingDesign,industry);
  const adaptiveDefaults=industryDesignDefaults(profile);
  const propertyPalette=isProperty&&!customerHasCustomPalette(existingDesign)?PROPERTY_CRM_GOLDEN_REFERENCE.palette:{};
  ...
  const designSystem={
    ...
    visualComposition:"Industry-relevant media remains clearly visible; foreground text is concise; display type must not overpower the visual subject; major imagery should not repeat without purpose.",
  ...
  qualityPlan.beauty=mergeNamed([
    ...
    "Auto Theme uses an industry-specific color and visual profile instead of one universal palette."
  ...
  const industryMediaPlan=createIndustryMediaPlan({industry,pages,seed:app?.id||spec?.name||app?.name||"project"});
  const industryVisualProfile={
    id:industry,
    label:profile.label,
    mood:profile.mood,
    primarySubjects:[...profile.primarySubjects],
    peopleActions:[...profile.peopleActions],
    supportingVisuals:[...profile.supportingVisuals],
    recommendedPalette:{...profile.palette},
    avoid:[...profile.avoid]
  };
  return {
    industry,
    standardId:GENERATED_EXPERIENCE_STANDARD_ID,
    specification:{...spec,designSystem,pages,features,navigation,qualityPlan,distributionPlan,industryVisualProfile,industryMediaPlan}
  };
}
...
When meaningful media is appropriate, the first screen cannot be text-only. Show recognizable industry content and keep it visible. Put concise foreground copy over or beside the visual instead of using giant display type. Major visual assets must be distinct rather than repeating the same scene throughout the project.

INDUSTRY AUTO-THEME MAP
When the customer has not chosen a palette or Brand Kit, use the industry profile as the automatic starting point. The industry profiles below are visual directions, not rigid templates. Customer brand choices remain authoritative.
${industryVisualInstruction()}
