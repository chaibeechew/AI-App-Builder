export async function getAppBuilderAccess(supabase, userId) {
  ...
  const { data, error } = await supabase
    ...
    .select("standard_project_credits,pro_valid_from,pro_valid_until,game_access_plan,game_cooldown_level,game_cooldown_until,creator_opportunity_active,creator_opportunity_bonus_share_percent,creator_opportunity_approved_at,creator_opportunity_approved_by")
    .eq("user_id", userId)
    .maybeSingle();
