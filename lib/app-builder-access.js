export async function getAppBuilderAccess(supabase, userId) {
  ...
  const { data, error } = await supabase
    ...
    .select("standard_project_credits,pro_valid_from,pro_valid_until,game_access_plan,game_cooldown_level,game_cooldown_until")
    .eq("user_id", userId)
    .maybeSingle();
  ...
  const gameAccessPlan = data.game_access_plan === "full" ? "full" : "professional";
  return {
    ...
    professional: {
      ...
      gameCooldownActive: gameAccessPlan === "professional" && Number.isFinite(cooldownUntilMs) && cooldownUntilMs > now,
    },
  };
}
