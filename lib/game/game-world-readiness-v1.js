...
import {runWorldReasoningOrchestration,auditWorldReasoningOrchestration} from "./game-world-reasoning-orchestrator-v1.js";
...
export function currentGameWorldReadinessEvidence({prompt="Epic mobile open world RPG with castle, quests, treasure, dungeons and bosses",seed="game-world-readiness"}={}){
  ...
  // compileGameWorldProject validates the raw scene and then stores the normalized scene document.
  // Therefore readiness must consume project-level validation + normalized scene/evidence instead
  // of looking for a second `valid` field that intentionally does not exist on project.scene.
  const sceneValid=project.validation?.valid===true&&Number(project.scene?.entityCount)>0&&project.evidence?.sceneSafety==="validated-scriptless-scene-document";
  const runtimeReady=Boolean(project.runtime?.playable===true&&project.runtime?.systems?.length&&project.runtime?.platforms?.length&&project.evidence?.runtimeContract==="existing-runtime-bridge");
