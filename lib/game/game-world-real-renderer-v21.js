...
export function buildRenderSceneV21(input={}){
 const world=compileProductionWorldV20(input);
 const terrain=world.world.city?.v18?.v17?.v16?.v15?.terrain||world.world.terrain||{};
 const vertices=terrain.mesh?.vertices||terrain.vertices||[]; const indices=terrain.mesh?.indices||terrain.indices||[];
 const veg=world.world.city?.v18?.v17?.v16?.vegetation?.instances||world.world.vegetation?.instances||[];
 const buildings=world.world.city?.buildings||[];
 ...
 return {version:'v21',world,geometry:{terrainVertices:vertices,terrainIndices:indices,buildings:buildings.slice(0,256),vegetation:veg.slice(0,budget.veg)},materials:{terrain:{baseColor:[.21,.32,.18],roughness:.86,metallic:0},water:{baseColor:[.05,.24,.35],roughness:.2,metallic:.05,transmission:.45},building:{baseColor:[.38,.4,.44],roughness:.72,metallic:.08}},lighting:{sunDirection:[-.45,.82,.28],sunLux:68000,ambient:.34,sky:'procedural-gradient'},atmosphere:{fogDensity:.0018,heightFog:true,weatherBlend:true},budget,truth:{browserRendererImplemented:true,hardwareGpuMeasured:false,productionRendererVerified:false}};
}
export function buildFramePlanV21(scene,{camera=[0,90,180],time=0}={}){const draws=[];if(scene.geometry.terrainVertices.length)draws.push({kind:'terrain',count:scene.geometry.terrainIndices.length||scene.geometry.terrainVertices.length,material:'terrain'});if(scene.geometry.buildings.length)draws.push({kind:'buildings',count:Math.min(scene.geometry.buildings.length,scene.budget.draws),material:'building'});if(scene.geometry.vegetation.length)draws.push({kind:'vegetation',count:scene.geometry.vegetation.length,instanced:true,material:'vegetation'});return {camera,time,draws,drawCalls:draws.length,renderScale:clamp(scene.budget.scale,.5,1)};}
