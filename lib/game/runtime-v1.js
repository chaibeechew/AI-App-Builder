...
export const GAME_RUNTIME_V1=Object.freeze({
  ...
  systems:[
    ...
    "win-lose-state","reward-replay-loop","world-bounds","damage-cooldown","autosave","haptics","accessibility","performance-budget","lifecycle-recovery","deterministic-spawns"
  ],
...
export function compileGameRuntimeV1(specification={}){
  ...
  const objective=text(game.objective||game.goal,coreLoop.length?coreLoop.join(" → "):"Collect energy, avoid enemies and clear the final level.");
  return {
    ...
    controls:{touch:true,drag:true,dpad:true,keyboardPreview:true,gamepadReady:true,inputRecovery:true},
