...
const SECRET_PATTERN=/(api[_-]?key|token|password|secret|authorization)\s*[:=]\s*[^\s,;]+/ig;
export function analyzeCrashReport({message="",stack="",events=[]}={}){const sanitizedMessage=text(message).replace(SECRET_PATTERN,"$1=[REDACTED]").slice(0,500),sanitizedStack=text(stack).replace(SECRET_PATTERN,"$1=[REDACTED]").slice(0,4000),fingerprint=hash(`${sanitizedMessage}\n${sanitizedStack.split("\n").slice(0,5).join("\n")}`);const lower=`${sanitizedMessage} ${sanitizedStack}`.toLowerCase(),category=lower.includes("out of memory")||lower.includes("oom")?"memory":lower.includes("network")?"network":lower.includes("render")||lower.includes("gpu")?"rendering":"runtime";return{fingerprint,category,message:sanitizedMessage,stack:sanitizedStack,recentEvents:(events||[]).slice(-32),containsRawSecret:SECRET_PATTERN.test(`${sanitizedMessage} ${sanitizedStack}`)};}
