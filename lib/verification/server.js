...
async function confirmCompatibilityEmail(userId){
  const admin=createAdminClient();
  const {data,error}=await admin.auth.admin.updateUserById(userId,{email_confirm:true});
  const user=data?.user;
  if(error||!user?.id||user.id!==userId)throw new Error("Compatibility email confirmation could not be synchronized.");
  if(!user.confirmed_at&&!user.email_confirmed_at)throw new Error("Compatibility email confirmation is incomplete.");
...
async function mintCompatibilitySession(tokenHash,userId){
  ...
  if(!data.user.confirmed_at&&!data.user.email_confirmed_at)throw new Error("Compatibility email confirmation is incomplete.");
...
export async function verifyLaneriqEmailVerification({email,challengeId:rawChallengeId,code}){
  ...
  try{
    ...
    // LANERIQ owns the verification decision. Before the temporary compatibility
    // bridge is used by Builder persistence, mirror that verified decision into
    // the provider identity so confirmed_* cannot lag behind the LANERIQ session.
    await confirmCompatibilityEmail(prepared.userId);
