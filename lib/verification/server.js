...
function verificationSecret(){
  const secret=String(process.env.LANERIQ_VERIFICATION_SECRET||process.env.LANERIQ_COMMUNICATION_PRIVACY_SECRET||"");
...
export function laneriqVerificationStatus(){
  return {
    ...
    sessionAuthority:"laneriq_transition",
    compatibilityBridge:"supabase_session_only",
  };
}
...
export async function requestLaneriqEmailVerification({email,scope,requestId,referral}){
  ...
  try{
    ...
    if(delivery?.status!=="completed"){
      await admin.rpc("laneriq_mark_verification_delivery",{p_id:id,p_status:"delivery_failed"}).catch?.(()=>{});
    ...
    await admin.rpc("laneriq_mark_verification_delivery",{p_id:id,p_status:"delivered"});
    ...
    if(challengeCreated)try{await admin.rpc("laneriq_mark_verification_delivery",{p_id:id,p_status:"delivery_failed"});}catch{}
...
async function mintCompatibilitySession(email,referral){
  const {tokenHash,userId}=await ensureSupabaseCompatibilityUser(email,referral);
...
export async function verifyLaneriqEmailVerification({email,challengeId:rawChallengeId,code}){
  ...
  try{
    const session=await mintCompatibilitySession(email,result?.referral_code||null);
    return {success:true,service:"LANERIQ Verification",otpAuthority:"laneriq",sessionBridge:"compatibility",userId:session.userId};
  }catch{
    return {success:false,code:"SESSION_BRIDGE_FAILED",status:503};
  }
}
