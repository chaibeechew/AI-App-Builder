...
function normalizePrincipal(user, authoritativeVerified = false) {
  return Object.freeze({
    principalId: user.id,
    verified: Boolean(authoritativeVerified || compatibilityVerified(user)),
  });
}

async function readLaneriqAuthority() {
  try {
    const cookieStore = await cookies();
    const token = String(cookieStore.get(LANERIQ_SESSION_COOKIE)?.value || "");
    const mode = cookieStore.get(LANERIQ_SESSION_MODE_COOKIE)?.value;
    const primaryMode = isLaneriqPrimarySessionMode(mode);
    if (!token) return primaryMode ? { present: true, primaryMode, error: "SESSION_REQUIRED" } : { present: false, primaryMode: false };
    try {
      const session = await validateLaneriqSessionToken(token);
      if (!session?.userId) return primaryMode ? { present: true, primaryMode, error: "SESSION_REQUIRED" } : { present: false, primaryMode: false };
      return { present: true, primaryMode, userId: String(session.userId) };
    } catch {
      return { present: true, primaryMode, error: "SESSION_NOT_READY" };
    }
  } catch {
    return { present: false, primaryMode: false };
  }
}
...
async function resolvePrincipal(client, { requireVerified = false } = {}) {
  try {
    ...
    const authority = await readLaneriqAuthority();
    if (authority?.error && authority.primaryMode) return fail(authority.error);
    if (authority?.userId && authority.userId !== data.user.id) return fail("SESSION_IDENTITY_MISMATCH");

    // LANERIQ Email/WhatsApp verification is the authoritative verification event.
    // The temporary provider-compatible identity may still have stale confirmed_* metadata,
    // so a valid same-user LANERIQ session must satisfy the verification gate without
    // weakening ownership or allowing identities to cross.
    const authoritativeVerified = Boolean(authority?.userId && authority.userId === data.user.id);
    const principal = normalizePrincipal(data.user, authoritativeVerified);
