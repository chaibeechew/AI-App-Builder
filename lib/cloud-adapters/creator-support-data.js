...
export function createCreatorSupportDataAdapter(){
  return Object.freeze({
    ...
    async setCreatorSupportApprovalMode({mode}={}){
      ...
      const {data,error}=await auth.client.rpc("admin_set_creator_support_mode",{p_mode:mode});if(error)return fail("CREATOR_SUPPORT_MODE_FAILED",error.message);return ok(data);
    },
    async reviewCreatorSupportRequest({requestId,decision,reason=""}={}){
      ...
      const {data,error}=await auth.client.rpc("admin_review_creator_support",{p_request_id:requestId,p_decision:decision,p_reason:String(reason||"").slice(0,500)||null});if(error)return fail("CREATOR_SUPPORT_REVIEW_FAILED",error.message);return ok(data);
    },
