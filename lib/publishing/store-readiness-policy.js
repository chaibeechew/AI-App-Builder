...
export function detectDataDisclosureNeeds(specification={}){
  const raw=JSON.stringify(specification||{}).toLowerCase();
  const definitions=[
    {key:"contact_info",label:"Contact Info",patterns:[/email/,/phone/,/contact info/,/address/,/profile/]},
    {key:"identifiers",label:"Identifiers",patterns:[/user id/,/account id/,/device id/,/identifier/,/login/,/authentication/]},
    {key:"usage_data",label:"Usage Data",patterns:[/analytics/,/usage data/,/event tracking/,/gameplay analytics/,/telemetry/]},
    {key:"diagnostics",label:"Diagnostics",patterns:[/diagnostic/,/crash/,/error log/,/performance monitoring/]},
    {key:"purchases",label:"Purchases",patterns:[/purchase/,/payment/,/checkout/,/in-app purchase/,/iap/,/subscription/,/battle pass/]},
    {key:"location",label:"Location",patterns:[/geolocation/,/\bgps\b/,/current location/,/nearby/]},
    {key:"user_content",label:"User Content",patterns:[/user content/,/photo upload/,/image upload/,/video upload/,/chat/,/message/,/post/]},
  ];
  const categories=definitions.filter(item=>item.patterns.some(pattern=>pattern.test(raw))).map(item=>({key:item.key,label:item.label}));
  const tracking=/(cross[- ]app tracking|tracking permission|advertising id|idfa|att\b|targeted ads?|personalized ads?|ad network)/.test(raw);
  const ads=/(rewarded ad|interstitial|advertising|ad network|contains ads)/.test(raw);
  return {categories,tracking,ads,requiresReview:true};
}

export function buildStoreReadiness({specification={},listing=null,assets=[],inferredAnswers={},customerDeclarations={}}={}){
  const apple=listing?.apple||{};
  const google=listing?.google_play||{};
  const declarations=safeObject(customerDeclarations);
  const permissionPurposes=safeObject(declarations.permissionPurposes);
  const assetState=classifyStoreAssets(assets);
  const permissionRequirements=detectPermissionNeeds(specification);
  const dataDisclosure=detectDataDisclosureNeeds(specification);
  const checks=[];
  ...
  if(declarations.ageRatingAcknowledged===true)push("age_rating","Age / content rating","external_required","stores","Customer acknowledged that the official age/content-rating questionnaire still must be completed truthfully in Apple/Google consoles.");
  else push("age_rating","Age / content rating","customer_required","customer","Customer must acknowledge that age/content-rating answers cannot be invented by AI and must be completed truthfully in the official store consoles.");

  const termsChoice=str(declarations.termsChoice);
  const termsUrl=str(declarations.termsUrl);
  const termsReady=termsChoice==="platform_default"||(termsChoice==="custom"&&present(termsUrl));
  push("terms","Terms / EULA decision",termsReady?"ready":"customer_required","customer",termsReady?(termsChoice==="custom"?"Customer selected a custom Terms/EULA URL.":"Customer selected the platform default EULA where permitted."):"Confirm whether the platform default EULA is sufficient or provide a real custom Terms/EULA URL.");

  for(const permission of permissionRequirements){
    const purpose=str(permissionPurposes[permission.key]);
    push(`permission_${permission.key}`,`${permission.label} permission purpose`,present(purpose)?"ready":"customer_required","customer",present(purpose)?`Customer provided the purpose: ${purpose.slice(0,240)}`:permission.reason);
  }

  const detectedData=dataDisclosure.categories.map(item=>item.label).join(", ");
  const applePrivacyReason=dataDisclosure.tracking
    ?`Potential tracking-related behavior was detected. App Store Connect must truthfully classify Data Used to Track You and any Data Linked/Not Linked categories. Detected categories to review: ${detectedData||"none inferred"}. AI must not auto-select a privacy answer.`
    :`App Store Connect privacy answers must be verified against the real runtime. Review whether the correct state is Data Not Collected or one/more collected categories${detectedData?`: ${detectedData}`:""}. AI must not auto-select Data Not Collected.`;
  push("apple_app_privacy","Apple App Privacy disclosure","external_required","apple",applePrivacyReason);
  push("google_data_safety","Google Play Data Safety","external_required","google",`Google Play Data Safety must match the real runtime, SDKs and integrations. Review collection, sharing, security and deletion behavior${detectedData?` for inferred categories: ${detectedData}`:""}.`);
  push("accessibility_declaration","Store accessibility declaration","external_required","stores","SoolenAI can prepare accessibility features and test guidance, but any App Store/Google Play accessibility declaration must reflect features actually implemented and tested on the release build.");
  ...
  return {checks,blocking,preparable,customerRequired,externalRequired,permissionRequirements,dataDisclosure,assets:assetState,customerDeclarations:declarations,readinessScore,readyForCustomerReview:customerRequired.length===0&&preparable.length===0,readyForOfficialSubmission:false};
}
