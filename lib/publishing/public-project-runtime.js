...
export function isPublishedProject(app) {
  return Boolean(app && app.publish_status === "published" && PUBLIC_VISIBILITY.has(app.visibility));
}

export async function loadVisibleProject({ id, userId = null }) {
  ...
  if (appError || !app?.current_version_id) return null;
  ...
  return { admin, app, version, isOwner, isPublished };
}
