...
export const LIUI_RELEASE_SCORE_REQUIRED = 95;
...
export function evaluateReleaseReadiness(report) {
  ...
  const liuiPresent=Boolean(report?.liui);
  const liuiScore=liuiPresent?Number(report?.liui?.score||0):null;
  const liuiReady=!liuiPresent||liuiScore>=LIUI_RELEASE_SCORE_REQUIRED;
  const releaseReady = missing.length === 0 && belowTarget.length === 0 && overall >= RELEASE_SCORE_REQUIRED && liuiReady;
  return {
    releaseReady,
    ...
    missing,
    belowTarget,
    liui:{present:liuiPresent,score:liuiScore,requiredScore:LIUI_RELEASE_SCORE_REQUIRED,ready:liuiReady,releaseBand:report?.liui?.releaseBand||null},
  };
}
...
export const RELEASE_POLICY_NOTE =
  "A 100-point deterministic project score plus the LIUI 95-point design-spec gate are strict internal release gates, not a guarantee of zero bugs, absolute security, privacy or legal compliance, provider availability or store approval. Production promotion also requires environment-specific evidence.";