...
export function resolveTemplateGenerationGuidance(idea) {
  ...
  // A title contains industry + archetype, so style is also required as an independent
  // canonical signal. This prevents an incomplete or user-forged title mention from
  // silently becoming trusted structural guidance.
  const template = getTemplateCatalog().find(candidate =>
    ...
    includesCanonical(normalized, candidate.style)
  );
