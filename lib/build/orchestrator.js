...
export function buildAutonomousPlan(input={}){
  ...
  const workflows=[];
  if(crm||messaging)workflows.push({name:"Lead follow-up",triggerType:"form_submitted",actions:[{type:"save_crm",label:"Save customer to CRM"},{type:"send_email",label:"Send confirmation email"},{type:"send_whatsapp",label:"Send WhatsApp acknowledgement"},{type:"notify_team",label:"Notify team"}]});
  ...
  return {
    ...
    principles:["Customer-facing infrastructure stays hidden","Private assets remain project-scoped","Use server-side processing for heavy video work","Mobile games target both iOS and Android with a web preview path","Quality gate is specification-level and does not replace runtime/device testing"],
