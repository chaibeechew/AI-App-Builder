...
export function buildAutonomousPlan(input={}){
  ...
  return {
    ...
    principles:["Customer-facing infrastructure stays hidden","Generated projects call LANERIQ backend contracts instead of provider APIs","Natural-language verification requests are auto-built without customer provider setup","Private assets remain project-scoped","Use server-side processing for heavy video work","Mobile games target both iOS and Android with a web preview path","Quality gate is specification-level and does not replace runtime/device testing"],
...
export function orchestrationBrief(plan){
  ...
  return [
    ...
    plan.verification?`Managed verification: ${plan.verification.channels.join(" + ")} through LANERIQ Verification. Auto-build the full request-code, verify-code, session, resend-limit, error and abuse-protection flow. Never ask the creator to configure provider APIs and never fall back to paid SMS.`:"",
