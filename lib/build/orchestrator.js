...
export function buildAutonomousPlan(input={}){
  ...
  const verification=has(text,["verification code","verify code","otp","one-time code","one time code","login code","sign in code","signup code","phone verification","whatsapp verification","email verification","验证码","驗證碼","手机验证","手機驗證","登录验证","登入驗證","注册验证","註冊驗證"]);
  const wantsWhatsAppVerification=verification&&has(text,["whatsapp","手机","手機","phone","mobile","number"]);
  const wantsEmailVerification=verification&&has(text,["email","e-mail","邮箱","郵箱","电邮","電郵"]);
  ...
  const modules={app:true,website:true,game:isGame,media:hasMedia||isGame,database:crm||booking||commerce||multiplayer||has(text,["database","data","record","inventory","listing","property","save game","leaderboard"]),workflows:crm||booking||commerce||messaging,integrations:verification||messaging||booking||commerce||multiplayer,payments:commerce,video:wantsVideo,analytics:true,quality:true,publish:true};
  const workflows=[];
  ...
  const verificationChannels=verification
    ? wantsWhatsAppVerification&&!wantsEmailVerification?["whatsapp"]
      : wantsEmailVerification&&!wantsWhatsAppVerification?["email"]
      : ["email","whatsapp"]
    : [];
  const selected=MODULE_ORDER.filter(name=>modules[name]);
  return {
    version:"autonomous-build-orchestrator-v3",
    ...
    verification:verification?{
      ...
      defaultChannel:verificationChannels.includes("email")?"email":verificationChannels[0],
      passwordless:true,
      sessionReuse:true,
      resendProtection:true,
      abuseProtection:true,
      paidSmsFallback:false,
      pricing:{launchYearMonths:12,customerPlatformFee:0,currency:"MYR",autoChargeCustomer:false,fairUse:true},
    }:null,
    ...
    principles:["Customer-facing infrastructure stays hidden","Generated projects call LANERIQ backend contracts instead of provider APIs","Natural-language verification requests are auto-built without customer provider setup","LANERIQ Verification and Communications charge no platform service fee during the first 12 launch months and never auto-charge users when fair-use limits are reached","Private assets remain project-scoped","Use server-side processing for heavy video work","Mobile games target both iOS and Android with a web preview path","Quality gate is specification-level and does not replace runtime/device testing"],
...
export function orchestrationBrief(plan){
  ...
  return [
    ...
    plan.verification?`Managed verification: ${plan.verification.channels.join(" + ")} through LANERIQ Verification. Auto-build the full request-code, verify-code, session, resend-limit, error and abuse-protection flow. Never ask the creator to configure provider APIs, never fall back to paid SMS, and apply the LANERIQ Launch Year Free policy with no automatic customer charges.`:"",
