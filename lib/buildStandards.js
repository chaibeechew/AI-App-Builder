export const RELEASE_READINESS_SCORE = 99;
...
export function assessBuildQuality(specification) {
  ...
  return { overall, passed: dimensions.every((item) => item.passed) && overall >= RELEASE_READINESS_SCORE, dimensions, methodology: "deterministic-spec-quality-gate-v2-99" };
}

export const GENERATION_QUALITY_RULES = `
Every generated App + Website, including the free promotional first project and Standard tier, must aim for a 99-point release-readiness standard across stability, security, privacy, comfort, beauty and naturalness. Professional Mode adds deeper control; it must not be a paywall for basic quality.
...
Do not publish merely because the specification score is high. Runtime, dependency, infrastructure, payment, integration and real-device checks must still be completed where applicable. A 99 score is an internal release target, not a guarantee of zero defects, absolute security, legal compliance or market leadership.
`;
