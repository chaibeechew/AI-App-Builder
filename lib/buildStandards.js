export const BUILD_STANDARDS = [
  ...
  },
  {
    ...
    target: 92,
    checks: [
      "Distinctive visual hierarchy rather than generic template composition",
      "Premium backgrounds or imagery appropriate to the product and brand",
      "Balanced typography, spacing, imagery and polished states",
      "Original responsive composition with a memorable hero experience",
    ],
  },
  {
    id: "naturalness",
...
function textOf(specification) {
  ...
  return [
    ...
    ...pages.flatMap((page) => [page?.name, page?.purpose, page?.description, page?.layout, page?.visualTreatment]),
...
Every generated App + Website, including the existing USD 10 user tier, must receive the same premium product-quality floor. Professional Mode adds deeper control; it must not be a paywall for basic design quality.
...
5. Beauty: create an original premium visual identity, distinctive layout, memorable hero treatment, refined typography and spacing, high-quality background/imagery direction, polished states and responsive composition. Avoid generic dashboard/template appearance.
6. Naturalness: human language, real-world workflows, natural rhythm and context-aware interactions.
Use industry context to make each product visually distinctive. A real-estate app, restaurant app, luxury service, retail product, education product and creator product should not receive the same composition.
Backgrounds must support readability and product identity; use layered gradients, customer media, original imagery direction or cinematic environmental treatment when appropriate rather than decorative clutter.
...
Do not claim guaranteed security, privacy, legal compliance or market leadership; create explicit safeguards and a quality target that can later be implemented and tested.
`;
