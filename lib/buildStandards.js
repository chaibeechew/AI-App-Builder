...
export const BUILD_STANDARDS = [
  ...
  { id: "beauty", name: "Beauty", target: RELEASE_READINESS_SCORE, checks: ["Distinctive visual hierarchy rather than generic template composition","Premium backgrounds or imagery appropriate to the product and brand","Balanced typography, spacing, imagery and polished app-like states","Original responsive composition with a memorable hero experience","Customer-selected colors coordinate the entire visual system rather than isolated controls","A deliberate wallpaper, card and image direction is recorded rather than guessed at render time","Every page has a purpose-specific scene and layout family","Loading, empty, error and success states preserve the same visual quality","Industry profile changes composition, imagery, typography and information density"] },
...
const BEAUTY_EVIDENCE_FIELDS=["backgroundDirection","heroDirection","layoutSignature","fontDirection","iconStyle","themeMode","colorPreference","paletteRationale","cardStyle","imageStyle","wallpaperPreset","industryProfile","experienceVersion"];
...
function textOf(specification) {
  ...
  return [
    ...
    specification?.designSystem?.industryProfile,
    specification?.designSystem?.experienceVersion,
    JSON.stringify(specification?.experienceSystem||{}),
    JSON.stringify(specification?.qualityPlan||{}),
    JSON.stringify(specification?.data||{}),
    ...pages.flatMap((page) => [page?.name,page?.purpose,page?.description,page?.layout,page?.visualTreatment,page?.backgroundTreatment,JSON.stringify(page?.experience||{})]),
...
function scoreDimension(id, specification) {
  ...
  const evidenceComplete=explicitPlan.length>=3 && (id!=="beauty" || designFieldCount===BEAUTY_EVIDENCE_FIELDS.length);
...
export function assessBuildQuality(specification) {
  ...
  return { overall, passed: dimensions.every((item) => item.passed) && overall >= RELEASE_READINESS_SCORE, dimensions, methodology: "deterministic-spec-quality-gate-v5-100-visual-evidence" };
}
...
5. Beauty: original premium visual identity, distinctive layout, memorable hero treatment, refined typography and spacing, high-quality background/imagery direction, polished app-like states and responsive composition. Avoid generic template appearance. Customer color choices must coordinate backgrounds, surfaces, buttons, cards, icons, typography, borders, gradients and imagery treatment as one system. Every page needs a purpose-specific scene, layout family, CTA, loading/empty/error/success treatment and iPhone contract. Industry changes composition, imagery, typography and information density rather than merely recoloring a universal template. A 100 Beauty score also requires explicit theme, palette rationale, card style, image style, wallpaper, industry profile and experience-system evidence.
