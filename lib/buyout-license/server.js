...
export async function issueBuyoutLicenseAsAdmin({appId,tier,paymentReference,resendEmail=false}={}){
  ...
  const {data:issued,error:issueError}=await auth.client.rpc("admin_issue_buyout_license",{p_app_id:appId,p_tier:normalizedTier,p_payment_reference:paymentRef});
