...
export function planSoolenVideoJob(input={}){
  ...
  const capabilities=input.deviceCapabilities||{};
  const chunkPlan=createSoolenChunkPlan({
    durationSeconds:Number(input.duration),
    capabilities,
    preferredChunkSeconds:Number(input.preferredChunkSeconds)||SOOLEN_DEFAULT_CHUNK_SECONDS
  });
  const continuityManifest=createContinuityManifest({
    visualStyle:input.style||null,
    referenceAssets:[input.referenceImage?"referenceImage":null,input.sourceVideo?"sourceVideo":null].filter(Boolean),
    voices:[input.voice||input.voiceSource].filter(Boolean)
  });
  return {
    engine:"soolen-ai",
    architecture:"device-first",
    ...
    chunkPlan,
    continuityManifest,
    finishing:{merge:true,music:Boolean(input.music),captions:Boolean(input.captions),container:"mp4"}
  };
}
...
export async function generateSoolenVideo(input){
  ...
  // Device-first is the primary architecture. When the browser/device runtime is
  // available it should execute plan.chunkPlan locally. This server function is
  // retained for company-pool or optional Soolen-controlled runtime execution.
  ...
  if(!runtimeUrl){
    throw new SoolenVideoEngineError(
      "This optional Soolen compute target has not been connected yet. You can use Device Compute instead.",
  ...
  try{
    ...
    console.error("Soolen runtime connection error:",error);
    throw new SoolenVideoEngineError("Soolen AI cannot reach the selected compute runtime right now.","SOOLEN_VIDEO_RUNTIME_UNREACHABLE",503);
  }
  ...
  return {
    ...
    executionTarget:input.executionTarget||"company-pool",
    plan,
    task:input.task,
    mode:input.mode,
    voiceSource:input.voiceSource
