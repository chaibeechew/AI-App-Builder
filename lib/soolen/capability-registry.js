...
function providerState(env = process.env) {
  ...
  const configuredTextProviders = [
    ...
    ["cloudflare", enabled("CLOUDFLARE_AI_ACCOUNT_ID", env) && enabled("CLOUDFLARE_AI_API_TOKEN", env)],
    ["huggingface", enabled("HF_TOKEN", env) && enabled("HF_MODEL", env)],
  ...
  return {
    ...
    premiumText: selectedPaid.some((provider) => provider !== "soolen-local"),
    premiumImage: ["zero","free"].includes(costMode) ? false : Boolean(env.SOOLEN_IMAGE_PROVIDER_URL || (env.AI_GATEWAY_API_KEY && env.SOOLEN_IMAGE_MODEL)),
    transcription: ["zero","free"].includes(costMode) ? Boolean(env.SOOLEN_STT_URL) : Boolean(env.OPENAI_API_KEY || env.SOOLEN_STT_URL),
    premiumVoice: ["zero","free"].includes(costMode) ? openVoiceReady : Boolean(openVoiceReady || (env.ELEVENLABS_API_KEY && env.SOOLENAI_VOICE_ID)),
    premiumVideo: ["zero","free"].includes(costMode) ? false : Boolean(env.SOOLEN_VIDEO_RUNTIME_URL || env.SOOLEN_VIDEO_EXECUTOR_URL || env.SOOLEN_VIDEO_PROVIDER_URL),
    web: ["zero","free"].includes(costMode) ? false : Boolean(env.SOOLEN_WEB_SEARCH_URL || env.TAVILY_API_KEY || env.PERPLEXITY_API_KEY),
    documents: Boolean(env.SOOLEN_DOCUMENT_WORKER_URL),
    automations: ["zero","free"].includes(costMode) ? false : Boolean(env.SOOLEN_AUTOMATION_WORKER_URL),
    connectors: Boolean(env.SOOLEN_CONNECTOR_BROKER_URL),
  };
}

// Compatibility helper for older callers. Current access is not a monthly subscription:
// Professional is one payment for 365 days and Standard keeps the premium quality floor.
export function planTier(planCode, accessStatus) {
  if (!["active","trialing"].includes(String(accessStatus || "").toLowerCase())) return "free";
...
export function resolveSoolenCapabilities({ tier = "free", env = process.env } = {}) {
  ...
  return {
    ...
    policy: {
      ...
      professionalFeaturesRequireActive365DayAccess: true,
      standardQualityFloorMatchesProfessional: true,
      failClosed: true,
      ...zeroCostPolicy(env),
    },
  };
