...
export const SOOLEN_CAPABILITIES = Object.freeze([
  ...
  { id:"premium-image-studio", category:"Images", name:"Premium image generation and editing", description:"Use an authorized multimodal provider for higher-fidelity generation and edits.", minimumTier:"pro", readiness:"premiumImage" },
...
function providerState(env = process.env) {
  ...
  const freeDefault = ["gemini","groq","cerebras","ollama"];
  const paidDefault = ["gateway","openai","xai","deepseek","mistral","together","openrouter","gemini","groq","cerebras","ollama"];
  ...
  return {
    ...
    freeTextProviders: split(env.SOOLEN_FREE_PROVIDERS, freeDefault).filter((provider) => configuredTextProviders.includes(provider)),
    paidTextProviders: split(env.SOOLEN_PAID_PROVIDERS, paidDefault).filter((provider) => configuredTextProviders.includes(provider)),
    premiumText: configuredTextProviders.some((provider) => ["gateway","openai","xai","deepseek","mistral","together","openrouter"].includes(provider)),
    premiumImage: Boolean(env.SOOLEN_IMAGE_PROVIDER_URL || (env.AI_GATEWAY_API_KEY && env.SOOLEN_IMAGE_MODEL)),
    transcription: Boolean(env.OPENAI_API_KEY || env.SOOLEN_STT_URL),
    premiumVoice: Boolean((env.SOOLENAI_TTS_URL && env.SOOLENAI_VOICE_SAMPLE_URL) || (env.ELEVENLABS_API_KEY && env.SOOLENAI_VOICE_ID)),
    premiumVideo: Boolean(env.SOOLEN_VIDEO_RUNTIME_URL || env.SOOLEN_VIDEO_EXECUTOR_URL || env.SOOLEN_VIDEO_PROVIDER_URL),
    web: Boolean(env.SOOLEN_WEB_SEARCH_URL || env.TAVILY_API_KEY || env.PERPLEXITY_API_KEY),
    documents: Boolean(env.SOOLEN_DOCUMENT_WORKER_URL),
    automations: Boolean(env.SOOLEN_AUTOMATION_WORKER_URL),
...
export function resolveSoolenCapabilities({ tier = "free", env = process.env } = {}) {
  ...
  const readiness = {
    local: true,
    browser: true,
    text: providers.configuredTextProviders.length > 0,
  ...
  return {
    ...
    capabilities,
    providers: {
      text: normalizedTier === "free" ? providers.freeTextProviders : providers.paidTextProviders,
