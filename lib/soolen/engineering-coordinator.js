...
export async function runEngineeringPhase(input = {}, handlers = {}) {
  ...
  const merged={
    ...
    sandbox:typeof handlers.sandbox==="function"?handlers.sandbox:async proposal=>verifyInRealSandbox({pages:specification?.pages||[],sourceFiles:proposal.files},{appId:input.appId}),
  ...
  return {...result,enabled:true,materialized:Boolean(deterministic),sourceType:deterministic?.sourceType||"ai-created",requiresSandboxBeforeAcceptance:true,verification:{security:"deterministic-source-scan",privacy:"deterministic-source-scan",sandbox:"isolated-runtime-required"}};
}
