...
assert.match(hostStatus,/PRODUCTION_CLOUD_OIDC_REQUIRED/);assert.match(hostStatus,/PRODUCTION_LIVE_OIDC_EXACT_SHA/);assert.match(hostStatus,/exactReleaseIdentity/);assert.match(hostStatus,/localReleaseSha/);assert.match(hostStatus,/remoteReleaseSha/);assert.match(hostStatus,/secretlessPeerAuthentication/);assert.match(hostStatus,/expectedAuthenticationMode = oidc\.token \? "VERCEL_OIDC" : "HMAC_SHA256"/);
assert.match(status,/peerAuthenticationPreferred: "VERCEL_OIDC"/);assert.match(status,/productionOidcRequired: production/);assert.match(status,/peerAuthenticationFallback: production \? null : "HMAC_SHA256"/);assert.match(status,/exactReleaseIdentityRequiredForLive: true/);assert.match(status,/live: false/);
...
assert.equal(manifest.peerAuthentication.preferred,'VERCEL_OIDC');assert.equal(manifest.peerAuthentication.fallback,'HMAC_SHA256_NON_PRODUCTION_ONLY');assert.equal(manifest.peerAuthentication.productionRequiresOidc,true);assert.equal(manifest.peerAuthentication.sharedPeerSecretRequired,false);
assert.equal(manifest.security.oidcIdentityVerification,true);assert.equal(manifest.security.productionOidcFailClosed,true);assert.equal(manifest.security.tripleScopeRequired,true);assert.equal(manifest.security.arbitraryQueryAllowed,false);assert.equal(manifest.security.rawProviderCredentialsForbidden,true);assert.equal(manifest.evidence.exactReleaseIdentityRequiredForLive,true);assert.equal(manifest.evidence.productionRuntimeEvidenceRequired,true);assert.equal(manifest.evidence.standaloneLive,false);
...
console.log('✓ Production Cloud routing fails closed without Vercel OIDC; HMAC fallback is non-Production only');
console.log('✓ OIDC peer verification pins the LANERIQ main Production project identity and rejects Preview identity');
console.log('✓ Production LIVE evidence requires verified Vercel OIDC plus exact main/Cloud Git SHA identity');
