...
assert.match(security,/timingSafeEqual/);assert.match(operate,/verifyLaneriqMainProductionPeer/);assert.match(operate,/requestAuthenticationMode/);assert.match(operate,/CLOUD_STORAGE_ADAPTER_NOT_READY/);assert.match(operate,/LANERIQ_CLOUD_STORAGE_ADAPTER_SECRET\|\|process\.env\.LANERIQ_CLOUD_SERVICE_SECRET/);assert.match(operate,/"authorization":`Bearer \$\{adapterSecret\}`/);
...
assert.match(hostStatus,/REQUEST_CONTEXT/);assert.match(hostStatus,/secretlessPeerAuthentication/);assert.match(hostStatus,/expectedAuthenticationMode = oidc\.token \? "VERCEL_OIDC" : "HMAC_SHA256"/);
assert.match(status,/peerAuthenticationPreferred: "VERCEL_OIDC"/);assert.match(status,/sharedPeerSecretRequired: false/);assert.match(status,/live:false/);
...
assert.equal(manifest.peerAuthentication.preferred,'VERCEL_OIDC');assert.equal(manifest.peerAuthentication.fallback,'HMAC_SHA256');assert.equal(manifest.peerAuthentication.sharedPeerSecretRequired,false);
assert.equal(manifest.security.oidcIdentityVerification,true);assert.equal(manifest.security.tripleScopeRequired,true);assert.equal(manifest.security.arbitraryQueryAllowed,false);assert.equal(manifest.security.rawProviderCredentialsForbidden,true);assert.equal(manifest.evidence.standaloneLive,false);
...
try{
  ...
  const payload=Buffer.from(JSON.stringify({
    ...
    sub:'owner:bryanbtxz-7929s-projects:project:laneriq-ai:environment:production',
    owner:'bryanbtxz-7929s-projects',owner_id:'team_r2xREQWKQzVw1bdswbpW5j8Y',
    project:'laneriq-ai',project_id:'prj_Q6mR7lmYGGKCW0ARu2Fgm9Pyzfcl',environment:'production',iat:now,nbf:now,exp:now+600,
  })).toString('base64url');
  const input=`${header}.${payload}`;
  const signature=crypto.sign('RSA-SHA256',Buffer.from(input),privateKey).toString('base64url');
  const verified=await verifyLaneriqMainProductionPeer({headers:{authorization:`Bearer ${input}.${signature}`}});
...
console.log('✓ Remote Cloud mode prefers verified Vercel Production OIDC and keeps HMAC only as a bounded fallback');
console.log('✓ OIDC peer verification pins the LANERIQ main Production project identity before Cloud operations run');
