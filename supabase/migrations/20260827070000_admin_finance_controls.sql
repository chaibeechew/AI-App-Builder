...
begin
  ...
  if new_status = 'rejected' then
    update public.cash_accounts
      set available_amount = available_amount + w.amount,
          reserved_amount = greatest(0, reserved_amount - w.amount),
          updated_at = now()
      where user_id = w.user_id;
    insert into public.cash_ledger(user_id, entry_type, amount, balance_after, reference_id, description)
      select w.user_id, 'withdrawal_release', w.amount, available_amount, w.id, 'Withdrawal rejected; reserved funds released'
      from public.cash_accounts where user_id = w.user_id;
  end if;
