-- Replace the two-argument SECURITY DEFINER helper with an auth-bound helper.
-- Callers may no longer supply an arbitrary user id to probe another customer's buyout state.
...
-- Folder policies: hidden source folders are visible/editable only to their authenticated owner with an active buyout.
...
using (
  ...
  and (is_hidden = false or public.has_active_buyout(app_id))
);
...
-- Source-file policies use explicit correlation and the auth-bound buyout helper.
...
-- Remove the legacy helper only after all policies have been moved to the safe signature.
