...
-- Refund amount is derived from the subscription plan, never trusted from the client.
alter table public.refund_requests
  add column if not exists amount numeric(18,2) check (amount is null or amount >= 0);
...
begin
  ...
  insert into public.refund_requests(user_id, subscription_id, amount, reason, eligible_until)
  values (uid, p_subscription_id, refund_amount, left(coalesce(p_reason, ''), 1000), eligible_until)
...
-- Keep payout account secrets/reference data server-controlled; users may only view their own records.
