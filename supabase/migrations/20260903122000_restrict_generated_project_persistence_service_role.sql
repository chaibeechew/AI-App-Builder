-- Keep atomic generated-project persistence behind LANERIQ's authenticated API control plane.
-- The API verifies the user, entitlement/credits, AI output and self-heal before invoking this
-- privileged RPC through the server-only Supabase service-role client.
...
revoke all on function public.server_persist_generated_project(uuid,text,text,text,text,jsonb,text) from public, anon, authenticated;
grant execute on function public.server_persist_generated_project(uuid,text,text,text,text,jsonb,text) to service_role;
