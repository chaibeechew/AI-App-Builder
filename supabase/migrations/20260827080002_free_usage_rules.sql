...
create policy "users read own usage counters" on public.user_usage_counters for select using (auth.uid() = user_id);
...
begin
  ...
  update public.user_usage_counters set free_create_used = true, updated_at = now() where user_id = auth.uid() and free_create_used = false returning true into ok;
...
begin
  if auth.uid() is null then raise exception 'Authentication required'; end if;
  insert into public.user_usage_counters(user_id) values (auth.uid()) on conflict (user_id) do nothing;
  update public.user_usage_counters set demo_modify_used = true, updated_at = now() where user_id = auth.uid() and demo_modify_used = false returning true into ok;
  return coalesce(ok, false);
end; $$;
...
begin
  if auth.uid() is null then raise exception 'Authentication required'; end if;
  insert into public.user_usage_counters(user_id) values (auth.uid()) on conflict (user_id) do nothing;
  update public.user_usage_counters set paid_modify_count = paid_modify_count + 1, updated_at = now() where user_id = auth.uid() returning paid_modify_count into n;
  return n;
end; $$;
