...
declare uid uuid := auth.uid(); free_used boolean; demo_ok boolean; referral_count integer;
begin
  ...
  if p_operation = 'modify' then
    update public.user_usage_counters set demo_modify_used = true, updated_at = now() where user_id = uid and demo_modify_used = false returning true into demo_ok;
    if coalesce(demo_ok,false) then return jsonb_build_object('allowed',true,'source','demo_free_modify'); end if;
    select count(*)::integer into referral_count from public.referral_rewards rr where rr.referrer_user_id = uid and rr.reward_type = 'first_app_created' and rr.status in ('pending','approved','paid');
    if referral_count < 7 and exists (select 1 from public.referral_rewards rr where rr.referrer_user_id = uid and rr.reward_type = 'first_app_created' and rr.status = 'pending' and rr.metadata->>'free_modify_consumed' is distinct from 'true') then
      update public.referral_rewards set metadata = coalesce(metadata,'{}'::jsonb) || jsonb_build_object('free_modify_consumed',true,'consumed_at',now()) where id = (select rr.id from public.referral_rewards rr where rr.referrer_user_id = uid and rr.reward_type = 'first_app_created' and rr.status = 'pending' and rr.metadata->>'free_modify_consumed' is distinct from 'true' order by rr.created_at limit 1);
...
begin
  ...
  insert into public.app_demo_sessions(user_id,app_id,version_id,expires_at) values(uid,p_app_id,p_version_id,now()+make_interval(hours=>least(greatest(coalesce(p_hours,72),1),168))) returning * into row;
  return row;
end; $$;
