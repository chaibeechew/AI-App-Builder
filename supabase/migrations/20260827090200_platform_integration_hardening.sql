...
alter table public.user_entitlements add column if not exists entitlement_type text;
alter table public.user_entitlements add column if not exists amount bigint not null default 0;
alter table public.user_entitlements add column if not exists metadata jsonb not null default '{}'::jsonb;
update public.user_entitlements set entitlement_type = coalesce(entitlement_type, 'access') where entitlement_type is null;
alter table public.user_entitlements alter column entitlement_type set not null;
alter table public.user_entitlements drop constraint if exists user_entitlements_source_type_check;
alter table public.user_entitlements add constraint user_entitlements_source_type_check check (source_type in ('promo','subscription','referral','admin','system'));
create unique index if not exists user_entitlements_source_unique_idx on public.user_entitlements(source_type, source_id, entitlement_type) where source_id is not null;

create or replace function public.redeem_promo_code(p_code text)
returns jsonb language plpgsql security definer set search_path = '' as $$
declare uid uuid := auth.uid(); promo public.promo_codes%rowtype; entitlement_until timestamptz; new_balance integer;
begin
  ...
  select * into promo from public.promo_codes where code = upper(trim(p_code)) and is_active = true for update;
  ...
  if promo.bonus_credits > 0 then
    ...
    insert into public.credit_transactions(user_id, amount, balance_after, transaction_type, reference_id, metadata) values (uid, promo.bonus_credits::integer, new_balance, 'promo', promo.id::text, jsonb_build_object('promoCodeId', promo.id, 'promoCode', promo.code));
  end if;
  if promo.free_days > 0 then
    entitlement_until := now() + make_interval(days => promo.free_days);
    insert into public.user_entitlements(user_id, source_type, source_id, entitlement_type, amount, valid_from, valid_until, metadata) values (uid, 'promo', promo.id, 'access', promo.bonus_credits, now(), entitlement_until, jsonb_build_object('promoCodeId', promo.id, 'promoCode', promo.code));
  end if;
...
exception when unique_violation then raise exception 'Promo code already redeemed by this user';
end; $$;
revoke all on function public.redeem_promo_code(text) from public, anon;
grant execute on function public.redeem_promo_code(text) to authenticated;

create or replace function public.generate_marketing_promo_code(p_description text default '90-day free access marketing campaign', p_max_redemptions integer default 1)
returns jsonb language plpgsql security definer set search_path = '' as $$
declare uid uuid := auth.uid(); code text := 'MARKET-' || upper(encode(gen_random_bytes(6), 'hex')); promo_id uuid;
begin
  if uid is null or coalesce((select raw_app_meta_data->>'role' from auth.users where id = uid), '') <> 'admin' then raise exception 'Admin access required'; end if;
  if p_max_redemptions < 1 or p_max_redemptions > 1000000 then raise exception 'Invalid redemption limit'; end if;
  insert into public.promo_codes(code, description, free_days, bonus_credits, max_redemptions, starts_at, expires_at, created_by) values (code, left(coalesce(p_description, ''), 500), 90, 0, p_max_redemptions, now(), now() + interval '90 days', uid) returning id into promo_id;
  insert into public.admin_audit_log(admin_user_id, action, target_type, target_id, metadata) values (uid, 'marketing_promo.create', 'promo_code', promo_id, jsonb_build_object('freeDays', 90, 'maxRedemptions', p_max_redemptions));
...
create table if not exists public.app_publish_requests (
  id uuid primary key default gen_random_uuid(), app_id uuid not null references public.apps(id) on delete cascade, user_id uuid not null references auth.users(id) on delete cascade,
  ...
  version_id uuid references public.app_versions(id) on delete set null, external_build_id text, external_submission_id text, error_message text, metadata jsonb not null default '{}'::jsonb, created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
...
create policy "Users can view their own publish requests" on public.app_publish_requests for select to authenticated using ((select auth.uid()) = user_id);

create or replace function public.queue_app_publish(p_app_id uuid, p_platform text, p_version_id uuid default null)
returns public.app_publish_requests language plpgsql security definer set search_path = '' as $$
declare uid uuid := auth.uid(); result public.app_publish_requests;
begin
  if uid is null then raise exception 'Authentication required'; end if;
  ...
  insert into public.app_publish_requests(app_id, user_id, platform, version_id) values (p_app_id, uid, p_platform, p_version_id) returning * into result;
  return result;
end; $$;
