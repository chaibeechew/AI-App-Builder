...
begin
  ...
  if e.valid_from > now() or (e.valid_until is not null and e.valid_until <= now()) then raise exception 'Entitlement is not active'; end if;
  if e.amount <= 0 then return 0; end if;
  select count(*) into existing_count from public.credit_transactions where user_id = e.user_id and transaction_type = 'subscription' and reference_id = e.id::text;
  if existing_count > 0 then return 0; end if;
  insert into public.credit_accounts(user_id, balance) values (e.user_id, 0) on conflict (user_id) do nothing;
  update public.credit_accounts set balance = balance + e.amount, updated_at = now() where user_id = e.user_id returning * into account;
  insert into public.credit_transactions(user_id, amount, balance_after, transaction_type, reference_id, metadata)
  values (e.user_id, e.amount, account.balance, 'subscription', e.id::text, jsonb_build_object('entitlement_id', e.id, 'source_type', e.source_type, 'source_id', e.source_id));
  return e.amount;
end;
$$;
...
set search_path = ''
as $$
declare s public.subscriptions; p public.subscription_plans; e public.user_entitlements;
begin
  ...
  select * into e from public.grant_entitlement(s.user_id, 'subscription', s.id, 'included_credits', p.included_credits, coalesce(s.current_period_start, now()), s.current_period_end, jsonb_build_object('plan_code', p.code, 'subscription_id', s.id));
