...
const imagePayload=buildCreativeImagePayload({task:'image.inpaint',requestId:'image:abc',input:{prompt:'replace chair',referenceImages:['asset:image:1'],mask:'asset:mask:1',negativePrompt:'distortion',seed:42,count:2}});
assert.equal(imagePayload.schemaVersion,2);assert.equal(imagePayload.capability,'inpaint');assert.equal(imagePayload.input.seed,42);assert.deepEqual(imagePayload.input.referenceImageAssetIds,['asset:image:1']);assert.equal(imagePayload.input.maskAssetId,'asset:mask:1');assert.equal(imagePayload.output.provenanceRequired,true);
...
console.log(`Creative media quality/runtime contracts passed with ${requiredTasks.length} canonical tasks.`);
