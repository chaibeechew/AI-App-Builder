...
// Server-to-server Meta webhook POSTs without browser Origin headers are intentionally allowed through proxy;
// browser cross-site mutations remain blocked.
assert.match(proxy, /Server-to-server\/webhook requests commonly have no browser Origin\/Sec-Fetch-Site headers/);
assert.match(proxy, /if\(fetchSite==="cross-site"\)return true/);
