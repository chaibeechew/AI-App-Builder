...
assert.match(builder,/\/api\/game\/generate/);assert.match(builder,/PRO · FAIR PRICE · FAIR USE/);assert.match(builder,/GAME CREATOR READINESS V2/);assert.match(builder,/Internal creation core/);assert.match(builder,/Production evidence is scored separately/);assert.match(builder,/INTERNAL CORE 100/);assert.match(builder,/Live Transport Contract V1/);assert.match(builder,/Real network\/device evidence/);assert.doesNotMatch(builder,/RM\s?\d/i);
