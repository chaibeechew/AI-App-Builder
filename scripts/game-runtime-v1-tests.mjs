...
assert.match(builder,/\/api\/game\/generate/);assert.match(builder,/PRO · FAIR PRICE · FAIR USE/);assert.match(builder,/100 points requires evidence/);assert.match(builder,/EVIDENCE GATED/);assert.match(builder,/Internal runtime gates can reach 100/);assert.match(builder,/Overall Game Creator production readiness remains separate/);assert.match(builder,/live transport, matchmaking, relay infrastructure and real network\/device evidence/);assert.doesNotMatch(builder,/RM\s?\d/i);
...
console.log("✓ Game Quality Gate scores 100 only when all 10 base-runtime evidence dimensions pass; overall Game Creator readiness remains separately evidence-gated");
