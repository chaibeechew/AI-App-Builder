...
assert.match(builder,/\/api\/game\/generate/);assert.match(builder,/PRO · FAIR PRICE · FAIR USE/);assert.match(builder,/Quality Target 100\/100/);assert.match(builder,/10 × 10 = 100/);assert.match(builder,/internal runtime evidence gate/);assert.match(builder,/does not promise zero defects/);assert.doesNotMatch(builder,/RM\s?\d/i);
...
console.log("✓ Game Quality Gate scores 100 only when all 10 evidence dimensions pass; missing runtime evidence cannot be marketed as 100");
