...
assert.equal(GAME_CREATOR_POLICY.creatorFirst,true);
assert.ok(GAME_CREATOR_POLICY.fairUse.maxNewGameStartsPerHour>0);
assert.deepEqual(GAME_CREATOR_POLICY.fairUse.progressiveCooldownMinutes,[30,60,120,240,480]);
assert.equal(GAME_CREATOR_POLICY.fairUse.maximumCooldownHours,8);
assert.equal(GAME_CREATOR_POLICY.fairUse.cooldownOnlyAffectsGameCreation,true);
assert.equal(GAME_CREATOR_POLICY.fairUse.ordinaryFeaturesRemainAvailable,true);
assert.equal(GAME_CREATOR_POLICY.accessPlans.professional.priceUsd,68);
assert.equal(GAME_CREATOR_POLICY.accessPlans.full.priceUsd,199);
assert.equal(GAME_CREATOR_POLICY.accessPlans.full.ordinaryGameCooldownExempt,true);
assert.equal(GAME_CREATOR_POLICY.commercialTerms.buyoutLicenseAvailable,false);
assert.equal(GAME_CREATOR_POLICY.commercialTerms.platformSalesSharePercent,5);
assert.equal(GAME_CREATOR_POLICY.commercialTerms.salesShareBasis,"gross_game_sales_revenue_excluding_taxes_refunds_chargebacks");
assert.equal(GAME_CREATOR_POLICY.commercialTerms.appliesAcrossAllSalesChannels,true);
...
assert.match(route,/getAppBuilderAccess/);assert.match(route,/professional\.active/);assert.match(route,/PRO_GAME_CREATOR_REQUIRED/);assert.match(route,/GAME_FAIR_USE_TEMPORARY_LIMIT/);assert.match(route,/GAME_CREATOR_COOLDOWN/);assert.match(route,/maxNewGameStartsPerHour/);assert.match(route,/x-soolen-game-gateway/);assert.match(route,/POST as generateApp/);
assert.match(route,/gameCommercialTerms/);assert.match(route,/X-LANERIQ-Game-Buyout/);assert.match(route,/X-LANERIQ-Game-Sales-Share/);assert.doesNotMatch(route,/X-LANERIQ-Game-Profit-Share/);
assert.match(mainGenerate,/isMobileGameIdea/);assert.match(mainGenerate,/x-soolen-game-gateway/);assert.match(mainGenerate,/access\?\.professional\?\.active/);assert.match(mainGenerate,/PRO_GAME_CREATOR_REQUIRED/);assert.ok(mainGenerate.indexOf("if(isMobileGameIdea(combinedInput))")<mainGenerate.indexOf("const entitlement=await consumeAppBuilderEntitlement(userId"),"Pro game gate must run before ordinary entitlement/credit charging");
...
assert.match(gate,/Game creation needs a creator plan/);assert.match(gate,/US\$68 \/ 12 MONTHS/);assert.match(gate,/US\$199 \/ 12 MONTHS/);assert.match(gate,/No game buyout license/);assert.match(gate,/5% share of game sales revenue/i);assert.match(gate,/outside LANERIQ AI/i);assert.match(gate,/continues after creator-plan access ends/i);
assert.match(pricing,/FULL ACCESS/);assert.match(pricing,/\$\{pricing\.fullAccess\.priceUsd\}/);assert.match(pricing,/30m → 1h → 2h → 4h → max 8h/);assert.match(pricing,/5% game sales revenue share/);assert.match(pricing,/outside-platform sales too/);
...
assert.match(banner,/\/game-builder/);assert.match(studio,/Pro Game Creator/);assert.match(proMode,/gameCreator/);assert.match(proMode,/normalGenuineUseIncluded: true/);assert.match(proMode,/fullAccessUsd: 199/);assert.match(proMode,/fullAccessOrdinaryCooldownExempt: true/);assert.match(proMode,/name: "Pro Game Creator"/);assert.match(proMode,/href: "\/game-builder"/);assert.doesNotMatch(banner,/RM\s?\d/i);
...
console.log("✓ Game Runtime V1 covers complete run win/lose, deterministic spawns, input recovery, bounded performance, validated autosave, audio/haptics, accessibility and mobile lifecycle recovery");
console.log("✓ Creator plans keep Game creation gated while App/Website generation cannot bypass into Game mode");
console.log("✓ Creator-first Fair Use uses 30m→1h→2h→4h→8h Game-only cooldowns and Full Access removes the ordinary progressive cooldown");
console.log("✓ Game commercial policy is locked: creator ownership, no buyout and continuing 5% LANERIQ AI share of game sales revenue across all sales channels");
