...
assert.doesNotMatch(stability,/ZERO_COST_PROVIDER_ALLOWLIST|providers\.text/,"Production stability must not require or expose provider-name arrays from the canonical API");
assert.match(stability,/providers\?\.providerNamesHidden,true/);
assert.match(stability,/providers\?\.count\|\|0\)>=1/);
...
console.log("✓ Every capability probe validates provider-opaque zero-cost semantics, eight free-ready core capabilities and fail-closed Professional access");
console.log("✓ Provider names stay hidden; readiness is proven through count + zero-cost policy + capability state instead of provider identity leakage");
