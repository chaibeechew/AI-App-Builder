...
assert.match(uploader,/credentials:"same-origin"/);
assert.match(uploader,/cache:"no-store"/);
assert.match(uploader,/storage\.from\("user-assets"\)\.upload/);
assert.match(uploader,/\.eq\("user_id",user\.id\)\.eq\("content_fingerprint",fingerprint\)/);
assert.match(uploader,/content_fingerprint:fingerprint/);
assert.match(uploader,/reusableAcrossUsers:false/);
assert.match(uploader,/privateCustomerAsset:true/);
