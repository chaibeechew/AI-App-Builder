...
assert.match(dispatchRoute,/LANERIQ_COMMUNICATIONS_ALLOW_CUSTOMER_BILLED/);
assert.ok(dispatchRoute.indexOf("message={...message,allowCustomerBilledProvider:false}")<dispatchRoute.indexOf("const result=await dispatchServiceMessage"),"Zero-cost customer-billed gate must execute before delivery.");
...
console.log("✓ Signed callers cannot bypass zero-cost mode unless server policy explicitly enables customer-billed routes");
