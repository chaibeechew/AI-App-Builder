...
assert.match(migration,/revoke all on function public\.server_claim_communication_service_request\(text,text,text,text,timestamptz\) from public,anon,authenticated/);
assert.match(migration,/revoke all on function public\.server_finish_communication_service_request\(text,text\) from public,anon,authenticated/);
assert.match(migration,/grant execute on function public\.server_claim_communication_service_request\(text,text,text,text,timestamptz\) to service_role/);
assert.match(migration,/grant execute on function public\.server_finish_communication_service_request\(text,text\) to service_role/);
...
console.log("✓ Persistent nonce + idempotency ledger is restricted by explicit function grants and replay-safe");
