...
for(const id of ["rpg","puzzle","action"]){const profile=inferGenreRuntimeProfile({archetype:id});assert.equal(profile.runtime,"specialist-runtime-v1");const generic=compileGameRuntimeV1({game:{archetype:id,genre:id}});assert.equal(generic.runtimeCompleteness,"dedicated");}

const page=fs.readFileSync("app/a/[id]/page.js","utf8"),client=fs.readFileSync("app/a/[id]/SpecialistRuntimeClient.js","utf8");
assert.match(page,/SpecialistRuntimeClient/);assert.match(page,/rpg_runtime_view/);assert.match(page,/puzzle_runtime_view/);assert.match(page,/action_runtime_view/);assert.match(client,/RPG \/ ADVENTURE/);assert.match(client,/PUZZLE \/ BRAIN/);assert.match(client,/ACTION COMBAT/);assert.match(client,/HINT/);assert.match(client,/UNDO/);assert.match(client,/DODGE/);assert.match(client,/COMBO/);
...
console.log("✓ RPG, Puzzle and Action projects route to a dedicated Specialist Runtime instead of generic arcade gameplay");
