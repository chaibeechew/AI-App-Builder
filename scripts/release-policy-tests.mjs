...
const weakResult=evaluateReleaseReadiness(oneWeak);assert.equal(weakResult.releaseReady,false);assert.deepEqual(weakResult.belowTarget,['security']);
...
const incompleteVisual=assessBuildQuality({...richSpec,designSystem:{...richSpec.designSystem,wallpaperPreset:'',cardStyle:''}});assert.equal(incompleteVisual.dimensions.find(x=>x.id==='beauty')?.score<=99,true);
