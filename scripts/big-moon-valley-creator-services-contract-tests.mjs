...
assert.match(css,/padding-top:clamp\(165px,24svh,270px\)/,'Credits must preserve a desktop cinematic landscape zone');
...
assert.ok(selectorCount>=100,'Creator-services selector scope check must cover the full Credits/Templates/Studio/Visual Studio layer');

assert.match(credits,/fetch\("\/api\/credits", \{ cache: "no-store" \}\)/,'Credits must keep its server-backed balance/ledger API');
assert.match(credits,/const ledger = Array\.isArray\(data\?\.ledger\) \? data\.ledger : \[\];/,'Credits must keep ledger normalization');
...
console.log('✓ Big Moon Valley now carries through Credits, Templates, Studio and Visual Studio');
console.log(`✓ ${selectorCount} creator-service selectors are scoped only to LANERIQ product-shell roots`);
console.log('✓ Credits, inspiration search/reimagine, quality modules and image generate/save contracts remain intact');
