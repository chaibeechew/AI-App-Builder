...
assert.ok(listCreativeMediaTasks().length>=requiredTasks.length);
for(const state of ['draft','queued','running','quality-check','retrying','completed','failed','blocked_by_policy','cancelled']) assert.ok(CREATIVE_MEDIA_JOB_STATES.includes(state));
for(const level of ['CODE_READY','CI_READY','PREVIEW_READY','PROVIDER_READY','PROVIDER_CONNECTED','LIVE_PROVIDER_VERIFIED','BROWSER_VERIFIED','PRODUCTION_VERIFIED','REAL_OUTPUT_QUALITY_VERIFIED','PRODUCTION_LIVE_VERIFIED','EVIDENCE_REQUIRED']) assert.equal(CREATIVE_MEDIA_TRUTH_LEVELS[level],level);
for(const surface of ['identity','image','cinema','audio','workflow']) assert.ok(CREATIVE_MEDIA_CONTROL_SURFACES[surface]?.length>0);
...
assert.equal(providerReady.truth,'PROVIDER_CONNECTED');
assert.notEqual(providerReady.truth,'LIVE_PROVIDER_VERIFIED');

const live=buildCreativeMediaExecutionPlan({task:'video.image-to-video',input:{prompt:'orbit camera',referenceImages:['asset:1']},providerConnected:true,providerCapabilities:['image-to-video'],providerProductionEvidence:true,verifiedOutputCount:1});
...
assert.equal(withConsent.truth,'PROVIDER_CONNECTED');
...
const total=listCreativeMediaTasks().length;
...
assert.equal(summary.codeReady,total);
...
console.log(`Creative media control plane contract tests passed with ${total} tasks.`);
