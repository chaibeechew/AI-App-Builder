...
assert.match(adminAuthority, /authorized\(user,\s*"laneriq"\)/, "LANERIQ-primary authorization must identify the LANERIQ session authority");
assert.match(adminAuthority, /authorized\(user,\s*"legacy_bridge"\)/, "Only the bounded legacy compatibility path may identify legacy_bridge authority");
