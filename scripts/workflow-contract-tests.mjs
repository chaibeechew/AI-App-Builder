...
const emailProvider=read('lib/email-provider/server.js');
...
// Generated projects call LANERIQ Communications. Email is provider-owned by LANERIQ and transport stays internal/replaceable.
...
const claimCallIndex=communications.indexOf('const claim=await claimLaneriqCommunication');
const deliveryCallIndex=communications.indexOf('const result=await deliverCommunication');
assert.ok(claimCallIndex>=0&&deliveryCallIndex>claimCallIndex,'LANERIQ persistent guard must run before provider delivery.');
assert.doesNotMatch(communications,/sendManagedEmail|sendManagedWhatsApp|graph\.facebook|resend\.com|TWILIO|api\.twilio/i);
assert.match(deliveryAdapter,/sendLaneriqEmail/);
assert.match(deliveryAdapter,/provider:"LANERIQ Email"/);
assert.match(deliveryAdapter,/sendManagedWhatsApp/);
assert.doesNotMatch(deliveryAdapter,/sendManagedEmail|sendManagedSms|TWILIO|api\.twilio/i);
assert.match(emailProvider,/PROVIDER_NAME="LANERIQ Email"/);
assert.match(emailProvider,/sendManagedEmail/);
assert.match(emailProvider,/laneriq_enqueue_email/);
assert.match(emailProvider,/createCipheriv\("aes-256-gcm"/);
...
assert.match(integrations,/smtpReady\(\)\|\|resendReady\(\)/);
for(const marker of ['integrationStatus().whatsapp.ready','integrationStatus().calendar.ready'])assert.match(integrations,new RegExp(marker.replace(/[().]/g,'\\$&')));
...
console.log('✓ LANERIQ Communications routes Email through LANERIQ Email Provider while internal transports remain replaceable');
