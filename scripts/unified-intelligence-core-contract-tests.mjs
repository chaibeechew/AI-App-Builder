...
assert.equal(log.headVersion,2);assert.equal(verifyWorldEventLog(log).ok,true);
...
const tampered=JSON.parse(JSON.stringify(log));tampered.events[0].patch.patch.weather='tampered';
assert.equal(verifyWorldEventLog(tampered).ok,false);
const branch=forkWorldEventLog(log,{branchId:'what-if-a',fromVersion:2});
assert.equal(branch.baseVersion,2);assert.equal(branch.events.length,0);assert.equal(branch.forkedFrom.branchId,'main');assert.equal(verifyWorldEventLog(branch).ok,true);

let ledger=createEvidenceLedger({worldId:'world-1',projectId:'project-1'});
ledger=appendEvidence(ledger,{evidenceId:'evidence-1',claimId:'creative-1',level:'SIGNED_OBSERVED',artifactHash:HASH_A,observerId:'laneriq-vision-1',sourceId:'creative-observer',signed:true,provenanceVerified:true,observedAt:'2026-09-05T05:00:00Z',expiresAt:'2099-01-01T00:00:00Z',metadata:{qualityScore:94}});
...
assert.equal(memoryScore.available,true);assert.ok(memoryScore.score>0);assert.equal(memory.rawPromptsStored,false);
...
const calibrationSummary=summarizeCalibration(calibration);assert.equal(calibrationSummary.calibrated,1);assert.equal(calibrationSummary.canClaimFutureAccuracy,false);assert.ok(calibrationSummary.meanAbsoluteError>0);
...
assert.equal(blockedAction.allowed,false);assert.ok(blockedAction.blockers.includes('explicit-user-approval-required'));assert.ok(blockedAction.blockers.includes('human-approval-required-for-irreversible-action'));assert.ok(blockedAction.blockers.includes('security-check-required'));
const allowedExternal=assessActionAuthority({action:{external:true,irreversible:true,effects:['publish-approved-asset']},authorization:{scope:'publish:asset-1',explicitUserApproval:true,humanApproval:true},securityAssessment:{checked:true,blocked:false,stale:false,assessmentId:'security-1'}});
assert.equal(allowedExternal.allowed,true);assert.equal(allowedExternal.security.intelligenceCanAuthorizeClean,false);
...
assert.equal(validateExecutableRealityPlan(normalPlan).ok,true);assert.equal(normalPlan.nodes.at(-1).id,'world-update');
...
assert.equal(premiumBlocked.executable,false);assert.ok(premiumBlocked.blockers.includes('premium-escalation-blocked-by-zero-free-mode'));
...
assert.equal(unifiedPlan.dagValidation.ok,true);assert.equal(unifiedPlan.canDispatch,true);assert.equal(unifiedPlan.creative.prepared.candidatePlan.candidates[0].providerId,'image-provider-free');
...
assert.equal(admittedA.allowed,true);assert.equal(admittedA.worldState.version,2);assert.equal(admittedA.eventLog.headVersion,2);assert.equal(admittedA.canonicalEntityId,'character-1');
assert.equal(admittedA.worldState.entities.find(row=>row.id==='character-1').attributes.lastProviderId,'provider-a');

let ledger2=appendEvidence(ledger,{evidenceId:'evidence-2',claimId:'creative-2',level:'SIGNED_OBSERVED',artifactHash:HASH_B,observerId:'laneriq-vision-1',sourceId:'creative-observer',signed:true,provenanceVerified:true,observedAt:'2026-09-05T05:30:00Z',expiresAt:'2099-01-01T00:00:00Z'});
...
assert.equal(admittedB.allowed,true);assert.equal(admittedB.canonicalEntityId,'character-1');assert.equal(admittedB.worldState.entities.length,1);assert.equal(admittedB.worldState.entities[0].attributes.lastProviderId,'provider-b');assert.equal(admittedB.worldState.version,3);assert.equal(verifyWorldEventLog(admittedB.eventLog).ok,true);
...
const summary=summarizeUnifiedIntelligenceCore();assert.equal(summary.policy.simulationIsNotPrediction,true);assert.equal(summary.policy.configuredIsNotLive,true);assert.equal(summary.liveClaims.length,0);
console.log('LANERIQ Unified Intelligence Core v1 contract tests: PASS');
