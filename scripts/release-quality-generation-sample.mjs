...
const MIN_AVERAGE_UNIQUE_CANDIDATES=2.5;
...
for(const benchmarkCase of plan.cases){
  try{
    ...
    rows.push({caseId:benchmarkCase.id,industry:benchmarkCase.industry,archetypeId:benchmarkCase.archetypeId,difficulty:benchmarkCase.difficulty,provider:generated.aiProvider,sourceKind:'zero-cost-full-autonomous-generation',validJson:true,score:evaluated.score,decision:evaluated.decision,originalityScore:evaluated.originalityScore,coverageScore:evaluated.coverageScore,liuiScore:evaluated.liuiScore,releaseReadinessScore:evaluated.releaseReadinessScore,hardBlockerCount:evaluated.hardBlockers.length,outcomeFingerprint:evaluated.outcomeFingerprint,...candidate});
  }catch(error){
...
console.log(`✓ Outcome candidates avg ${candidates.averageCandidateCount} / unique ${candidates.averageUniqueCandidateCount} (floor ${MIN_AVERAGE_UNIQUE_CANDIDATES}) | paid shadow calls ${candidates.paidShadowCalls}`);
...
if(summary.caseCount!==50||summary.industryCoverage!==50||summary.validJsonRate!==100||candidates.paidShadowCalls!==0||candidates.providerSet.some(provider=>provider!=='soolen-local')){
  ...
  process.exitCode=1;
}else if(candidates.averageUniqueCandidateCount<MIN_AVERAGE_UNIQUE_CANDIDATES){
  console.error(`Candidate structural diversity fell below ${MIN_AVERAGE_UNIQUE_CANDIDATES}.`);
  process.exitCode=1;
