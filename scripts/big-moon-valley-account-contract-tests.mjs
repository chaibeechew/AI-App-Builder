...
assert.match(auth,/const \{ data: trustedUserData, error: trustedUserError \} = await supabase\.auth\.getUser\(\)/,'Trusted compatibility session verification must remain intact during migration');
...
assert.match(projects,/supabase\.auth\.getUser\(\)/,'My Projects must remain authenticated during the session migration');
...
assert.match(projects,/await client\.auth\.signOut\(\); redirect\("\/auth"\)/,'My Projects sign-out must retain the current session close flow');
...
console.log('✓ Big Moon Valley carries through LANERIQ-owned Email OTP and authenticated My Projects shells');
console.log(`✓ ${selectorCount} account-shell selectors are scoped only to Auth/loading/My Projects roots`);
console.log('✓ Email Code is LANERIQ-generated/verified; WhatsApp remains the only compatibility phone path');
console.log('✓ Trusted session compatibility, safe redirects, owner filtering and sign-out remain intact');