...
for(const[id,idea]of specialistCases){const plan=inferAdvancedGenreKnowledge(idea,id);assert.equal(plan.matched,true);assert.equal(plan.id,id);assert.ok(plan.systems.length>=7,`${id} should have deep specialist knowledge`);const profile=inferGenreRuntimeProfile({archetype:id});assert.equal(profile.runtime,"remaining-genre-runtime-v1");}
...
const all=evaluateGameCreatorReadiness({internalEvidence:Object.fromEntries(current.internal.passed.map(k=>[k,true])),productionEvidence:{liveTransport:true,matchmaking:true,networkLoadTests:true,realDeviceIos:true,realDeviceAndroid:true,storeSubmissionEvidence:true}});assert.equal(all.canClaimProduction100,true);
console.log("✓ Game Creator Readiness V2 separates internal creation-core 100 from production evidence 100");

const page=fs.readFileSync("app/a/[id]/page.js","utf8"),builder=fs.readFileSync("app/game-builder/page.js","utf8");for(const token of ["RemainingGenreRuntimeClient","shooter_runtime_view","platformer_runtime_view","tower_defense_runtime_view","idle_runtime_view","party_runtime_view","educational_runtime_view"])assert.ok(page.includes(token),`Generated runtime route missing ${token}`);for(const token of ["Live Transport Contract V1","Game Creator Readiness V2","INTERNAL CORE 100","Educational","Platformer","Tower Defense"])assert.ok(builder.includes(token),`Game Builder missing ${token}`);
console.log("✓ Customer Game Builder exposes the expanded specialist matrix and truthful readiness boundary");
