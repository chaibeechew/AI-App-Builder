...
export default async function AppDashboard({ params }) {
  ...
  const qualityReport=assessBuildQuality(spec);const releaseReadiness=evaluateReleaseReadiness(qualityReport);
  const readiness=buildProjectReadiness({specification:spec,qualityReport,releaseReady:releaseReadiness.releaseReady,dataGroups,workflowCount,publishStatus:app.publish_status});
  ...
  return <main className="page"><div className="wrap">
    ...
    <section className="readinessPanel">
      <div className="readinessHead"><div><span>PROJECT READINESS</span><h2>SoolenAI shows what is ready — and what still needs work.</h2><p>This is a saved-project readiness view. Live providers, store submission, payments and real-device evidence stay separate until actually verified.</p></div><strong>{readiness.overall}<small>/100</small></strong></div>
      <div className="readinessGrid">{readiness.areas.map(area=><article key={area.id} className={area.ready?"ready":"attention"}><div><b>{area.label}</b><strong>{area.score}</strong></div><div className="meter"><i style={{width:`${area.score}%`}}/></div><small>{area.note}</small></article>)}</div>
      <div className="readinessActions"><Link className="fixAll" href={{pathname:`/editor/${id}`,query:{instruction:readiness.fixInstruction}}}>✨ Fix Everything →</Link><Link href={`/release/${id}`}>Open Release Check</Link><Link href={`/operations/${id}`}>AI Health Check</Link></div>
    </section>
