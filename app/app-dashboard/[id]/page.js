...
export default async function AppDashboard({ params }) {
  ...
  return <main className="page"><div className="wrap">
    <Link href="/my-apps" className="back">← My Projects</Link>
    ...
    <section className="card"><div className="eyebrow">PROJECT ACTIONS</div><h2>Manage App + Website together</h2><div className="actions"><Link href={`/editor/${id}`} className="primary">Modify Project</Link><Link href={`/release/${id}`} className="secondary">Publishing Center</Link></div><div className="flow"><span>Demo</span><i>→</i><span>App + Website</span><i>→</i><span>Modify</span><i>→</i><span>Test</span><i>→</i><span>Publish</span></div></section>
