...
function authErrorMessage(error, method) {
  ...
  if (code === "phone_provider_disabled" || /unsupported phone provider/i.test(raw)) return "SMS verification is not enabled on the authentication service yet. Use Email Code for now, or enable the managed SMS provider in Auth settings.";
...
function AuthForm() {
  ...
  return <main className="authPage"><div className="shade" />
    ...
    <div className="authCard">
      <div className="authEyebrow">SECURE VERIFICATION</div><h1>Choose how to receive your code</h1><p className="authIntro">Use a one-time 6-digit verification code by Email or SMS. No password required.</p>
      <div className="authTabs"><button type="button" className={method === "email" ? "active" : ""} onClick={() => switchMethod("email")}>✉ Email Code</button><button type="button" className={method === "sms" ? "active" : ""} onClick={() => switchMethod("sms")}>◉ SMS Code</button></div>
      ...
      {!sent ? <form onSubmit={sendCode}>
        ...
        {method === "sms" && <div className="providerNote">Use international phone format. SMS codes are delivered by the managed authentication provider when phone authentication is enabled.</div>}
        ...
        <button type="button" className="switchFallback" disabled={loading} onClick={() => switchMethod(method === "email" ? "sms" : "email")}>Use {method === "email" ? "SMS" : "Email"} instead</button>
      </form>}
      ...
      <div className="authSecurity">● One-time code · Rate-limit aware · Short-lived verification · Email fallback</div>
    </div>
    <style jsx>{`
