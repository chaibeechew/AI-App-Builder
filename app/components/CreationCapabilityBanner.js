...
const CREATIVE=[
  ...
  {href:"/image-studio?mode=create",eyebrow:"PHOTO + VIDEO",title:"AI Photo & Video Generator",text:"Mixed visual creation for products, games, social content and store media."},
...
export default function CreationCapabilityBanner(){
  ...
  return <section className="creationExpansion" aria-label={`${PRODUCT_BRAND.name} creation capabilities`}>
    <div className="inner">
      <div className="headline"><div><small>{PRODUCT_BRAND.descriptor.toUpperCase()}</small><h2>Create your own <b>App</b>, <b>Website</b> or <b>Mobile Game</b>.</h2><p>{PRODUCT_BRAND.tagline} SoolenAI plans, designs, builds, tests and prepares the product foundation—mobile-first, with iOS + Android targets and a fast web preview path.</p></div><div className="platforms"><span>APPS</span><span>GAMES</span><span>WEB</span><span>iOS + Android</span></div></div>
      ...
      <div className="gameCallout"><div><small>SOOLENAI PRO GAME CREATOR · FAIR PRICE · FAIR USE</small><strong>From one game idea to a playable mobile game foundation.</strong><span>2D/3D runtime paths · touch controls · player/enemy/Boss AI · levels · save/load · audio · mobile performance contracts · automated playtesting · iOS/Android readiness. Live multiplayer and store release remain evidence-gated.</span></div><Link href="/game-builder">Create a Game ›</Link></div>
    </div>
