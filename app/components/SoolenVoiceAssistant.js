...
export default function SoolenVoiceAssistant() {
  ...
  async function start() {
    try {
      ...
      recorder.onstop = async () => {
        stream.getTracks().forEach((track) => track.stop()); setRecording(false); setStatus("Soolen AI is listening to your idea…");
        try {
          const blob = new Blob(chunksRef.current, { type: recorder.mimeType || "audio/webm" }); const buffer = await blob.arrayBuffer(); let binary = ""; const bytes = new Uint8Array(buffer);
          ...
          const d = await r.json(); if (!r.ok || !d.transcript) throw new Error(d?.error || "Voice transcription failed."); setTranscript(d.transcript); setStatus("Ready. Review your idea, then build.");
        } catch (e) { setStatus(e?.message || "Unable to process voice input."); }
      };
      recorderRef.current = recorder; recorder.start(); setRecording(true); setStatus("Listening… tap the microphone again when finished.");
    } catch (e) { setStatus(e?.message || "Microphone permission is required."); }
  }
  function stop() { recorderRef.current?.stop(); }
  async function build() {
    if (!transcript.trim()) return; setStatus("Soolen AI is building your app…");
    try { const r = await fetch("/api/generate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ idea: transcript, voiceTranscript: transcript }) }); const d = await r.json(); if (!r.ok) throw new Error(d?.error || "Generation failed."); setStatus(`App ready: ${d?.app?.name || d?.specification?.name || "Your app"}`); }
  ...
  return <>
    ...
    {open && <div className="sv-backdrop" onClick={() => setOpen(false)}><div className="sv-panel" onClick={(e) => e.stopPropagation()}>
      <button className="sv-close" onClick={() => setOpen(false)}>×</button><div className="sv-kicker">SOOLEN AI</div><h2>Tell me what you want to build.</h2><p>Speak naturally. Soolen AI understands your idea, industry patterns and visual direction.</p>
