...
function isTouchAppleDevice() {
  ...
  return /iPhone|iPad|iPod/i.test(navigator.userAgent) || (navigator.platform === "MacIntel" && Number(navigator.maxTouchPoints || 0) > 1);
}

async function ensureTouchAppleMicrophoneAccess() {
  if (!isTouchAppleDevice()) return { checked: false, granted: true };
  if (typeof navigator.mediaDevices?.getUserMedia !== "function") return { checked: false, granted: true };
  const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
  try {
    return { checked: true, granted: true };
  } finally {
...
export default function SoolenVoiceAssistant() {
  ...
  async function start(){
    ...
    try{
      ...
      // On iPhone/iPad, prime microphone permission only after this explicit user tap.
      // The stream is released immediately; LANERIQ never keeps a hidden microphone stream alive.
      const micAccess=await ensureTouchAppleMicrophoneAccess();
      if(micAccess.checked)setStatus("Microphone access granted. Starting voice recognition…");
  ...
  return <>
    <button className="sv-fab" type="button" onClick={()=>{setOpen(true);setStatus("");}} aria-label="Voice idea">🎙️<span>Voice Idea</span></button>
    {open&&<div className="sv-backdrop" role="dialog" aria-modal="true" aria-label="Voice Idea">
      <div className="sv-panel">
        <button className="sv-close" type="button" onClick={close} aria-label="Close Voice Idea">×</button>
        ...
        <p className="sv-sub">Voice starts only after you tap the microphone. On iPhone/iPad, LANERIQ checks microphone access from that same tap and releases the temporary stream immediately. Apple may show its normal system permission prompt.</p>
        ...
        <div className="sv-voicecard">
          <button className={recording?"sv-mic recording":"sv-mic"} type="button" onClick={recording?()=>stopRecognition():start} aria-label={recording?"Stop listening":"Start listening"}><span>🎤</span></button>
          ...
          <button className="sv-play" type="button" onClick={speaking?stopSpeaking:()=>speak(text)} disabled={!text.trim()}>{speaking?"■ STOP PLAYBACK":"▶ PLAY BACK TEXT"}</button>
        </div>
        ...
        <button className="sv-build" type="button" onClick={sendToBuilder}>CREATE APP · WEB · GAME →</button>
        {status&&<div className="sv-status" role="status" aria-live="polite">{status}</div>}
      </div>
    </div>}
    <style jsx global>{`
      .sv-fab{position:fixed;right:max(12px,env(safe-area-inset-right));bottom:max(12px,env(safe-area-inset-bottom));z-index:40;min-height:48px;border:1px solid #e1b83d;border-radius:999px;padding:12px 18px;background:#0a6d54;color:#fff;font-size:15px;font-weight:900;box-shadow:0 14px 35px #0005;touch-action:manipulation}.sv-fab span{margin-left:8px}
      .sv-backdrop{position:fixed;inset:0;z-index:50;background:#00130fe8;backdrop-filter:blur(8px);display:grid;place-items:center;padding:12px}.sv-panel{position:relative;width:min(560px,100%);max-height:min(900px,96svh);overflow:auto;background:#fffdf7;color:#151d1a;border-radius:28px;padding:28px;box-shadow:0 30px 90px #0008}.sv-close{position:absolute;right:12px;top:8px;min-width:44px;min-height:44px;border:0;background:none;font-size:31px;color:#68716d;touch-action:manipulation}.sv-topline{display:grid;grid-template-columns:44px 1fr 44px;align-items:center;margin-bottom:18px}.sv-globe{font-size:29px;color:#f4b71b;font-weight:900}.sv-brand{text-align:center;font-size:22px;font-weight:950}.sv-brandline{width:74px;height:4px;border-radius:999px;background:#f3b617;margin:7px auto 0}.sv-langtag{justify-self:end;border:1px solid #eee3cf;border-radius:13px;padding:9px;font-weight:900}.sv-heroicon{width:82px;height:82px;border-radius:999px;display:grid;place-items:center;margin:8px auto 14px;background:#fff4cf;font-size:38px}.sv-panel h2{text-align:center;font-size:32px;line-height:1.08;margin:0 0 9px}.sv-sub{text-align:center;font-size:14px;line-height:1.55;color:#69716d;margin:0 auto 22px;max-width:470px}.sv-label{display:block;font-size:11px;font-weight:950;margin:14px 0 7px;letter-spacing:.05em}.sv-panel select,.sv-panel textarea{width:100%;border:1px solid #ddd8cb;border-radius:15px;padding:13px 15px;background:#fff;color:#17201d;font:inherit;font-size:16px;box-sizing:border-box}.sv-panel select{min-height:44px}.sv-panel textarea{min-height:126px;resize:vertical}.sv-voicecard{border:1px solid #eee4d1;border-radius:22px;padding:20px;text-align:center;background:#fffefb;margin-top:16px}.sv-mic{width:82px;height:82px;border:0;border-radius:999px;background:#ffc21f;box-shadow:0 12px 28px #d99c1745;touch-action:manipulation}.sv-mic.recording{animation:svpulse 1.2s infinite}.sv-mic span{font-size:31px}.sv-voicecard strong{display:block;font-size:20px;margin-top:13px}.sv-voicecard>span{display:block;color:#6f7672;margin:7px auto 14px;font-size:13px;line-height:1.45}.sv-play{width:100%;min-height:44px;border:0;border-radius:14px;padding:12px;background:#fff;color:#5d615f;box-shadow:0 6px 18px #0000000c;font-weight:900;touch-action:manipulation}.sv-play:disabled{opacity:.45}.sv-count{text-align:right;color:#888;font-size:10px;margin-top:4px}.sv-toggle{display:flex;align-items:center;gap:10px;font-weight:900;margin:16px 0 13px;min-height:44px}.sv-toggle input{width:20px;height:20px;accent-color:#f1b517}.sv-build{width:100%;min-height:50px;border:0;border-radius:16px;padding:14px;background:linear-gradient(90deg,#ffd84c,#ffc11f);font-size:16px;font-weight:950;touch-action:manipulation}.sv-status{margin-top:11px;border-radius:13px;padding:10px 12px;background:#edf7f1;color:#1e5f49;font-size:12px;font-weight:750;line-height:1.45}@keyframes svpulse{0%,100%{box-shadow:0 0 0 0 #ffc21f55}50%{box-shadow:0 0 0 14px #ffc21f00}}
      @media(max-width:600px){.sv-backdrop{padding:0}.sv-panel{width:100%;height:100svh;max-height:none;border-radius:0;padding:calc(18px + env(safe-area-inset-top)) max(16px,env(safe-area-inset-right)) calc(24px + env(safe-area-inset-bottom)) max(16px,env(safe-area-inset-left));}.sv-panel h2{font-size:28px}.sv-fab span{display:none}.sv-heroicon{width:70px;height:70px;font-size:32px}.sv-sub{font-size:13px}.sv-voicecard{padding:16px}.sv-mic{width:72px;height:72px}}
