...
const STORAGE_KEY="ai-build-wallpaper";const SEED_KEY="ai-build-wallpaper-seed";const COOKIE_KEY="ai_build_wallpaper";const RETIRED_BIG_MOON_PRESET="moon-city";const LIUI_FALLBACK_PRESET="neon-skyline";
...
function normalizePath(pathname){return String(pathname||"/").toLowerCase().split("?")[0]||"/";}
function customerSurface(pathname){const path=normalizePath(pathname);return /^\/a\//.test(path)||/^\/website\//.test(path);}
function wallpaperControlHidden(pathname){const path=normalizePath(pathname);return path==="/"||path==="/auth"||path==="/mobile-readiness"||path==="/production-e2e";}
...
function currentLiuiWallpaper(candidate){return candidate===RETIRED_BIG_MOON_PRESET?LIUI_FALLBACK_PRESET:candidate;}
