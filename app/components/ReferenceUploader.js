...
export default function ReferenceUploader() {
  ...
  function addFiles(event) {
    ...
    if (!room) { setError(`Maximum ${MAX_FILES} references per build.`); return; }
    const accepted = [];
    for (const file of selected.slice(0, room)) {
      const kind = fileKind(file);
      const max = kind === "video" ? MAX_VIDEO_BYTES : MAX_IMAGE_BYTES;
      if (kind === "file") continue;
      if (file.size > max) { setError(`${file.name} is too large. Images/sketches: 12 MB max. Video: 80 MB max.`); continue; }
      accepted.push({ id: `${file.name}-${file.size}-${file.lastModified}`, name: file.name, size: file.size, type: file.type, kind, file, url: URL.createObjectURL(file) });
    }
    if (accepted.length) { setItems((current) => [...current, ...accepted]); setError(""); setAnalysis(""); }
  }
  ...
  async function persistPrivateAssets() {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return [];
    const saved = [];
    for (const item of items) {
      const safe = (item.name || "asset").replace(/[^a-zA-Z0-9._-]+/g, "-").slice(-100);
      const path = `${user.id}/${crypto.randomUUID()}-${safe}`;
      const { error: uploadError } = await supabase.storage.from("user-assets").upload(path, item.file, { contentType: item.type, upsert: false });
      ...
      const { data: asset, error: dbError } = await supabase.from("asset_library").insert({
        ...
        alt_text: "Customer-owned media uploaded for AI project placement",
      }).select("id,file_name,mime_type,category").single();
  ...
  async function analyzeAndUse() {
    ...
    try {
      ...
      const response = await fetch("/api/reference-analyze", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ references: references.slice(0, 12) }) });
      const value = await response.json().catch(() => ({}));
      if (!response.ok || !value?.analysis) throw new Error(value?.error || "Reference analysis failed.");

      const savedAssets = await persistPrivateAssets();
      ...
      try {
        ...
        sessionStorage.setItem("soolenPendingAssetIds", JSON.stringify(savedAssets.map((item) => item.id)));
        sessionStorage.setItem("soolenPendingAssetMeta", JSON.stringify(savedAssets));
      } catch {}

      const existingIdea = String(document.querySelector("textarea")?.value || "").trim();
      const combinedIdea = existingIdea && !existingIdea.includes("VISUAL REFERENCE ANALYSIS:") ? `${existingIdea}\n\n${brief}` : brief;
      window.dispatchEvent(new CustomEvent("soolen-app-idea", { detail: { idea: combinedIdea, assetIds: savedAssets.map((item) => item.id) } }));
      setOpen(false);
    } catch (e) {
      setError(e?.message || "Unable to analyze and save references.");
    } finally { setAnalyzing(false); }
  }

  return <div className="referenceDock">
    <input ref={inputRef} hidden type="file" multiple accept="image/*,video/*" onChange={addFiles} />
    <button className="referenceTrigger" type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open}><span>＋</span><b>{countLabel}</b></button>
    {open && <section className="referencePanel">
      <div className="referenceHead"><div><small>AI REFERENCES</small><h3>Show AI what you mean.</h3></div><button type="button" onClick={() => setOpen(false)}>×</button></div>
      <p>Upload photos, screenshots, video or hand-drawn sketches. SoolenAI analyzes them, saves customer-owned media privately, then places useful assets into the generated project.</p>
      <button className="referenceDrop" type="button" onClick={choose}><b>Upload Photo · Video · Sketch</b><span>Images/sketches up to 12 MB · video up to 80 MB · max {MAX_FILES} files</span></button>
      {!!items.length && <div className="referenceGrid">{items.map((item) => <article key={item.id}>{item.kind === "image" ? <img src={item.url} alt="User reference preview"/> : <video src={item.url} muted playsInline/>}<div><b>{item.kind === "image" ? "Image / sketch" : "Video reference"}</b><small>{readableSize(item.size)}</small></div><button type="button" onClick={() => remove(item.id)}>Remove</button></article>)}</div>}
      {!!items.length && <button className="analyze" type="button" onClick={analyzeAndUse} disabled={analyzing}>{analyzing ? "AI analyzing + saving privately…" : "AI ANALYZE + SAVE + USE →"}</button>}
      {analysis && <div className="analysis"><b>✓ Reference brief + private media ready</b><span>SoolenAI can associate these customer-owned assets with the generated project.</span></div>}
      {error && <div className="referenceError">{error}</div>}
      <div className="referenceRule"><b>AI rule:</b> Analyze → understand → reimagine → place customer-owned media appropriately → generate original App + Website. Never reuse another customer's private assets.</div>
    </section>}
    <style jsx>{`
