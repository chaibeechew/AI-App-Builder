...
export default function AccountNav() {
  ...
  return <div className="accountNav" ref={rootRef}>
    ...
    {open && <div className="accountMenu" role="menu" aria-label="Account menu">
      ...
      <button type="button" role="menuitem" onClick={() => go("/credits")}>Credits</button>
      <button className="signout" type="button" role="menuitem" onClick={signOut} disabled={signingOut}>Sign out</button>
    </div>}
    <style jsx>{`.accountNav{position:fixed;right:max(14px,env(safe-area-inset-right));top:max(70px,calc(env(safe-area-inset-top) + 54px));z-index:1000;font-family:Inter,system-ui,-apple-system,sans-serif}.accountBar{display:flex;align-items:center;gap:7px}.accountTrigger{display:flex;align-items:center;gap:8px;min-height:44px;max-width:220px;border:1px solid rgba(216,191,98,.26);border-radius:999px;padding:6px 10px 6px 6px;background:rgba(3,16,13,.9);color:#fff;box-shadow:0 12px 34px #0006;backdrop-filter:blur(14px);cursor:pointer;touch-action:manipulation}.visibleLogout{min-height:44px;border:1px solid rgba(216,191,98,.34);border-radius:999px;padding:0 13px;background:rgba(3,16,13,.94);color:#e9c968;font-size:11px;font-weight:950;letter-spacing:.02em;box-shadow:0 12px 34px #0006;backdrop-filter:blur(14px);cursor:pointer;touch-action:manipulation}.visibleLogout:disabled,.accountMenu button:disabled{opacity:.55;cursor:not-allowed}.avatar{display:grid;place-items:center;width:30px;height:30px;border-radius:50%;background:linear-gradient(135deg,#f1d77c,#b88428);color:#07130e;font-weight:1000}.accountTrigger b{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:11px}.accountTrigger i{color:#d8bf62;font-style:normal}.accountMenu{position:absolute;right:0;top:51px;width:230px;padding:10px;border:1px solid rgba(216,191,98,.25);border-radius:17px;background:#03120ff5;box-shadow:0 22px 60px #0009;backdrop-filter:blur(20px)}.accountMenu small{display:block;padding:6px 8px 9px;color:#d8bf62;font-size:9px;letter-spacing:.1em;font-weight:900}.accountMenu button{display:block;width:100%;min-height:44px;border:0;border-radius:10px;background:transparent;color:#dce7e2;text-align:left;padding:10px 9px;font-weight:800;cursor:pointer;touch-action:manipulation}.accountMenu button:hover{background:#ffffff0d}.accountMenu .signout{margin-top:5px;border-top:1px solid #ffffff10;color:#ffb9b2}.signOutError{position:absolute;right:0;top:52px;width:min(320px,calc(100vw - 28px));padding:10px 12px;border:1px solid rgba(255,126,116,.32);border-radius:12px;background:#3a1110f2;color:#ffc1bb;font-size:10px;font-weight:800;line-height:1.4;box-shadow:0 14px 40px #0008}@media(max-width:720px){.accountNav{right:max(10px,env(safe-area-inset-right));top:max(62px,calc(env(safe-area-inset-top) + 48px))}.accountTrigger{max-width:118px;min-height:44px}.accountTrigger b{display:none}.visibleLogout{min-height:44px;padding:0 11px;font-size:10px}.accountMenu{width:220px}}`}</style>
  </div>;
}
