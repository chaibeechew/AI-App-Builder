...
export default async function CombinedPreviewPage({params}){
  ...
  const pinned=encodeURIComponent(versionId);
  return <main className="combinedPreview" data-project-id={id} data-version-id={versionId}>
    <div className="wrap">
      <header className="top"><div><small>ONE PROJECT · ONE CURRENT VERSION</small><h1>{app.name||"Your Project"}</h1><p>App and Website below are locked to the same saved project version{versionNo?` · v${versionNo}`:""}. Modify once, then reopen this page to review both updated surfaces together.</p></div><div className="topActions"><Link href={`/app-dashboard/${id}`}>Project</Link><Link className="gold" href={`/release/${id}`}>Publish Center →</Link></div></header>
      <section className="deliveryStatus"><span>✓ Same project ID</span><span>✓ Same version ID</span><span>✓ Same Brand Kit & memory</span><span>✓ Independent customer surfaces</span></section>
      <section className="previewGrid">
        <article data-surface="app"><div className="previewHead"><div><small>01 · MOBILE APP</small><h2>App Preview</h2></div><Link href={`/a/${id}`}>Open Full App ↗</Link></div><div className="phoneStage"><iframe title={`${app.name||"Project"} App Preview`} src={`/a/${id}?surface=app&previewVersion=${pinned}`} loading="eager"/></div></article>
        <article data-surface="website"><div className="previewHead"><div><small>02 · RESPONSIVE WEB</small><h2>Website Preview</h2></div><Link href={`/website/${id}`}>Open Full Website ↗</Link></div><div className="webStage"><iframe title={`${app.name||"Project"} Website Preview`} src={`/website/${id}?surface=website&previewVersion=${pinned}`} loading="eager"/></div></article>
      </section>
      <section className="next"><div><small>REFINE BOTH</small><h2>One edit updates the shared project version.</h2><p>Use AI Modify from the project workspace. App and Website remain synchronized because neither surface owns a separate shadow specification. Reopen Preview Both after saving to lock both surfaces to the new current version.</p></div><Link href={`/editor/${id}`}>Modify with AI →</Link></section>
    </div>
