...
export default function DeviceEvidenceV7Panel({route="/game-world-v7"}){
  ...
  useEffect(()=>{
    if(!consent){sessionRef.current=null;setExported(null);return;}
    ...
    sessionRef.current=s;setExported(sanitizeDeviceEvidenceForExportV7(s));
    const tick=()=>{
      if(!sessionRef.current)return;
      const e=window.__LANERIQ_V6_EVIDENCE__,sample=e?.samples?.at?.(-1)||null,summary=e?.summary;
      const now=performance.now();
      if(document.visibilityState==="visible"&&now-lastSample.current>=700&&(sample||summary?.medianFps>0)){
        const fps=Number(sample?.fps||summary?.medianFps||0),frameMs=Number(sample?.frameMs||summary?.medianFrameMs||(fps?1000/fps:0));
        sessionRef.current=recordDeviceFrameSampleV7(sessionRef.current,{fps,frameMs,longFrameRatio:Number(sample?.longFrameRatio||summary?.avgLongFrameRatio||0),residentChunks:Number(sample?.residentChunks||summary?.maxResidentChunks||0),renderScale:Number(sample?.renderScale||1),foreground:true,elapsedMs:now});
        lastSample.current=now;setExported(sanitizeDeviceEvidenceForExportV7(sessionRef.current));
      }
    };
  ...
  return <article className={styles.panel} data-testid="device-evidence-v7">
    ...
    <div className={styles.list}>
      <label><span>Device claim</span><select value={claim} onChange={e=>setClaim(e.target.value)}><option value="unknown">Unknown</option><option value="iphone">iPhone</option><option value="android">Android</option><option value="tablet">Tablet</option><option value="desktop">Desktop</option></select></label>
      <label><span>Consent</span><input type="checkbox" checked={consent} onChange={e=>setConsent(e.target.checked)}/></label>
      ...
      <div><span>Native device attested</span><b>{String(exported?.truth?.nativeDeviceAttested===true)}</b></div>
      <div><span>Median FPS</span><b>{Math.round(exported?.summary?.medianFps||0)}</b></div>
