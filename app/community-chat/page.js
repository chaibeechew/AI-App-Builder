    ...
    <main className="chatPage">
      <section className="chatShell">
        <header className="chatHeader">
          <div>
            <div className="eyebrow">OPTIONAL COMMUNITY</div>
            <h1>{room?.name || "Community Chat"}</h1>
            <p>{room?.description || "Chat with other users and the AI assistant."}</p>
          </div>
          <div className="headerActions">
            {joined ? <button className="ghostButton" onClick={closeChat}>Close Chat</button> : <button className="primaryButton" onClick={openChat}>Open Chat</button>}
            <a className="ghostButton" href="/my-apps">My Apps</a>
          </div>
        </header>
        ...
        {!joined ? (
          <section className="optInCard">
            ...
            <h2>Community Chat is off</h2>
            <p>This chat is not opened automatically. Tap <strong>Open Chat</strong> only when you want to join. You can close it at any time.</p>
            <button className="primaryButton large" onClick={openChat}>Open Community Chat</button>
          </section>
        ) : (
          <>
            <div className="notice">You chose to open this chat. Other participating users can see community messages. The AI assistant is available in the same room.</div>
            <section className="messages" aria-live="polite">
              {messages.length ? messages.map((message) => (
                <article className={`message ${message.sender_type}`} key={message.id}>
                  <div className="messageLabel">{message.sender_type === "ai" ? "AI Assistant" : message.sender_type === "system" ? "System" : message.user_id === user?.id ? "You" : "User"}</div>
                  <div className="bubble">{message.body}</div>
                </article>
              )) : <div className="empty">No messages yet. Start the conversation.</div>}
            </section>
            <form className="composer" onSubmit={sendMessage}>
              <textarea value={text} onChange={(event) => setText(event.target.value)} maxLength={4000} placeholder="Message the community or ask the AI…" rows={2} disabled={sending} />
              <button className="primaryButton" disabled={sending || !text.trim()}>{sending ? "Sending…" : "Send"}</button>
            </form>
          </>
        )}
