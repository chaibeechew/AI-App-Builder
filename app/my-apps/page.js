...
export default async function MyAppsPage() {
  ...
  // Graceful fallback while the Supabase visibility migration is being applied.
  let apps = appsResult.data || [];
  let appsError = appsResult.error;
  if (appsResult.error) {
    ...
    <main className="appsPage">
      <header className="appsHeader">
        <div>
          ...
          <div className="ownerLine">Owner: <strong>{ownerName}</strong></div>
        </div>
      ...
      <section className="referralCard">
        ...
        <div className="rewardBox"><span>Reward records</span><strong>{rewards?.length || 0}</strong></div>
      </section>
          ...
          <article className="appCard" key={app.id}>
            <div className="appCardTop">
              ...
              <div className="statusStack">
                <span className="statusPill">{app.publish_status === "published" ? "Published" : "Draft"}</span>
                <span className="visibilityPill">{app.visibility === "listed" ? "🌐 Listed" : "🔒 Private"}</span>
              </div>
            </div>
            <h2>{app.name}</h2>
            ...
            <div className="appActions">
              <Link href={`/editor/${app.id}`} className="editButton">Open / Modify →</Link>
              <Link href={`/apps/${app.id}/publish`} className="manageButton">Publish settings</Link>
            </div>
          </article>
        )) : (
