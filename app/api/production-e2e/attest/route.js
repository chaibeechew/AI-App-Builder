...
export async function POST(request) {
  try {
    ...
    if (Buffer.byteLength(canonicalReport, "utf8") > MAX_REPORT_BYTES) {
      return json({ error: "Evidence report is too large." }, 413);
    }
    ...
    if (!Number.isFinite(generatedAtMs) || ageMs < -120_000 || ageMs > MAX_REPORT_AGE_MS) {
      return json({ error: "Evidence report is stale or has an invalid generatedAt timestamp." }, 422);
    }

    if (
      report?.build?.commitSha !== build.commitSha ||
      report?.build?.commitRef !== "main" ||
      report?.build?.environment !== "production" ||
      report?.build?.exactProductionBuildVerified !== true
    ) {
    ...
    if (requiredClaims.some((claim) => !claim)) {
    ...
    if (String(project?.current_version_id || "") !== currentVersionId) {
      return json({ error: "Evidence current version no longer matches the owned Production project." }, 409);
    }
    if (publicState(project)) {
      return json({ error: "Evidence project is still public; release attestation is blocked until cleanup is proven." }, 409);
    }
