...
export async function GET() {
  try {
    ...
    const [cash, withdrawals, refunds, payoutAccounts] = await Promise.all([
      ...
      supabase.from("refund_requests").select("id,amount,status,created_at").eq("user_id", user.id).order("created_at", { ascending: false }).limit(50),
      supabase.from("payout_accounts").select("id,label,status,cooling_until").eq("user_id", user.id).order("created_at", { ascending: false })
    ]);
