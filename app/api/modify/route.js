...
export async function POST(request){
 ...
 try{
  ...
  const ai=await withTimeout(runBuilderAi(prompt,{userId,appId,stage:"modify",baseVersionId}),PRIMARY_AI_TIMEOUT_MS,"Primary AI modification"),finalProviderStart=ai.provider;
  ...
  if(qualityRegressed(currentQuality,candidate.quality)){
    ...
    const repairedAI=await withTimeout(runBuilderAi(repairPrompt,{userId,appId,stage:"quality-repair",baseVersionId}),REPAIR_AI_TIMEOUT_MS,"AI quality repair"),repaired=normalizeAndTest(extractJson(repairedAI.result));
  ...
  if(!candidate.selfHeal.passed){
    ...
    const healedAI=await withTimeout(runBuilderAi(selfHealPrompt,{userId,appId,stage:"self-heal",baseVersionId}),REPAIR_AI_TIMEOUT_MS,"AI self-heal"),healed=normalizeAndTest(extractJson(healedAI.result));
  ...
  if(message.includes("All authorized AI providers failed")||message.includes("No admitted zero-cost AI capacity")||message.includes("blocked by zero-cost admission policy"))return NextResponse.json({error:"Zero-cost AI capacity is temporarily unavailable. Please try again."},{status:503});
