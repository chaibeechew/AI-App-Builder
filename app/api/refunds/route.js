...
export async function POST(request) {
  try {
    ...
    const { data, error } = await supabase.rpc("request_subscription_refund", { p_subscription_id: subscriptionId, p_reason: reason || null });
    if (error) {
      const message = error.message?.toLowerCase() || "";
      if (message.includes("expired") || message.includes("already requested") || message.includes("not found")) return NextResponse.json({ success: false, error: error.message }, { status: 400 });
