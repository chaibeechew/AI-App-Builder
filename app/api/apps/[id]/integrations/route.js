...
export async function POST(request,{params}){
  try{const {id}=await params;const supabase=await createClient();const {data:{user}}=await supabase.auth.getUser();if(!user)return NextResponse.json({error:"Authentication required."},{status:401});const {data:app}=await supabase.from("apps").select("id,owner_id").eq("id",id).eq("owner_id",user.id).single();if(!app)return NextResponse.json({error:"Project not found."},{status:404});const body=await request.json().catch(()=>({}));const type=String(body?.type||"").trim().slice(0,80);if(!["email","sms","calendar","payments","maps","whatsapp"].includes(type))return NextResponse.json({error:"Unsupported integration type."},{status:400});const display={email:"Email Delivery",sms:"SMS Delivery",calendar:"Calendar",payments:"Payments",maps:"Maps",whatsapp:"WhatsApp"}[type];const safeConfig=sanitizeConfig(body?.config&&typeof body.config==="object"?body.config:{});const {data,error}=await supabase.from("project_integrations").upsert({app_id:id,owner_id:user.id,integration_type:type,display_name:display,enabled:body?.enabled!==false,config:safeConfig||{},updated_at:new Date().toISOString()},{onConflict:"app_id,integration_type"}).select("id,integration_type,display_name,enabled,config,updated_at").single();if(error)throw error;return NextResponse.json({success:true,integration:data,note:"Secrets and credential-like fields are recursively removed from project-visible integration config."});}catch(error){console.error("INTEGRATIONS_POST_ERROR",error);return NextResponse.json({error:"Unable to save integration settings."},{status:500});}
}
