...
function buildSuggestedSchema(spec) {
  ...
  const entities = [
    entity("users", ["id: uuid", "display_name: text", "email: text", "created_at: timestamptz"], "Account profile only; authentication secrets are never stored here."),
  ];
  ...
  return {
    ...
    policies: [
      "Private by default",
      "Signed-in users can access only rows they own unless a feature explicitly requires sharing",
      "Sensitive writes require server-side validation",
      "No API keys, passwords or payment credentials in generated business tables",
      "Deletion/export paths should exist for personal data where relevant",
    ],
  };
}
