...
function suggestedSchema(spec){
  const text=featureText(spec),entities=[entity("users",["id: uuid","display_name: text","email: text","created_at: timestamptz"],"Account profile only; authentication secrets are never stored here.")];
...
export async function POST(request,{params}){
  try{
    ...
    if(plan?.modules?.database){const schema=suggestedSchema(version?.specification||{});const {data,error}=await supabase.from("app_backend_models").upsert({app_id:id,owner_id:user.id,schema_json:schema,status:"ready",updated_at:new Date().toISOString()},{onConflict:"app_id"}).select("id,status").single();results.database=error?{status:"failed",message:error.message}:{status:"ready",id:data.id};}

    if(plan?.modules?.workflows&&Array.isArray(plan.workflows)){
      const {data:existing}=await supabase.from("app_workflows").select("name").eq("app_id",id).eq("owner_id",user.id);const names=new Set((existing||[]).map(x=>x.name));
    ...
    if(plan?.modules?.video){
      ...
      if(existingVideo)results.video={status:"existing",id:existingVideo.id};else{const style=["realistic","cartoon","mixed"].includes(plan?.video?.style)?plan.video.style:"realistic";const deviceClass=["mobile","desktop","high_performance_desktop"].includes(body?.deviceClass)?body.deviceClass:"mobile";const maxDuration=deviceClass==="mobile"?60:120;const {data,error}=await supabase.from("video_projects").insert({owner_id:user.id,app_id:id,name:`${app.name} Promo Video`,style,device_class:deviceClass,max_duration_seconds:maxDuration,edit_json:{tracks:[],settings:{aspectRatio:"9:16",autoConnect:true,serverRender:true}},status:"draft"}).select("id,status").single();results.video=error?{status:"failed",message:error.message}:{status:"ready",id:data.id};}
    }
