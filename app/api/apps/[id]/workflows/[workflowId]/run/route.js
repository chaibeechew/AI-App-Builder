...
async function runAction(supabase,{action,appId,userId,payload,workflowName}){
  if(action.type==="save_crm"||action.type==="save_order"){
    ...
    if(error)throw error;return {type:action.type,status:"completed",record:data};
  }
  if(action.type==="notify_team"){
    ...
    if(error)throw error;return {type:action.type,status:"completed",channel:"in_app",message:"Team notification added to AI Operations."};
  }
  if(action.type==="send_email"){
    const to=action?.config?.to||payload.email;if(!to)return {type:action.type,status:"skipped",message:"No recipient email was supplied."};
  ...
  if(action.type==="send_sms"){
    const to=action?.config?.to||payload.phone;if(!to)return {type:action.type,status:"skipped",message:"No recipient phone number was supplied."};
  ...
  if(action.type==="send_whatsapp"){
    const to=action?.config?.to||payload.whatsapp||payload.phone;if(!to)return {type:action.type,status:"skipped",message:"No WhatsApp recipient was supplied."};
    const result=await sendManagedWhatsApp({to,body:action?.config?.body||`${workflowName}: your request has been received.`});
    return {type:action.type,...result,message:result.status==="completed"?"WhatsApp message sent through the managed delivery layer.":"WhatsApp delivery is not connected yet."};
  }
...
export async function POST(request,{params}){
  let runId=null;
  try{
    const {id,workflowId}=await params;const supabase=await createClient();const {data:{user}}=await supabase.auth.getUser();
    ...
    const body=await request.json().catch(()=>({}));const payload=cleanPayload(body?.payload||{});
    const {data:run,error:runError}=await supabase.from("workflow_runs").insert({app_id:id,workflow_id:workflowId,owner_id:user.id,trigger_payload:payload,status:"started"}).select("id").single();if(runError)throw runError;runId=run.id;
    const results=[];for(const action of Array.isArray(workflow.actions)?workflow.actions.slice(0,12):[]){try{results.push(await runAction(supabase,{action,appId:id,userId:user.id,payload,workflowName:workflow.name}));}catch(error){results.push({type:action?.type||"unknown",status:"failed",message:error?.message||"Action failed."});}}
    const failed=results.filter(x=>x.status==="failed").length;const pending=results.filter(x=>x.status==="integration_required").length;const status=failed?"partial":pending?"partial":"completed";
  ...
  }catch(error){console.error("WORKFLOW_RUN_ERROR",error);return NextResponse.json({error:error?.message||"Unable to run workflow.",runId},{status:500});}
}
