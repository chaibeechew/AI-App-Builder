...
export async function GET(request,{params}){
  try{
    ...
    const{data:asset,error}=await supabase.from("asset_library").select("id,storage_path,file_name,mime_type").eq("id",id).eq("user_id",user.id).maybeSingle();if(error)throw error;
    if(!asset||asset.mime_type!=="video/mp4")return noStore({error:"Video output not found."},404);
