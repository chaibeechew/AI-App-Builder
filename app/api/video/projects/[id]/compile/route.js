...
export async function POST(request,{params}){
  try{
    ...
    const editJson={
      version:1,
      autoConnected:Boolean(body?.autoConnect!==false),
      aspectRatio:["9:16","16:9","1:1"].includes(body?.aspectRatio)?body.aspectRatio:"9:16",
      clips:clips.map((clip,index)=>({...clip,order:index})),
      audio:{musicAssetId:body?.musicAssetId||null,voiceOverAssetId:body?.voiceOverAssetId||null,normalize:true},
      branding:{logoAssetId:body?.logoAssetId||null},
      render:{location:"server",quality:body?.quality==="best"?"best":"balanced",devicePreviewOnly:true},
    };
    const {data:version,error}=await supabase.from("video_versions").insert({project_id:id,owner_id:user.id,version_no:versionNo,edit_json:editJson,duration_seconds:total,render_status:"queued"}).select("id,version_no,duration_seconds,render_status,created_at").single();
    if(error)throw error;
    await supabase.from("video_projects").update({edit_json:editJson,status:"rendering",updated_at:new Date().toISOString()}).eq("id",id).eq("owner_id",user.id);
    return NextResponse.json({success:true,version,renderPlan:{serverRender:true,autoConnect:editJson.autoConnected,clipCount:clips.length,durationSeconds:Number(total.toFixed(2)),aspectRatio:editJson.aspectRatio,experience:policy.label,note:"Final compile is queued for server-side rendering so the customer device only handles lightweight preview work."}});
