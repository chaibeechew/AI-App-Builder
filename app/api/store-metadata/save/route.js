...
export async function POST(request) {
  try {
    ...
    const { data, error } = await supabase
      .from("store_listings")
      .upsert({ app_id: appId, version_id: versionId, language, apple, google_play: googlePlay, checklist }, { onConflict: "app_id,version_id,language" })
      .select("*")
      .single();

    if (error) {
      console.error("STORE_LISTING_SAVE_ERROR:", error);
