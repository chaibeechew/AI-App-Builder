...
const allowedTasks=new Set(["create","edit-video","animate-photo","restyle-photo"]);
...
export async function POST(request){
 try{
  ...
  if(hasImage){if(referenceImage.size>MAX_IMAGE)return NextResponse.json({success:false,error:"Photo must be 20 MB or smaller."},{status:413});if(!/^image\//.test(referenceImage.type))return NextResponse.json({success:false,error:"Photo must be an image file."},{status:400});}
  if(hasVideo){if(sourceVideo.size>MAX_VIDEO)return NextResponse.json({success:false,error:"Video must be 200 MB or smaller."},{status:413});if(!/^video\//.test(sourceVideo.type))return NextResponse.json({success:false,error:"Uploaded media must be a video file."},{status:400});}
  if(task==="edit-video"&&!hasVideo)return NextResponse.json({success:false,error:"Upload the video you want to modify."},{status:400});
  if((task==="animate-photo"||task==="restyle-photo")&&!hasImage)return NextResponse.json({success:false,error:"Upload the photo you want to use."},{status:400});
  ...
  return NextResponse.json({success:true,jobId:data.jobId||data.id||null,status:data.status||"queued",videoUrl:data.videoUrl||data.url||null,task,mode,voiceSource});
