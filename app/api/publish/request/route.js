...
export async function POST(request) {
  try {
    ...
    if (!appId || !versionId || !listingId || !["apple", "google_play"].includes(platform)) {
      return NextResponse.json({ error: "appId, versionId, listingId and a valid platform are required." }, { status: 400 });
    }
    ...
    const { data, error } = await supabase.from("publish_requests").insert({
      app_id: appId,
      version_id: versionId,
      store_listing_id: listingId,
      platform,
      status: "customer_approved",
      requested_by: user.id,
      customer_approved_at: listing.customer_approved_at,
    }).select("id,app_id,version_id,platform,status,created_at").single();

    if (error) {
      console.error("PUBLISH_REQUEST_ERROR:", error);
