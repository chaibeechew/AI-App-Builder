...
export async function GET(request) {
  try {
    ...
    for (const [key, value] of Object.entries(spec)) rows.push([key, value]);
    const csv = rows.map((row) => row.map(csvCell).join(",")).join("\n");
    return new Response(csv, { status: 200, headers: { "Content-Type": "text/csv; charset=utf-8", "Content-Disposition": `attachment; filename="${app.id}.csv"`, "Cache-Control": "private, no-store" } });
  } catch (error) {
