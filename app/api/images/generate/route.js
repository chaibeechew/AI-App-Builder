  ...
  try{
    ...
    if(mode!=="icon"&&gateway.configured){
      ...
      if(generated.generated){
        ...
        return noStore({success:true,image:images[0]?.image||null,images,engine:"Soolen Image Runtime",generated:true,mode,style,palette:paletteName,source:"model",placement,note:"Generated for its intended App + Website placement with responsive crop guidance. Provider credentials and routing stay server-side."});
      }
    }
    ...
    for(let i=0;i<count;i++){
      if(mode==="icon"){
        const svg=iconSvg({primary,accent,background,variation:i,title:body?.includeText?prompt.split(/[.!?。！？]/)[0]:""});images.push({id:`icon-${i+1}`,image:`data:image/svg+xml;charset=UTF-8,${encodeURIComponent(svg)}`,mode,style,palette:paletteName,source:"local",placement});continue;
      }
    ...
    return noStore({success:true,image:images[0]?.image||null,images,engine:"Soolen Visual Engine",generated:true,mode,style,palette:paletteName,source:"local",placement,note:gateway.blockedByCostPolicy?"A connected external image runtime is blocked by the current zero-cost policy, so the original local visual engine was used instead with placement-aware composition guidance.":"Original prompt-driven SVG visual generation with placement-aware responsive composition guidance. This local visual engine is not presented as photorealistic external-model output."});
  }catch(error){
