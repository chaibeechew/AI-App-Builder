...
export async function POST(request) {
  ...
  try {
    ...
    const rawReferenceImages=body?.referenceImages||body?.imageRefs||[];
    const referenceImages=Array.isArray(rawReferenceImages)?rawReferenceImages.filter(v=>typeof v==="string"&&v.trim()).slice(0,10):[];
    const sketchImage=String(body?.sketchImage||body?.referenceImage||"").trim();
    if(sketchImage && referenceImages.length<10) referenceImages.push(sketchImage);
    ...
    if(!idea&&!voiceTranscript&&!referenceImages.length) return NextResponse.json({success:false,error:"Please describe the app you want to build or upload a sketch/reference image."},{status:400});
    const combinedInput=[idea,voiceTranscript,sketchImage?"[Sketch/reference image uploaded]":""].filter(Boolean).join("\n\n");
    ...
    if(!entitlement?.allowed){
      const {data:charge,error:chargeError}=await supabase.rpc("consume_ai_credits",{p_amount:GENERATE_CREDIT_COST,p_request_id:chargeRequestId,p_description:"AI app generation",p_metadata:{operation:"generate",hasSketch:Boolean(sketchImage),referenceImageCount:referenceImages.length}});
    ...
    const result=await runAutonomousEngine(idea||voiceTranscript||"Create an app from the uploaded sketch.",{voiceTranscript,referenceImages,language,industry,terminology,createDemoVideo});
    ...
    const {data:version,error:versionError}=await supabase.from("app_versions").insert({app_id:app.id,version_no:1,specification,change_summary:sketchImage?"Initial Soolen AI app generated from idea/sketch":"Initial Soolen AI-generated application",created_by:user.id}).select("id,version_no,created_at").single();
    ...
    return NextResponse.json({success:true,...result,specification,explanation:buildAppExplanation(specification),selfTest:test,entitlement:{source:entitlementSource,charged},pricing:{mode:"transparent",estimatedCredits:GENERATE_CREDIT_COST,failedGenerationRefund:true},credits:{charged:charged?GENERATE_CREDIT_COST:0,requestId:chargeRequestId},input:{hasSketch:Boolean(sketchImage),referenceImageCount:referenceImages.length},app:{id:savedApp.id,name:savedApp.name,versionId:version.id,versionNo:version.version_no,visibility:savedApp.visibility,publishStatus:savedApp.publish_status}});
  } catch(error){
