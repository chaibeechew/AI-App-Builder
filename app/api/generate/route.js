...
function verifyGeneration(result){
  ...
  return {passed:test.ok,test,normalized:test.normalizedSpec};
}
...
export async function POST(request){
 ...
 try{
  ...
  const adult=await runSoolenAdultMode({taskType:"app-build",goal:combinedInput,privateData:referenceImages.length>0,requirements:body?.requirements||{},executors:[{id:"soolen-autonomous-engine",available:true,local:false,requiresNetwork:true,baseScore:50,historicalSuccess:0.5}],permissions:{network:true,privateUpload:referenceImages.length>0}}, {
   ...
   verify:async(result)=>verifyGeneration(result),
   repair:async({result,review})=>runAutonomousEngine(`${combinedInput}\n\nRepair the generated application. Fix these verification failures only: ${review.failed.map(x=>x.id).join(", ")}. Preserve working requirements and return a complete corrected specification.`,generationOptions)
  });
  ...
  const verified=verifyGeneration(adult.result);if(!verified.passed)throw new Error(`Generated app failed final self-test: ${verified.test.errors.join("; ")}`);const specification=verified.normalized;
  ...
  const {data:version,error:versionError}=await supabase.from("app_versions").insert({app_id:app.id,version_no:1,specification,change_summary:"Initial Soolen Super Brain generated and verified application",created_by:user.id}).select("id,version_no,created_at").single();if(versionError)throw new Error(`App version save failed: ${versionError.message}`);
