...
function buildBrandBrief(kit){
  if(!kit)return "";
  const rows=[
    kit.company_name&&`Brand/company: ${kit.company_name}`,
    kit.primary_color&&`Primary color: ${kit.primary_color}`,
    kit.secondary_color&&`Secondary color: ${kit.secondary_color}`,
    kit.accent_color&&`Accent color: ${kit.accent_color}`,
    kit.font_style&&`Typography direction: ${kit.font_style}`,
    kit.brand_voice&&`Brand voice: ${kit.brand_voice}`,
    kit.logo_url&&`Logo reference: ${kit.logo_url}`,
  ].filter(Boolean);
...
export async function POST(request){
 ...
 try{
  ...
  const brandBrief=buildBrandBrief(brandKit);
  const buildInput=[combinedInput,brandBrief].filter(Boolean).join("\n\n");
  ...
  const adult=await runSoolenAdultMode({taskType:"app-build",goal:buildInput,privateData:referenceImages.length>0,requirements:{...(body?.requirements||{}),brandKit:brandKit||undefined},executors:[{id:"soolen-autonomous-engine",available:true,local:false,requiresNetwork:true,baseScore:50,historicalSuccess:0.5}],permissions:{network:true,privateUpload:referenceImages.length>0}}, {
   ...
   repair:async({result,review,verification})=>{
     const report=verification?.report||verifyGeneration(result);const criticFailures=(review?.failed||[]).map(x=>x.id);const instruction=buildRepairInstruction(report.execution||{});
     return runAutonomousEngine(`${buildInput}\n\nSOOLEN AUTONOMOUS REPAIR MODE\n${instruction}\nCritic failures: ${criticFailures.join(", ")||"none"}\nSelf-test failures: ${(report.selfTest?.errors||[]).join(", ")||"none"}\nDo not remove working features. Preserve the saved Brand Kit unless it conflicts with accessibility or safety. Return the full corrected specification only.`,generationOptions);
   }
  });
