...
export default function ReleaseOptionsPage({ params }) {
  ...
  const blockers=useMemo(()=>{
    ...
    return rows.filter(x=>Number(x.score||0)<Number(x.target||99));
  },[quality]);
  ...
  const repairPrompt = blockers.length ? `Improve this project until every release quality area reaches the 99 release target. Focus especially on: ${blockers.map(x=>LABELS[x.id]||x.name).join(", ")}. Preserve all working features, customer data, permissions and brand identity.` : "Review the whole project for release readiness and improve any weak areas without removing working features.";
  ...
  return <main className="releasePage"><div className="wrap">
    ...
    <section className={canPublish ? "quality ready" : "quality attention"}>
      <div className="qualityHead"><div><div className="eyebrow">99 RELEASE CHECK</div><h2>{canPublish ? "Ready for release" : "AI found items to improve"}</h2></div><strong>{quality?.report?.overall ?? "—"}<small>/100</small></strong></div>
