...
export default function AssetLibraryPage(){
  ...
  async function upload(event){
    ...
    if(!ACCEPTED.some(v=>v.endsWith("/")?file.type.startsWith(v):file.type===v)){setError("Use an image, video or PDF file.");return;}
  ...
  return <main className="assetPage"><div className="wrap">
    ...
    <header><div><small>REUSE, DON'T RE-UPLOAD</small><h1>Your creative assets.</h1><p>Keep logos, photos, videos and documents in one private library. Assets stay user-scoped and are served with short-lived signed preview links.</p></div><label className="upload">{uploading?"Uploading…":"+ Add Asset"}<input type="file" accept="image/*,video/*,application/pdf" onChange={upload} disabled={uploading}/></label></header>
    <div className="privacy">🔐 Private by default · per-user RLS · signed previews expire automatically</div>
    ...
    {loading?<div className="empty">Loading your assets…</div>:assets.length?<section className="grid">{assets.map(item=><article key={item.id}>
      <div className="preview">{item.category==="image"&&previews[item.id]?<img src={previews[item.id]} alt={item.alt_text||item.file_name}/>:item.category==="video"&&previews[item.id]?<video src={previews[item.id]} controls preload="metadata"/>:<div className="fileIcon">{item.category==="document"?"PDF":"FILE"}</div>}</div>
