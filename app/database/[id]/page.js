...
export default function DatabaseBuilder({ params }) {
  ...
  async function generate() {
    ...
    try {
      ...
      setMessage(`Data structure v${data.version || "new"} is ready. Your previous version is kept for rollback.`); await load();
    } catch (err) { setError(err.message); }
  ...
  async function rollback(version) {
    ...
    try{
      ...
      setMessage(`Restored data structure v${data.restoredFrom} safely as new version v${data.newVersion}.`);await load();
  ...
  return <main className="page"><div className="wrap">
    ...
    <header><div><div className="eyebrow">AI APP BUILDER · DATA</div><h1>Your App Data</h1><p>{app?.name || "Project"} · No SQL or database knowledge needed. AI organizes the data safely behind your App + Website.</p></div><span className="badge">PRIVATE BY DEFAULT</span></header>

    <section className="heroCard"><div><small>NO-CODE DATA</small><h2>Tell AI what the App does.<br/>We organize the data behind it.</h2><p>You see Customers, Properties, Orders or Appointments — not infrastructure, passwords or database keys.</p></div><button onClick={generate} disabled={building}>{building ? "Working safely…" : model ? "✨ Update from Current App" : "✨ Build My Data Structure"}</button></section>

    {message && <div className="success">{message}</div>}{error && <div className="error">{error}</div>}

    {model ? <>
      <section className="summary"><article><small>Status</small><strong>{model.status === "ready" ? "Ready" : model.status}</strong></article><article><small>Data Version</small><strong>v{schema.version || 1}</strong></article><article><small>Data Groups</small><strong>{entities.length}</strong></article><article><small>Saved Rollbacks</small><strong>{history.length}</strong></article></section>
      ...
      {history.length>0&&<section className="panel"><div className="eyebrow">UNDO DATA CHANGES</div><h2>Previous data structures</h2><p>Restoring never deletes the current version. It creates a new version from the one you choose.</p><div className="history">{[...history].reverse().map(item=><div key={`${item.version}-${item.savedAt}`}><span><b>Version {item.version}</b><small>{item.savedAt?new Date(item.savedAt).toLocaleString():"Saved version"}</small></span><button disabled={building} onClick={()=>rollback(item.version)}>Restore this version</button></div>)}</div></section>}

      <div className="next"><b>Next:</b> Automations can use these data groups for flows such as enquiry → customer record → notification → follow-up. You still do not need code.</div>
    </> : <section className="empty"><h2>No data model yet</h2><p>Tap “Build My Data Structure” and AI App Builder will create a safe first version from the current App + Website.</p></section>}
  </div><style jsx>{`
