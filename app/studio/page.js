...
const modules = [
  ...
  {name:"Export & Ownership",description:"Keep project portability, exportability and customer ownership visible.",live:false},
];
  ...
  return (
    <main className="studioShell">
      ...
      <section className="heroStudio">
        ...
        <p>AI App Builder is organized as one continuous service: idea → design → build → data → automation → quality → preview → publish → improve.</p>
      ...
      <section className="modules">
        <div className="sectionHead"><div><small>ONE-STOP PLATFORM</small><h2>Everything stays inside the builder</h2></div><b>{modules.filter(m=>m.live).length} modules connected</b></div>
        <div className="moduleGrid">
          {modules.map((item, index) => {
            const card=<><span>{String(index + 1).padStart(2,"0")} · {item.live?"LIVE":"NEXT"}</span><h3>{item.name}</h3><p>{item.description}</p>{item.href&&item.live?<b className="open">Open →</b>:null}</>;
            return item.href&&item.live?<Link className="moduleCard" href={item.href} key={item.name}>{card}</Link>:<article className="moduleCard" key={item.name}>{card}</article>;
          })}
        </div>
      </section>
