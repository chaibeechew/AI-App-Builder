...
export default function PublishPage({ params }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setAnswers((current) => ({ ...current, websiteUrl: `${window.location.origin}/website/${appId}` }));
  ...
  async function generate() {
    ...
    try {
      ...
      setMessage("SoolenAI auto-filled the Apple App Store and Google Play listing from your project plus the key answers below. Review everything before approval.");
    } catch (e) { setError(e.message); }
